...
def main() -> None:
    ...
    spec = task(
        ...
        due=_iso_days(-14),
        exit_criteria=[
            {"description": "scope agreed", "met": True},
            {"description": "API sketch reviewed", "met": True},
        ],
    ...
    contract = task(
        ...
        action_plan=[
            {"content": "draft OpenAPI", "completed": True},
            {"content": "review with frontend", "completed": True},
        ],
    )
    backend = task(  # the BOTTLENECK — many tasks depend on this
        ...
        labels=["backend", "api"],
        assignee="mei",
        depends_on=[contract, spec],
        due=_iso_days(-2),
        agent_instructions_template=(
            "Implement each endpoint behind the documented contract; add unit tests per handler."
        ),
        exit_criteria=[
        ...
        research_notes=[
            {"content": "Reused the existing pagination helper from the search module."}
        ],
    ...
    migrate = task(
        ...
        due=_iso_days(0),
        exit_criteria=[
            {"description": "migration written", "met": True},
            {"description": "rollback tested", "met": False},
        ],
    )
    frontend = task(
        ...
        due=_iso_days(5),
        action_plan=[
            {"content": "list view", "completed": False},
            {"content": "detail drawer", "completed": False},
        ],
    )
    seed_data = task(
    ...
    e2e = task(
        ...
        labels=["qa"],
        assignee="dana",
        depends_on=[frontend, seed_data],
        due=_iso_days(7),
        exit_criteria=[
            {"description": "happy path", "met": False},
            {"description": "error states", "met": False},
        ],
    )
    docs = task(
    ...
    task(
        ...
        notes=[
            {
                "content": (
                    "Blocked on the vendor — waiting for their new key-issuance window"
                    " (external, no prerequisite)."
                )
            }
        ],
    )  # manual block (no unfinished prereq) — exercises manual-vs-auto block
    task(
        title="Pen-test the login flow",
        task_list_id=sec_list,
        status="open",
        priority="high",
    ...
    task(
        title="Deprecate the legacy webhook",
        task_list_id=sec_list,
        status="cancelled",
        priority="medium",
        labels=["cleanup"],
        notes=[{"content": "Cancelled — the webhook has external consumers we can't migrate yet."}],
    )
    ...
    task(
        ...
        labels=["planning"],
        action_plan=[
            {"content": "gather input", "completed": True},
            {"content": "draft themes", "completed": False},
        ],
    )
    task(
    ...
    print(
        f"\nSeeded: {tasks['total']} tasks across {len(projects)} projects"
        f" / {len(lists)} task lists"
    )
