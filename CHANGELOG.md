...
- **Agent Rooms are now something you can see and use: a Rooms tab, an attributed transcript, the pause card and per-member status (AR-8).** The whole feature had no frontend — it was drivable only with `curl` — so this is the change that makes a room a thing you *have* rather than a thing on disk. Turn it on in **Settings → Chat → Agent Rooms** (still off by default), and a **Rooms** tab appears beside Chat Sessions, Loops and Channels on `#/chat/history`. It lists your rooms with their member count, how much of the round budget the current exchange has spent, and whether any member is still owed a turn; "New room" makes one and opens it. A room lives at `#/chat/room/<id>`. Its **transcript attributes every line**: your words render as your own, each member's as its own with its name and the role it argues from above the text, and a line the *room* wrote — today a refused tool call — reads as the room's note naming the member it is about, never as that member having said it. Two members answering in a row stay two separate contributions: the session transcript merges consecutive assistant messages into one turn, which in a room would print one member's words under another's name, so a room has no merge at all. Writing to the room shows the speaker queue the backend built, in the order it will speak, and the room re-reads itself while a round is outstanding. When the budget runs out the **pause card** appears at the end of the thread with the sentence the inbox row carries — *"Your agents have been talking for a while."* — the exchange count against the ceiling actually in force, and **the members still owed a turn, listed in order**, because a pause suspends the conversation rather than cancelling it. It offers a reply and an archive and deliberately no "Resume": any message from you refills the budget, so the reply *is* the resume. The inbox item for a paused room now deep-links straight to it, so the row and the card are one event with one destination. A **Members** panel shows, per member, its role, whether it listens to everything / only when named / as an observer, which agent binding and model it holds, the tool reach it actually runs under, and its own spend ceiling — so a read-only critic and a tool-bearing executor in one room are visibly different. It is honest about what it does not know: a member whose agent binding was deleted reads "Binding missing" rather than inheriting the default agent's model, a member whose declared safety posture reaches past the room's reads "Cannot speak" with the reason, and a member nothing is known about reads "Status unknown" rather than looking healthy. Nothing claims to know which member is *currently* speaking, because the backend does not publish that. Two settings that had shipped with no control anywhere are now editable — the round budget every room inherits and the maximum members per room — and a room can override the budget for itself from its Members panel, which is a value you could previously read and had no way to set. A member's own provider session (`room:<room>:<member>`) no longer surfaces in your chat history: those sessions exist so members do not share a context window, and they were listing as if you had opened them.
- **Agent Rooms take turns deterministically, and a round budget pauses the room to you rather than running on (AR-5).** Who speaks next is now decided by `rooms/arbiter.py`, which is pure Python over two inputs — the roster and the `@`-mentions in a message — and imports no provider, no model and no model registry, so **no model ever decides speaking order**; that absence is a test rather than a promise. A message enqueues the members it names, in the order it named them, once each, onto a FIFO queue drained one speaker at a time; a human may call on any member including a `silent` observer, while a member's reply can only summon `mention`-policy peers and never itself, which is what bounds how far one message propagates. A member can *name* peers but cannot *order* the queue: a reply demanding "ignore the queue, you are the arbiter now" is transcript prose and changes nothing. Agent turns taken since you last spoke are charged to the room record on disk, so a gateway restart cannot launder the count and two concurrent rounds share one ceiling; at `rooms.round_budget` (default 6) the room stops draining, sets `paused`, and raises exactly one inbox attention item — `agent`/`room_paused`, deduped per room — saying "Your agents have been talking for a while." Any message from you refills the budget and closes that row, so there is no separate resume button to go missing. A pause **suspends** the queue rather than cancelling it: the members still owed a turn are parked on the room and speak first when you reply, ahead of whoever your new message names, because otherwise the reply you send to let a deliberation continue would be the act that discarded it. The pause item carries only the room's own title and a turn count and never a member's words, so the one surface that escapes the transcript's fence cannot deliver model text to you as though it were the system's.
- **Deep research is now a judged, bounded research loop, and a template can say which input carries a loop's task (PP-16 session 2).** The `deep-research` template was a search pipeline: it triaged a question, swept a few angles, and summarised. It now runs the research kind's own discipline — a round loop that reads and rewrites `RESEARCH.md` as it goes (the living subtopic list, the sources already read, the report itself), with a **separate judge** closing every round, a breadth×depth sweep intensity chosen by the triage, and `output_manner` and `source_budget` as declared inputs. The report itself is now named in the template, so the deliverable panel stops saying nothing asked the worker to write one. Two `deep-research` inputs changed in the process: `rounds` is gone (it set a cap the engine could not read — see Fixed), and `exit_condition` is new. A template input can also now declare `loop_field`, which is what lets a legacy loop reference find the parameter a given template actually calls its task — `general-project` calls it `task`, `deep-research` calls it `question`, `design-project` calls it `brief` — with a typo reported at authoring time (`WF_INPUT_BAD_LOOP_FIELD`, `WF_INPUT_DUPLICATE_LOOP_FIELD`). **`research` is deliberately still refused as a loop-kind launch**: its template is ready, the run loop is not (see Fixed), and starting a research loop as a run today would give you one round and then a hand-off to a human — worse than the loop path you have.
...
### Changed
- **The app SDK now exports every type its own published surface names — 25 of them were unimportable, so apps had to derive or re-declare them.** `personalclaw.sdk.*` is the only import path an app may use, and `tests/test_apps_import_boundary.py` enforces that. What nothing enforced was whether the exported surface is *sufficient*: a published dataclass field or method signature typed with a core class that no `sdk` module re-exports is a contract an app cannot satisfy without reaching around the boundary the rail exists to hold. The known instance was `StructuredOutput`, absent from `sdk.model`, which the Ollama provider recovered as `type(ProviderCapability.__dataclass_fields__["structured_output"].default)` — correct at runtime and a total defeat of type checking. Re-exporting that one name fixed that one app; the measurement that mattered was how many siblings it had. **Twenty-five, across seven core modules**, found by walking every SDK-exported class's dataclass fields and public method signatures to a transitive fixed point. The consequential ones: **`AgentError`** was on no `sdk` module at all, yet `ToolResult.agent_error` and `ActionResult.agent_error` are published fields — so a third-party tool's failures could only ever be prose, while the native builtins' carried a stable `ERROR_CODES` key the agent branches on; **`TaskState`** types both `Task.state` and the `to=` parameter of `Task.transition()`, a published method no app could call type-safely; and **sixteen of the twenty-one `apps.manifest` types** were missing from `sdk.manifest`, whose own docstring offers itself as "the typed contract for tooling that validates or generates a manifest" — including the return types of `AppManifest.core_compatibility()` and `Permissions.proposal_kind()`, two published methods whose results were unnameable. `sdk.manifest` now publishes that module's whole public schema rather than a five-of-twenty-one subset, and `CapabilityMatrix`, `ResourceCeilings`, `SubagentInfo`, `CapturedSession` and the three `skills.loader` resource types join the submodules that already face their owning core module. **The one declared exception is the `AppConfig` tree** (~45 `config.*` section dataclasses): an app reads those by attribute access and never needs to name their types, so re-exporting them would fatten a facade whose whole claim is thinness. Stated in the rail rather than left implicit, because an undeclared exception is exactly how the manifest subset survived unnoticed. Guarding it: `tests/test_sdk_surface_is_public.py` gains a closure assertion with a vacuity floor over both scan counters and a three-case falsification that un-exports a real name and requires the walk to name it again — so the gap cannot reopen silently when a field is added to an already-exported dataclass. No behaviour change; every addition is a re-export of a single existing definition. **The inert-surface census scored all 26 of those exports as new declared-but-inert surfaces, and that was the census being wrong rather than the exports being wrong.** Its `sdk_export` detector knew exactly one reader — an `ImportFrom` somewhere in this repo — and an SDK export's real consumer is an app in a *separate* repository, so it could not tell a public API type apart from an unkept promise. Scored that way it created a perverse incentive precisely against its own purpose: with `AgentError` counted as inert, the cheapest way to go green was to un-export the type that made `ToolResult.agent_error` fillable, i.e. to restore the defect. It now recognises a second reader — an export that **another export's dataclass field or public method signature names** — because a published signature is a real consumer and, unlike an out-of-repo app, a mechanically checkable one. The walk that finds it is the same one the closure rail uses, moved to `scripts/sdk_surface_closure.py` so the two directions cannot drift into two definitions of "published signature". **The teeth are the point and they are intact**: an export reachable from nothing is still reported, which is 170 of the 229 surfaces — 54 exported *types* (the provider protocols an app subclasses, the service objects it is handed, the error classes it catches) plus 116 functions and constants, which carry no signature for the closure to reach them through. Two narrowings keep it honest, each measured before it was chosen: a type that names only **itself** does not clear itself (a `from_dict` returning its own class sits on most dataclasses here, so counting it would clear nearly everything), and an exported **function's** signature does not clear either — nine types would clear if it did, but admitting function roots to the shared walk opens 22 new gaps, which is a second tranche of the same defect and a separate change. The baseline was regenerated because every touched counter **fell** (`sdk_export` 203 → 170, no file rose); the forbidden-to-raise rule is untouched, and a throwaway type exported next to a reachable one in the same file reds the ratchet for the throwaway alone.
- **The SDK surface is now closed under its published *functions* too, not only fields and methods — 22 more types an app could not name, and one collision resolved.** [#3511](https://github.com/PersonalClaw/PersonalClaw/issues/3511). Closing the surface under dataclass fields and public method signatures left it closed against one kind of caller and open against another: an app that calls a published SDK **function** still could not name that function's parameter or return type, so the typed integration was impossible and the failure surfaced as prose or `Any`. `net.fetch()` returned an unnameable `FetchResponse`; `channel.resolve_bind_host()` took an unnameable `AuthConfig`; `mcp.get_mcp_client_registry()` handed back a registry whose `get()` and `list_tools()` results were unnameable two hops down. Nothing in the boundary's stated intent drew that line, so `scripts/sdk_surface_closure.py` now admits exported functions as reachability roots — in the **same** walk that serves the inert-surface census, so the two directions still cannot drift into two definitions of "published signature". **Nineteen of the 22 types the widening owed an app are exported**, across `sdk.channel` (8), `sdk.mcp`, `sdk.model`, `sdk.net` (3 each) and `sdk.skill` (2): `AuthConfig`/`AuthMode`, `ScheduleJob`/`ScheduleDefinition`, `SecurityEventLog`/`SecurityEvent`, `McpServerInfo`, the three `mcp_client` registry types, `ProviderRegistry`, `StreamingTagSplitter`/`Segment`, `FetchResponse`/`FetchOutcome`/`ExtractOutcome`, and `SkillsRegistry`/`InstallResult`. **The other three are a declared exemption, and the reason is a different rail rather than a judgement about thinness**: `DashboardState` (the first parameter of the published `channel.save_session_to_history`) and the `SseRegistry`/`SseHub` its `*_sse()` accessors return can only be exported by having `sdk/channel.py` import `personalclaw.dashboard.state` and `.sse` — the fifth and sixth `core-must-not-import-the-http-surface` edges on a file the structural import-direction ratchet pins at four, a rule that grandfathers those four for exactly this reason (*"an allowlist is a thing that rots, a measured floor is not"*). So the HTTP surface stays out of the app type contract, the exemption is declared on `CLOSURE_EXEMPT_PREFIX` beside the pre-existing `AppConfig` one, and its residual is recorded rather than papered over: `save_session_to_history` remains published with a parameter type an app cannot name, which most honestly reads as a function that does not belong on the boundary — a cross-repo change, since a bundled channel app drives it. A test asserts the exemption is declared, load-bearing (without it the gap rail reds on exactly those three) and still justified (the structural floor is still four), so the justification cannot rot into prose. **One of the 22 collided by name with the already-published `ToolResult`**, and a facade that answers `from personalclaw.sdk.x import ToolResult` with "it depends which module" is not a contract — so `personalclaw.triggers.tools.ToolResult` is renamed **`AutomationToolResult`**, which is what `sdk.channel`'s `delete_automation`, `delete_all_automations` and `set_automation_paused` now return. The rename rather than a facade-only alias, and that one rather than the other, both measured: the triggers type had **zero** by-name importers anywhere in the repo and **zero** references in the apps repo, with all 49 of its references inside its own defining module, while `tool_providers.base.ToolResult` is imported by name by **all eleven** first-party apps (690 references); an alias would have left core with two `ToolResult`s for the next `sdk` module to re-collide on, and a traceback disagreeing with the docs about what the type is called. The two are **not** merged — their fields are disjoint and `{ok, text, data}` is on the wire — so that remains a wire change rather than a rename. There are **no exclusions** from the 22: excluding one would have left three published functions with an unnameable return, which is the defect rather than a ruling about it. **The teeth are intact and measured**: an export reachable from nothing is still reported — 159 of 248 surfaces, 43 of them exported *types* plus the same 116 functions and constants, because admitting a function as a *root* never makes it a *target*. The self-edge ruling is unchanged, and `net.EgressPolicy` leaving its load-bearing set (it gained three real function readers) is why those are now pinned by name rather than counted. The baseline was regenerated because every touched counter **fell** (`sdk_export` 170 → 159 across eight files, zero rose, zero new) — a tranche of 19 new published names that moves the counter *down* is the arithmetic to check before trusting the direction.
...
### Fixed
- **The release's own per-arch image smoke could not run the command it was given, and v0.2.0 would have been the first release to find out.** [#3562](https://github.com/PersonalClaw/PersonalClaw/issues/3562). `release.yml`'s release-blocking smoke handed each image's liveness command to a **login** shell (`sh -lc`). A login shell sources `/etc/profile`, and `/etc/profile` does not *extend* `PATH`, it **overwrites** it — so the gateway image's own `ENV PATH="/opt/venv/bin:$PATH"` was discarded and `personalclaw` was simply not on `PATH`. Measured in containers against the published 0.1.3 gateway image: `sh -lc 'personalclaw --version'` exits **127** with `sh: 1: personalclaw: not found`, while the same command as `sh -c` prints `personalclaw 0.1.3`. The step landed on 2026-08-10 and the last release was v0.1.3 on 2026-07-31, so it had **never executed once** — and the failure mode is the expensive one, because `images` pushes `:X.Y.Z`, `:X.Y` and `:latest` and only *then* smokes what it pushed: the tag would have moved in GHCR and the job would have failed afterwards, leaving published tags with no GitHub Release to explain them. The fix drops `-l`. It deliberately does **not** reach for `/opt/venv/bin/personalclaw`, because an absolute path would go green through exactly the Dockerfile regression this step exists to catch — what a container user depends on is that `personalclaw` resolves on `PATH`, since the image's own `CMD ["personalclaw", "gateway"]` is resolved that way. The `web` leg was unaffected, but by coincidence rather than design: Debian's `/etc/profile` branches on `id -u` and the gateway drops to uid 10001, while web's Alpine `/etc/profile` overwrites `PATH` in one unconditional line that happens to contain `/usr/sbin`, where `nginx` lives — both bases clobber `PATH`, only one clobbers it into something that still works. The smoke now also asserts its command's **output** and not merely its exit status, because a version command that prints nothing exits 0 too, and for the gateway the release-relevant property is not "a version printed" but "**the** version being released printed" — a cached build layer serving a stale venv under a `:latest` claiming the new release is the same published-tag-that-lies shape. Driven locally against both real published images before landing, in four arms: the pre-fix body exits 127, the fixed body prints the version on both loop iterations, a deliberate version mismatch fails with `did not report 'personalclaw 0.2.0'`, and an empty expectation is refused rather than matching every possible output. A new rail over **every** workflow in the tree fails if any step ever hands a command to a container through a login shell again, and names the workflow, job and step when it does.
- **App installs that declare Python dependencies were refused on every clean install: `packaging` was never a declared dependency of core.** The installer proves an app's declared `pythonDependencies` cannot move a dependency out from under the running gateway, and it does that with `packaging` — correctly failing **closed** when it cannot evaluate a version specifier, because installing into the venv the gateway is running out of is not something to guess at. But `packaging` itself was not in core's dependency list, so on a wheel install the proof could never run and the refusal fired for every app that declares any Python dependency at all: `openrouter-models`, `claude-subscription` and both speech apps each failed with *"cannot verify app X's python dependencies against core's (No module named 'packaging'); refusing rather than risk moving a gateway dependency"*. Apps declaring none — Web Search among them — installed fine, which is what made it look selective rather than systemic. **A development checkout could not see this**: pip's own build tooling puts `packaging` in the venv transitively, so it imported fine in dev and in CI while being absent from the shipped artifact — the same shape as a wheel that serves a placeholder SPA. The new rail asserts the **declaration** rather than importability, since an importability test passes in dev whether or not the bug is present, and also checks every third-party module the installer imports so the next one cannot ship undeclared either.
- **The install-consent dialog no longer freezes on "Install anyway" when a confirmed install fails for a reason the scanner never anticipated.** [#3540](https://github.com/PersonalClaw/PersonalClaw/issues/3540). Reported live against the Slack Channel app: the Store shows its `warning` scan verdict and an "Install anyway" button; clicking it fires a real `POST /api/apps {confirm:true}` that comes back a real, correct error (on a clean install, the python-dependency admission gate's `cannot verify app slack-channel's python dependencies against core's (No module named 'packaging')` — see #3538/#3539), and the dialog just sits there: the spinner stops, nothing closes, nothing installs, and no error is visible anywhere. `useGuardedInstall`'s `attempt()` cleared `blocked` only at the *start* of a first (non-confirm) attempt; a CONFIRMED re-attempt whose result is `ok:false` and not itself blocking (no `needs_consent`, no terminal dangerous/invalid-signature verdict, no client-install directive) left `blocked` holding the FIRST attempt's warning, so `ConsentModal` — gated on `blocked` alone by every one of its three callers (the Store card grid, Manage Sources, and onboarding's essential-apps step) — stayed mounted showing the stale scan report, while the real error rendered correctly into the page's `GuardedFailure` banner sitting **behind** the modal's own full-viewport backdrop: set, but unseeable. `blocked` is now cleared whenever the latest attempt's result is not blocking, in that failure branch and in the equivalent `catch` branch (a thrown/network failure during a confirmed re-attempt is exactly as unblocking) — so the modal closes and the real reason becomes the next thing on screen, exactly as a plain first-attempt failure already read, and the app is left retriable rather than stuck. Confirmed general rather than Slack-specific by reading all three call sites (identical `if (r?.ok) {…}` with no `else`) and by reproducing it against a build that does not have the #3538 packaging gap: the freeze is unchanged, because any app whose first scan returns `warning` and whose confirmed install then fails for *any other* reason hits the identical symptom. #3539 fixes why the dependency gate itself fails on a clean install; it does not touch this state machine.
- **A failed workflow stage can be retried immediately, instead of being refused by its own no-double-execution claim for fifteen minutes.** [#3533](https://github.com/PersonalClaw/PersonalClaw/issues/3533). A `stage` node takes a `leases` claim before it spawns, and the spawn is still live when the dispatcher returns — so the claim outlives the dispatcher, and its 900-second TTL was the **only** way it was ever released. A stage that settled FAILED therefore went on fencing its own instance, and the retry met its own lease. Measured end to end by reaping a stage and rewinding the node (the ordinary way a user retries a failed step): the second attempt spawned **nothing**, landed `DEGRADED` with `another worker holds the claim on this node (held by workflow:…#a72e3341191c for another 899s) — not executing twice`, and because DEGRADED is a *success* state the run then reported **COMPLETE** having re-run nothing. The user asks for a retry, waits, and is told the run finished. Keying the claim more finely cannot fix this — the retry **is** the same instance — so the identity that is now per-attempt is the **holder**: it travels out on `NodeResult`, is recorded on the instance beside `subagent_id` (persisted, because the lease file survives the process), and is handed back at the FAILED settle, so the retry takes a fresh claim on the same target. The invariant the claim exists for is asserted in both directions, because without the second half this would be a hole rather than a fix: a retry after a failed attempt proceeds (ledger: two `step_started`, one `step_failed`, one `step_completed`), and a genuinely *concurrent* second execution of a live instance is still refused with its claim intact. A spawn that RAISES also releases now — that path returns no `NodeResult`, so the holder reached nobody and the claim could never be released at all. The per-attempt identity is deliberately a value **inside** the claim file and never part of its name: `leases._lease_path` truncates to 64 characters and collapses `.`, `@`, `[` and `#` to `_`, so anything folded into the key can alias two claims onto one file, and an aliased lease is indistinguishable from a correctly-refused duplicate. A **succeeded** stage still keeps its claim, on purpose: the symptom that causes is a stage-bodied *loop* refusing its own next round, which is fixed by keying the claim per node instance rather than by shortening the window here — releasing on both outcomes clears it too (measured: `deep-research`'s round loop goes from 1 dispatch to 8), and two mechanisms for one symptom is the dual path this project does not keep.
- **An empty code fence no longer paints the literal word `undefined` into a chat answer.** react-markdown hands a childless `<code>` no `children` at all, and both of `Markdown.tsx`'s `code` call sites coerced that with a bare `String(children)` — which is the string `"undefined"`, not `""`. That string then flowed straight into `CodeBlock`, so a model that emitted an empty fence produced a code card whose one line read `undefined`. Found driving a fresh container in a browser, where it was visible **only** on the rendered surface: zero non-2xx responses, zero console errors, zero page errors and zero gateway tracebacks, because nothing in the stack considers an empty fence an error. One shared `codeText()` helper now coerces the **absence** of children rather than their value, so a fence whose content genuinely is the word `undefined` still renders it; both call sites use it, because chat goes through the second one (`componentsWith`) and a coercion fixed in only one leaves the highest-traffic consumer painting the word. `Markdown.emptyFence.test.tsx` carries the rail plus two controls — a fence with content still renders it, and `undefined` as real code content is still shown — so neither assertion can be satisfied by dropping code blocks or by scrubbing the string.
- **A `<widget>` shown inside a code fence is now shown, not run — and the fence keeps the line it was about.** [#3526](https://github.com/PersonalClaw/PersonalClaw/issues/3526). A code fence is the one thing in markdown that means *"do not interpret this"*, and `parseWidgetBlocks` had no notion of one: it scanned the raw string with a plain regex, so a widget written **inside** a fence — an answer explaining the tag, a doc quoting it, a tool result echoing a page that contains one — was cut out of the fence and mounted as a live sandboxed iframe. Two wrong outcomes from one bug: the example **executed**, and the fence the user asked to read was left with its content removed (measured on a fresh container: a `diff` block whose one `+` line was the tag rendered as an **empty** `+` row, plus a second widget that was never asked for; the orphan fence that left behind is what surfaced [#3525](https://github.com/PersonalClaw/PersonalClaw/issues/3525)). This is the second instance of a family whose first was [#633](https://github.com/PersonalClaw/PersonalClaw/issues/633) — an unrecognized artifact kind silently coerced to `widget`, the sandboxed-**execution** kind. That fix was scoped to the API's kind field, so it closed one door into execution and left the render path open; the rule now lives at the parser, where both doors are the same door. **Deliberately not a chat-only fix:** `parseWidgetBlocks` feeds three surfaces — chat, a workflow gate's prompt and a dashboard tile's body — and a content-to-execution boundary that holds on one of three is not a boundary, so all three are asserted, each paired with its unfenced counterpart so no "no iframe" assertion can pass by looking in the wrong place. **What this changes for you:** a fenced or backticked widget renders as code with its text intact, and an unfenced one behaves exactly as before, including painting progressively while it streams. The scanner follows CommonMark for fenced blocks (``` and `~~~`, longer fences containing shorter ones, an unclosed fence running to the end) and for single-line inline spans, and it treats a widget's own body as opaque so markup containing three backticks cannot silence the widgets after it. **One deliberate limit, stated rather than papered over:** a 4-space *indented* block is not treated as code, because indentation is structure rather than a marker — a tile body is pretty-printed HTML and a widget under a list item sits at its item's content column, so an indentation rule would stop rendering real widgets on the two surfaces that are not markdown at all. Filed under Fixed rather than Security on three grounds: the widget iframe is `sandbox="allow-scripts"` off a blob URL, so it has an opaque origin and no reach into the app's document; a widget cannot fire its action channel without a real user gesture (`e.isTrusted` in the host script); and the platform executes agent-authored widgets by design, so the fence bypass granted no capability the unfenced path did not already have. It is still a boundary, not a rendering nit, which is why it is rail-guarded at the parser and at every consumer.
- **A `javascript:` URL with a tab in its scheme no longer survives the HTML sanitizer, and a URL it cannot classify is now rejected instead of kept.** [#3534](https://github.com/PersonalClaw/PersonalClaw/issues/3534). `isSafeUrl` tested for a scheme with `/^[a-z][a-z0-9+.-]*:/` on a value that `trim()` had only stripped at the ends, then ended `return true // scheme-less relative-ish`. The URL parser removes every ASCII tab, LF and CR *before* it reads the scheme, so `java<TAB>script:alert(1)` is `javascript:` to the DOM while that character class cannot match a tab at all — the pattern wants `:` where the tab sits, does not match, and the fail-open last line then kept the value as a relative URL. The docstring above it said fail-closed. This is the boundary between model-authored markup and the app origin that holds the user's session, injected by `dangerouslySetInnerHTML` in `renderers.tsx` for both the `svg` and editorial `document` types, and the threat model already covers untrusted content (browse automation, tool output, MCP servers, fetched pages) — so a prompt-injected page emitting `[click me](java&#9;script:…)` was one click from running script in that origin, the HTML parser having decoded `&#9;` to a real tab before the check ever saw it. **The tab was one of 15 payloads, not the defect:** the defect is that the fallthrough allowed anything the scheme test failed to recognise, so every gap in that test was a bypass rather than a rejection. Also live were LF, CR, CRLF, a tab before the colon, a tab between every letter, the `&#9;`/`&#x09;`/`&Tab;`/`&NewLine;` entity forms, an upper-case scheme, `vb<TAB>script:`, and `da<TAB>ta:text/html,` (which cleared the `data:text/html` block as well) — each measured reaching the DOM as a script scheme through `href`, `HREF`, `src` and `xlink:href`, on `<a>`, `<img>`, `<source>`, SVG `<a>`, SVG `<use>` and inside `<foreignObject>`. A **second** mechanism landed in the same place: a leading `\x01` or `\x1b` is not whitespace, so `trim()` left it and `^[a-z]` could not match, while the parser strips leading C0-control-or-space and reads the scheme (`\x0b`/`\x0c` were already safe, because `String.trim()` does treat those as whitespace). Normalisation now mirrors the URL spec exactly — remove tab/LF/CR anywhere, strip C0-control-or-space at the edges — so the check and the browser read the same string, and classification is a positive allowlist (`http`, `https`, `mailto`, `tel`, `data:image/*` unchanged, or no scheme at all) with `return false` as the only fallthrough. **Deliberately normalise rather than reject on sight:** an interior *space* is not removed by the parser, so `my notes: draft.md` is a relative path and must keep working, and a model wrapping a long `href` across lines must not have its link dropped; both are asserted. `cite` (`<blockquote>`, `<q>`) is now URL-checked too — it was allowed by `GLOBAL_ATTRS` and missing from `URL_ATTRS`, so it was the one URL in the output nothing validated, and a plain `javascript:alert(1)` survived there with no trick at all. The file had **no tests**; it now has 420, half of them asserting that relative links, fragments, queries, `mailto:`/`tel:`, `http(s)`, `data:image/*`, tables, lists, prose and inline SVG still render, because a strip-only suite cannot tell a correct sanitizer from one that strips everything. The security assertions ask the DOM what every surviving attribute resolves to rather than grepping the output for `javascript:`, and carry their own vacuity control — a name-matching version of that helper silently skipped `xlink:href`, which the SVG serializer renames to `ns1:href`.
- **A room's member picker no longer says "Loading…" forever when the agent list cannot be read, and no longer tells a user with no agents that they are all already in the room.** Two wrong sentences on one small form, each a failure wearing a benign state. The picker's agent list is a second read alongside the room itself, and it dropped its own rejection: `agents` is `undefined` both while `/api/agents` is in flight and after it has failed, so keying the placeholder off that single value reported a **permanent** "Loading…" for a request that was already dead — no error, no retry, nothing naming what went wrong. It now distinguishes the three cases it actually has, says *"Couldn't load your agents"* with the rejection's own message, and offers a **Retry** on the field itself. Separately, the "every agent you have configured is already in this room" branch was reached whenever the option list was empty, because `[].every(…)` is vacuously **true** — so a user who had configured *no* agents was told a sentence about a set that does not exist, and the "No agents configured" text that should have shown them was unreachable. "All of them are taken" and "there are none" are different facts and now get different sentences. This is the (B) half of the `useQuery` error class (a failed load indistinguishable from one still in flight); `ui/loadErrorState.test.tsx`'s `UNBOUND_ERROR_BUDGET` is the rail, and because `RoomView.tsx` carries no entry there, its zero default is what enforces it.
- **The default accent was under AA as chip text on every light surface — 3.52:1 on a page every user visits — so `--color-primary`'s light value is retuned across all 12 schemes.** [#3503](https://github.com/PersonalClaw/PersonalClaw/issues/3503). A status chip paints its tone twice, as the ink **and** as a 16% wash of itself, so the ground is a translucent tint of the ink and every percent of tint drags the two together. The 12 light accent values were chosen against the **bare** ground (`schemes.ts` recorded the rule as "≥4.5:1 as white-text button fill AND as text on white"), which is the same compositing mistake the `--color-info` retune corrected one token over. Swept the way every other tone is swept — 12 schemes x 2 modes x 4 resting tiers, 8-bit quantized composite — `--color-primary` was under WCAG AA's 4.5 in **42 of 96 cells**, worst **3.4870** (rose/light/canvas). **Every one of the 42 was light mode**; dark passed 48 of 48 and is untouched. `coral`, the default, read **3.5229**, live on Settings today. No token substitution fixed it — `--color-primary-emphasis` only reached 20 of 96 — so the values moved. The method is the `--color-info` precedent: OKLCH, **hue angle held exactly**, chroma held (0.908-1.000 of the original, given up only where sRGB cannot hold the requested lightness), lightness moved the **minimum** needed to clear 4.5 on the worst resting tier. Deliberately not a flattening — each scheme moves by its own amount (ΔL -0.017 lavender to -0.074 rose, and `mono` needed nothing at 7.80), so the twelve identities stay twelve. All 96 cells now clear, worst 4.5007. **`--color-primary-emphasis` moved by the same delta, and that is the half a value swap would have got wrong.** Emphasis is the hover fill (`bg-primary hover:bg-primary-emphasis`) and the accent-text ink for the three grounds plain primary cannot carry, and both uses rest on one property: emphasis sits *further from the ground than primary*, in either mode. Retuning primary alone **crossed it outright in 6 of 12 schemes** (honey, jade, ember, forest, rose, amber — rose by -1.10), so hovering a filled accent button would have made it **lighter**; and on `coral` the step collapsed to 0.20, a hover nobody can see. Every rail in the tree stayed green through both, so the ordering is now an assertion with a minimum step rather than a sentence in five comments. **The collateral was measured, not assumed**, because every surface reading this token moves: white-on-primary button fill **gains** (4.83 -> 6.42 worst, since `--color-on-primary` is white in light in all 12 schemes, so a darker fill only helps); primary as text on white 4.83 -> 6.42; the focus ring on every light tier 3.9986 -> 5.1180 against a 3:1 floor; the session-map mark on the rail 4.3714 -> 5.5952. One family **loses** and is reported rather than buried: the `bg-primary/15` tonal ground sits under a dark ink, so darkening the token costs it — 12.07 -> 11.44 at rest and 10.36 -> 9.42 on hover, against a 4.5 floor and a 6.0 margin rail, so not close. The `::highlight(pc-find)` and `.kl-highlight` tints move the same way (7.99 -> 7.14, 9.54 -> 8.84). The inverted focus-ring assertion that a 50% accent tint *cannot* reach 3:1 still holds — its global maximum is pinned by a dark `mono` cell and does not move. There is no dark-in-light-mode ground carrying primary ink anywhere in the tree, verified by census rather than assumed, so darkening is monotone for every ink site. A **collateral benefit** worth naming: the class-spelled `bg-primary/N text-primary` family that `accentChip.test.ts` guards moves from failing at every strength to clearing AA at 10-20% on white and 10-16% on the canvas — the rail stays as strict as it was (25% still fails on both grounds, and the strengths that now pass do so by hundredths) but its numbers were corrected rather than left to mislead. **The retune had to land in four places, and the least obvious one is the one that ships**: `app/appearance.tsx` writes every registered colour token inline on `<html>` at mount and falls back to `tokenRegistry`'s pair whenever the user has no stored override — and overrides are only populated once the user picks a scheme. So on a fresh light-mode install `schemes.ts` is never read, the inline style beats `.light`'s class rule, and `tokens.css` is a pre-hydration fallback; fixing two of the three would have shipped contrast that depends on where the user's value came from. All three are now asserted to agree, the way `--color-info` already was, and a fourth site — the custom-theme swatch fallback, which had drifted two retunes stale on `#e85a3f` — is current and pinned. **The 4.5 floor was not touched and no ratchet was re-baselined.** `UNSWEPT_TONES`, the mechanism that made `primary`'s exclusion legible instead of invisible, is now empty and pinned at zero, so a tone cannot quietly drop out of the sweep again.
- **The `neutral` status pill is readable: its text was drawn in a border token at 1.63:1, and the contrast rail that should have caught it measured four of the six tones.** [#3493](https://github.com/PersonalClaw/PersonalClaw/issues/3493). `ui/StatusPill`'s `neutral` tone set its ink to `--color-outline-variant`, a hairline/divider value. A hairline is designed to be *barely* separable from its surface, which is the exact opposite of what text needs, so over the pill's own 16% tint it read **1.6346 dark** on `surface-container` and **1.1381 light** on the canvas against WCAG AA's 4.5 for 12px text. All eight resting cells failed, in both themes, and no choice of ground rescues it: the bare token is 1.07–2.04 against every tier before any tint is applied, which `knowledge/graphMarkContrast.test.ts` independently records (2.04 dark / 1.17 light) as its own reason for deleting the same value from two graph marks. So this was an ink fix rather than a compositing one. `neutral` now draws in `--color-on-surface-low`, the muted tier of the same ink ramp (0 of 8 cells under AA, worst 4.5472, best 7.2320), which is also the ink `settings/bento.tsx`'s `muted` pill already ships, so the tree's two neutral pills agree instead of minting a third grey. `--color-on-surface-var` was measured and rejected at 4.4308 on the light canvas. One value in the primitive, so all ten call sites inherit it: the "Fit unknown" model-fit chip, three room-status tones in `chat/roomMeta.ts`, the voice-profile kind and level chips, the desktop live-view state, and two in the browse mirror. The second half of the defect was the rail. `design/statusChipContrast.test.ts` swept `ok`/`warn`/`danger`/`info` from a hand-written list, and the two tones it never named, `neutral` and `primary`, were exactly where the next failure lived, because a hand-written list cannot notice a tone it does not mention. The list is now **derived from the primitive**: the rail parses `TONE_VAR` out of `ui/StatusPill.tsx`, cross-checks the parsed keys against the `StatusPillTone` union so a skipped entry reds rather than silently shrinking the sweep, and requires every tone to be either measured against 4.5 or named in `UNSWEPT_TONES` with its numbers. A seventh tone now reds the file until someone does one or the other. `neutral` joined the sweep (600 cells to 640), and a new `no_hairline_ink` assertion guards the *class* by name, with `hairline_unreadable` as its inverted vacuity floor so the denylist carries its own justification. The 4.5 floor was not touched: `primary` is genuinely under AA in 42 of 96 cells and no token substitution fixes it, so it is named as the one unswept tone with its measurement and filed as [#3503](https://github.com/PersonalClaw/PersonalClaw/issues/3503) rather than absorbed by a lowered bar, which would have read as broader coverage while asserting less. A tree-wide census for the same class found one other live case, `learning/learningMeta.ts`'s `DAY_TONE.out_of_scope` at 3.1833 in light, filed as [#3504](https://github.com/PersonalClaw/PersonalClaw/issues/3504).
- **The provider-boundary document is true again, and the rail that enforces it now reads the same thing the document declares.** [#3500](https://github.com/PersonalClaw/PersonalClaw/issues/3500). Two claims in `docs/architecture/provider-boundary.md`'s judgment table — the enumeration whose whole purpose is to be authoritative about what core may touch — were verifiably false: it listed `constants.APP_LOGGER_ROOTS`, a name `constants.py` does not contain, and in the same row called apps registering their own logger roots *"deliberately not built yet"* while a manifest's `loggerRoots` plus `apps.catalog.installed_logger_roots()` do exactly that for two live consumers (the CLI log setup and the dashboard log-level handler). The row is gone, because it is not an in-core exception any more; a stale exception list is worse than no list, since it reads as a ruling and answers wrongly in both directions. Separately, `test_provider_boundary_residue.py` knew only the **channel** vendor SDKs, so core's eight `import openai` statements across four modules — every one of them a *declared* exception in rows 1–2 of that same table — were invisible to it and the sweep was green on a tree it could not see. The vendor set is now derived from `pyproject.toml`'s provider-SDK extras plus the SDK distributions app manifests declare (so adding a vendor has a rule, not a guess), the four OpenAI-protocol clients are listed in `provider-boundary-keeps.txt` with their judgments, and the import half of the sweep reads `ast` Import nodes rather than raw text — so a ninth vendor SDK import in a module the document does not cover now goes red, while a comment or docstring mentioning a vendor cannot. Nothing moved out of core: this makes the boundary legible and **enforced as it actually is**, which is the baseline a future relocation needs. Also corrects a stale in-code contract comment on `permissions.backgroundTasks`, which still read *"NOT ENFORCED TODAY"* although `apps/worker_runtime.py` refuses to spawn or revive a worker without the grant and the Store lists it among the enforced permissions — the same defect class as the document, inviting the next reader to rely on a model the code does not implement.
- **The shell no longer corrects away the two routes it renders itself: `#/companion` — the PWA's own `start_url` — is reachable again, and every deep-link out of first-run setup lands where its button says.** [#3506](https://github.com/PersonalClaw/PersonalClaw/issues/3506). `App.tsx` had two effects deciding the route, and the second one — the unknown-hash corrector (#306) — tested `ROUTABLE`, which is the set of routes the **nav switch** has a case for. `onboarding` and `companion` render from early returns above that switch and are deliberately outside it, so the corrector treated both as typos and `replace`d them with `#/dashboard`. Two dead ends, one cause. An installed PWA opened on the dashboard instead of the approval surface it exists for, measured as `location.hash = '#/companion'` settling on `#/dashboard`. And **every** exit from onboarding landed on the dashboard: the correction fires on the very commit that `onboarded` flips, while `route` is still the stale `'onboarding'` (the guard sets `location.hash`; `route` catches up only on the browser's async `hashchange`), so it overwrote the destination the guard had just pushed — measured on a fresh container as exactly one `history.replaceState('#/dashboard')` from `navigate(…, { replace: true })`, with the guard's push never reaching a listener. That silently broke six first-run controls: the try-one failure card's Settings deep-link (`settings/models` / `settings/providers` / `settings/doctor`), its success outcome link, and the done screen's Inbox, Updates and Store-sources links. The two effects are now **one**, with correction downstream of every route decision, so ordering holds by construction rather than by a predicate — the previous gate (`loaded && onboarded`) named the wrong window, and this is the second time this destination has been measured landing on the dashboard. The corrector now asks `renderable`, the union of `ROUTABLE` and a named `SHELL_ROUTES`, and a rail asserts that every route the shell returns for early is one the corrector admits. Three of the tests that were green throughout are replaced: `manifest.test.ts` asserted only that App.tsx *contained the string* `'companion'`, and `routeCasesCoverRoutable.test.ts` pinned the buggy gate as the contract. The behavioural half is new — `e2e/pwa.spec.ts` opens the real built bundle at the real `start_url` and reads the hash back, with a bogus-route positive control, because the first draft of that test satisfied itself on its first read and passed against the build that has the bug.
- **The bundled-Ollama registration test no longer reds in any run where a gateway booted first, because resetting the LLM registry now evicts the app module cache with it.** [#3516](https://github.com/PersonalClaw/PersonalClaw/issues/3516). On pristine `main`, `pytest tests/test_gateway_boot_provider_sync.py tests/test_native_ollama_bundle.py` failed with `ProviderResolutionError: unknown provider type 'ollama'; known types: []` while the same file passed 10/10 alone. One enable-the-bundle cycle writes **three** process globals and `registered_bundle` reset two: `load_bundle_module` caches an app's module in `sys.modules` under its namespaced name on purpose (an availability read and a factory read must not re-execute app code, and a re-exec would mint two classes for one provider), and the provider TYPE is an import-time side effect of that module — so `reset_default_registry()` with the module still cached leaves nothing able to re-register the type, and the loader answers from cache into an empty registry. The leaker is a real boot path rather than a test fake: `test_gateway_boot_provider_sync.py`'s first test boots `start_dashboard`, which enrols every `type: model` app and leaves this bundle cached for the rest of the worker (probed: `_pclaw_app_ollama_models__provider cached=True` from that test onward). The three globals are now reset together by one helper both halves of the fixture call, so the setup and teardown cannot drift apart again. **No product code changed and none should:** `reset_default_registry()` is documented "Intended for tests only" and has no `src/` caller, the app's own `register_type` is already wrapped in `except ProviderResolutionError` for exactly this reload, and the module cache is load-bearing. The register-once-per-gateway-lifetime behaviour is untouched. A new test makes it deterministic in ONE file — it pre-caches the module exactly as a booted gateway leaves it and then drives the same manifest → `ProviderRegistry` → `ModelTypeHandler` path — so the failure no longer needs a file ordering to appear; deleting the eviction reds it in isolation. Distinct from the `local_models.registry` guard in #3441, which snapshots a module-level dict of the same name and does not cover this registry (measured: the identical two-file invocation fails the same way on #3441's head).
- **A local-model download now shows real progress from the moment you start it, instead of freezing at `0 MiB` through completion.** [#3520](https://github.com/PersonalClaw/PersonalClaw/issues/3520). `POST /api/models/downloads` answers `state: "queued"` — deterministically, because `ModelDownloadRegistry.start()` creates the job with that default, schedules its worker with `asyncio.ensure_future`, and the handler returns `202` with no `await` in between, so the coroutine that sets `running` cannot have run yet. `useModelDownloads` tested `state !== 'running'` in all three of its checks, so on the one path that STARTS a download it opened no SSE stream and reported the job settled the instant it began. Measured on a fresh container: `Getting a small model ready — 0 MiB of 138 MiB` held for over six minutes with a Cancel button beside it, while the full 144,811,072-byte weight had landed on disk about thirty seconds in; the entire `/api/*` census after the click was one `202 POST /api/models/downloads` and nothing else — no stream, no re-list. Only a manual page reload, which nothing suggests, showed the truth, and that re-attach path working is exactly what made the bug look cosmetic. Four call sites answered "is this job still live?" three different ways, and the one that got it wrong was the one that subscribes; `LocalModelManager` had its own third answer (`state === 'running'`), so a just-started download rendered as not downloading at all — no progress row, and the Download button still offered. There is now one predicate, `isLiveDownload`, next to the wire type it reads, and terminal means `done`/`error`/`cancelled` and nothing else. This is every local weights fetch, every STT model, every TTS voice and every sidecar install, not just the bundled chat model. The already-downloaded short-circuit is kept and asserted separately, because a naive fix breaks it: an immediately-`done` job must still settle without a stream.
- **A handled tool failure now reaches the wire AND the audit log as a failure, so a refused destructive operation is no longer recorded as one that ran.** [#3487](https://github.com/PersonalClaw/PersonalClaw/issues/3487). `InProcessMcpToolProvider.invoke` answered `ToolResult(success=True, …)` for every call that did not RAISE — and the `mcp_*` handlers deliberately do not raise for expected failures (an unknown slug, a missing required argument, a service error code), so `POST /api/tools/invoke` answered `200 {ok: true, error: ""}` over text beginning `Error`, and the hash-chained Security Event Log recorded four refused DESTRUCTIVE calls as `outcome: "completed"`. An audit log whose whole question is *what destructive tool ran* was answering it wrongly, in the direction that manufactures false positives. Handlers now state a handled failure through a structured channel — `tool_failure(reason, code=…)`, returning a `ToolFailure` — and the bridge reads the TYPE, so the wire envelope and the SEL outcome derive from ONE signal rather than each re-deriving it from the prose. A predicate over the text was rejected rather than tuned: `artifact_list` answering `No artifacts found.` is a success while `artifact_delete` answering `Artifact not found: X` is a failure, and a leading-`Error` sniff calls both successes. The same change deletes `call_tool_with_logging`'s existing `startswith("Error:")` sniff, which had never matched a coded failure (`Error [CODE]: …`) at all, and adds an AST ratchet so a new handler cannot fall back to prose.
- **Doctor is clean on a fresh install: the last two "in NO snapshot" paths are now recorded as deliberately not state, with the reasoning beside them.** [#3473](https://github.com/PersonalClaw/PersonalClaw/issues/3473). `durability.inventory.audit_home()` reported `2 unclaimed path(s) … these are in NO snapshot` on a newly installed home, naming `footprint.json` and `trigger-claims/`. The probe was right — nothing had decided about either — and after #3468 this was the **only** remaining doctor fault a new user saw, so a fresh install's first look at Doctor showed exactly this and nothing else. The judgement is per path and the two went the same way for different reasons, which is why it could not be one rule. **`footprint.json`** is a measurement series of *this machine's* disk, not user state: the maintenance tick re-measures every declared store, and the only thing a restore could not reproduce is the trend — which is precisely what must not travel, because a snapshot is restored onto another machine or the same one after a wipe, where those byte counts describe a filesystem that no longer exists; `growth()` would then read a window straddling the restore and report a rate that never happened. The module's own rule is that a single sample reports **no** rate rather than a fabricated zero, and a foreign series fabricates something worse than a zero because it looks like real history. **`trigger-claims/`** is runtime coordination, and restoring it is actively harmful: a claim answers "which trigger is running right now" and names `owner_pid`, so on any restored home every pid is dead or belongs to an unrelated process, and because expiry is read-time rather than swept the claim reads as **live** until its deadline elapses — suppressing fires that should happen through `overlap: skip` and asserting a run that does not exist through `is_running`, i.e. reintroducing by backup the single-flight failure the claim store was written to fix. Both are now `IGNORED` rows carrying that argument where the next reader meets it, beside the same decision for `machine_id`, `gateway.runtime.json` and `update_run.json`. **No archive changes, in either direction:** `IGNORED` is read by the audit and by nothing else, and a snapshot walks the declared entries — so these paths were already absent from every archive (that is what "unclaimed" meant), a snapshot taken before this change and one taken after contain the same paths, and restoring either behaves identically. There is nothing to migrate; what changed is that a deliberate absence is recorded as one instead of reported as a gap.
- **A misspelled permission in `app.json` is now refused at install by name, instead of vanishing silently from both the manifest and the consent screen.** `Permissions.from_dict` reads the fifteen known keys **by name**, and nothing else looked at the block — so an unrecognised key was dropped with no error and no trace. Measured: an `app.json` declaring `{"strorage": true, "filesystem": true, "root": true, "exec": true}` **validated clean**, and `permissions.to_dict()` returned `{}`. Unlike the manifest's top level, which deliberately preserves unknown keys in `extra` for forward compatibility, a permission block has nowhere to put them, so the declaration simply ceased to exist. That one silence misleads two different people. The **author** ships believing the app holds a permission it will never be granted, and learns otherwise as an unexplained runtime denial far away from the manifest — a single transposed letter in `strorage` costs an app all of its storage access with nothing said. The **user** is asked to consent to a set that is not the set the author wrote, which is the one thing an install-consent surface must never do: every unknown key is invisible on the Store's permission list, so consent is collected against a quietly different manifest. Each unknown key is now an install error naming the key, saying that it grants nothing and would not reach the consent surface, and offering the nearest real permission where one is close (`'strorage'` → *did you mean `'storage'` or `'storageRead'`?*). Suggestions are drawn from the same vocabulary the check reads, so a message can never name a key that does not exist. Two deliberate shapes: the vocabulary is **derived from the dataclass fields** rather than hand-listed, because a hand-listed copy falls behind the moment a permission is added and would then refuse a permission that actually works; and the unknown keys are **recorded during parsing but refused only in `validate()`**, because parsing runs on every load path (the Store scan, the catalog listing, the provider loader) where raising would take out the listing, while `validate()` runs only at install and update. So a malformed app still renders a Store card that explains itself, and still cannot be installed. **No shipped app is affected** — all 101 manifests in the apps clone and the bundled natives validate unchanged, and the new bookkeeping field never reaches the wire shape. The check did immediately find one instance of the trap **inside this repository**: `tests/test_app_catalog_provenance.py`'s fixture declared `"filesystem": "read"|"write"`, which has never been a permission here, so the half of that fixture whose stated purpose was "the two copies differ in PERMISSIONS" was inert and no assertion ever read it. It now declares `storage`, which is real and enforced. That is the defect's own shape appearing in the project's own test data, which is the best available evidence that a silent drop was easy to fall into.
- **The one rail enforcing "apps import core only via `personalclaw.sdk.*`" was scanning 2 of 179 app files from a worktree, and 355 of another lane's files from the main checkout.** `tests/test_apps_import_boundary.py` is the whole enforcement of the provider-agnostic-core tenet — there is no second copy in the apps repo — and its docstring already records that it "had never run, anywhere" before issue 1777 fixed its root resolution. That fix was still wrong, in the same way for a subtler reason. It resolved the workspace as `_REPO_ROOT.parent`, which *is* the workspace from the main checkout; but `_REPO_ROOT` is the tree the tests live in, and this project's work happens in linked worktrees at `<checkout>/.worktrees/<lane>`, where `.parent` is `.worktrees`. Neither sibling spelling resolved there, so the scan fell back to the two bundled apps: **2 files, against 177 in the sibling clone** — and `test_the_lint_has_something_to_lint` stayed **green**, because a non-empty assertion is satisfied by 1.1% of the population exactly as well as by all of it. The opposite failure was live at the same time: `rglob` has no exclusion for `.worktrees`, and the apps clone hosts other lanes' linked worktrees under `PersonalClawApps/.worktrees/<lane>/`, so from the main checkout the scan matched **532 files of which 355 (67%) were two other lanes' uncommitted working trees** — making this rail's verdict a function of other people's scratch state, able to red a branch over a file it never opened. Both are one defect (the population is not derived correctly) and are fixed together: the apps root is now found by an **ancestor walk with positive proof** — a candidate directory counts only if a child carries an `app.json`, so the search can look upward without mistaking an unrelated `apps/` for a clone — and the exclusion set gains `.worktrees` and `.git`, applied to each path **relative to its own root** rather than absolutely (matching absolute segments looks equivalent and silently drops the bundled apps whenever the suite runs from a worktree, since `.worktrees` is then part of their own location — measured at 0 files, which in a clone with no sibling checkout would empty the scan and red the floor for a reason having nothing to do with apps). Two new assertions keep both directions honest: one rejects any scanned path inside a linked worktree, and one pins the resolution against a **constructed** worktree layout rather than against whatever machine runs it. Measured effect: the same command that collected **4** items now collects **183**, scanning 179 files across both roots. **No app was violating the boundary** — all 177 real app files are clean, which is the other half of why this went unnoticed for so long.
...
- **A dev home named anything but exactly `.dev-home` is no longer walked by the licence census, so a stray one stops reding a local suite as if it were a governance violation.** [#3462](https://github.com/PersonalClaw/PersonalClaw/issues/3462). Two rules disagreed about what a dev home is called: `.gitignore` ignores the **glob** `/.dev-home*/` — with a comment saying not to narrow it, because every dev home holds live credentials — while `tests/test_licence_governance.py` skipped the **literal** `".dev-home"` inside a `frozenset` the walk tested with `d not in _SKIP_DIRS`. So `.dev-home-i3` was invisible to `git status` **and** walked, and a dev home holds installed app copies, i.e. real `LICENSE` files. Reproduced by planting one non-canonical dev home with a single installed app licence: it reds **three** tests, not the one the report named — `test_every_declaration_site_is_in_the_census`, `test_every_licence_file_carries_the_pinned_grant` and `test_every_licence_file_names_an_approved_holder` — while `git status --porcelain` prints nothing. The skip is now a **prefix predicate** (`_is_skipped_dir`), because `_SKIP_DIRS` is a membership test and another literal would fix one machine and no other: the next `make serve`-style run in a worktree mints a new name. And the invariant is now **agreement with `.gitignore`**, asserted by reading the ignore file and comparing both rules name by name in both directions — the drift between two independently-written rules is the defect, so maintaining it by hand is what failed. The red was local-only and needed three conditions CI satisfies none of (run from inside a worktree, a non-canonical name, an app installed into it), which is exactly what made it expensive: it looked like a real licence violation in your diff and re-running on a runner cleared it without explaining it.
- **The audit-outcome rail now sees the outcome words a subsystem names as CONSTANTS, and its raisable ceiling is replaced by a named ledger — so the better practice is no longer the one that evades the rail.** [#3443](https://github.com/PersonalClaw/PersonalClaw/issues/3443). `tests/test_audit_outcome_families.py` scanned with `re.compile(r'outcome="([a-z_]+)"')`, so its count was never a count of the SEL outcome vocabulary — it was a count of the portion written inline. **Measured: 69 words visible against a real 102.** Two blind spots, both with live victims: a module constant handed to the writer as `outcome=result.outcome` (six of `browse/vision.py`'s eight words, including `grounded` and `no_model`), and a **ternary read HALF** — `outcome="executed" if executed else "declined"` showed the scanner `executed` and hid `declined`, so one decision had one arm classified `Succeeded` and the other rendering as nothing. The rail did not merely fail to surface those words: it **refused the correct fix**, answering *"denied: no writer emits anything matching 'declined'"* to anyone who tried to classify one. And the cheapest way to satisfy the ceiling was to move a literal into a constant — i.e. to make the word *more* silent — which penalised the better practice (one spelling, refactor-safe, greppable by symbol). The scanner is now an AST pass that resolves module constants, `str(...)` and `.get(..., DEFAULT)` wrappers, `Enum.MEMBER.value`, **both** branches of a ternary, and every constant whose name declares it an outcome (`OUTCOME_*`), and it **counts what it cannot resolve** so "the scan found nothing here" can never pass as "there is nothing here". `declined` joins the `Denied` family beside its sibling `rejected`, and `needs_human` joins `Needs confirmation`, whose docstring is already its definition. **The ceiling is gone**: `assert len(unclassified) <= 34` had both escapes a number always has — absorb a new word by editing one digit, or delete it from the census by hiding it in a constant — and is replaced by `_NO_FAMILY`, a per-word record of why a word belongs in no filter. A new word now fails by name with its emitting site, and the only two ways to make it pass are a family or a written reason. Both directions are asserted: a recorded word must not also be claimed by a family, and it must still be emitted, so the ledger cannot rot into a list nobody writes.
- **The Doctor no longer tells every pip-installed instance that its dashboard “serves a stale SPA”.** `serving-fs.dist` classified `static/dist` as a shadowing `copy` on the mere fact of being a real directory — and in a wheel a real directory is *what ships*. Measured on a fresh container (0.1.3 wheel, nothing configured): `static/dist` was a real directory holding the very `index.html` whose `/assets/index-*.js` the gateway had just served `200`, there was **no `web/` anywhere in the image** for a symlink to point at, and `/api/doctor` reported `serving-fs` not ok with “static/dist is a COPY shadowing the runtime symlink (serves a stale SPA)”. That is a permanent fault on every real install, carrying a degraded badge onto a new user’s home screen — and the only remediation it offered (`serving-fs.symlink-repair`) *refuses* on that layout with “no web/dist build found to link”, so the probe and its own fix disagreed about whether anything was wrong. The freshness half of this same probe already had the distinction (`spa_dist_freshness` returns `no-sources` and documents it as “an installed wheel. Not checkable, never a fault”); the symlink half did not. A directory is now `packaged` rather than `copy` unless a `web/dist` build actually exists to be shadowed, using the one resolver (`frontend.resolve_website_dist`, now public) that both the probe and the fix read — the fix used to re-derive it, with a docstring apologising for doing so. A dev tree with a `web/dist` still reports the copy, and a packaged `static/dist` missing its `index.html` still fails as “SPA not resolvable”, which is the placeholder trap `scripts/verify_wheel.py` exists for.
- **`personalclaw.agent` no longer freezes the agents directory at import time, so a test that relocates the home after import stops writing to the developer's real one — and stops handing a green run a non-zero exit.** [#3463](https://github.com/PersonalClaw/PersonalClaw/issues/3463). `AGENTS_DIR = _user_dir() / "agents"` was a module-level constant, and `_user_dir()` is `config_dir()` — so importing this module before `PERSONALCLAW_HOME` was set, or before a test patched `config_dir`, pinned the path to `~/.personalclaw` for the life of the process and no later isolation could move it. Whether that happened was a function of pytest's collection and import order, not of any test's own correctness, which produced the worst shape a gate can have: `148 passed, 0 failed` with **exit 1** from the real-home rail, reading as *"my diff touched the real home"*. **Demonstrated deterministically**: import `personalclaw.agent`, *then* set `PERSONALCLAW_HOME` to a fresh temp dir, and the resolved path is `/Users/<you>/.personalclaw/agents` — the real home — while the same probe on the fix answers the temp dir. `agents_dir()` now resolves per call, following `subagent_persistence._subagents_dir`, the shipped precedent for exactly this shape. **Not the `DASHBOARD_PORT` case**, and the contrast decides the fix: that constant is also import-time and is documented safe, because `PERSONALCLAW_PORT` is validated at CLI entry before `loader.py` is imported — here, relocating the home *after* import is the supported way to isolate a test, so the same shape is a defect. All nine consumers were converted rather than left importing the constant, because a top-level `from personalclaw.agent import AGENTS_DIR` re-freezes the value in a second module: `session`, `cli_doctor`, `mcp_discovery`, `dashboard/chat_persistence` and the `agents`/`hooks`/`mcp`/`skills` handlers. Two of those had already worked *around* the constant by deriving `config_dir() / "agents"` themselves — each with a comment naming the freeze — so the duplicate derivations collapse into the one owner. **`_USER_MCP_JSON` is deleted rather than made lazy**: it had the identical defect and **zero readers** anywhere in `src/`, `tests/`, `harness/`, `web/` or `scripts/`, and the path it named is separately read as `_USER_DIR / "mcp.json"` — a constant that does not exist cannot be exposed.
- **The worktree fan-out test no longer decides a coin flip on the CI runner's load — and the property it was supposed to guard is now actually guarded.** [#3477](https://github.com/PersonalClaw/PersonalClaw/issues/3477). `test_fanout_of_four_is_faster_than_serial_creation` gated on a ratio between two wall-clock samples (`batched <= serial * 1.6`), taken at different moments in the same process, so on a shared runner the batched arm could lose to a serial arm that ran in a quieter window. It reddened PRs whose diffs cannot have reached it — one whose entire content was four deleted markdown files — and its JUnit appeared on at least two other unrelated open PRs. The multiplier was deliberately **not** widened, because the measurement shows the gate was vacuous in the one direction that matters: with `add_worktrees` pinned to `max_workers=1`, i.e. batching removed entirely, the ratio assertion still **passed** while the sibling peak-in-flight assertion failed with `assert 1 > 1`. A bigger multiplier would have bought silence at the price of guarding nothing. So the timing gate is deleted and the fan-out claim is asserted as **overlap**: `test_batch_overlaps_where_serial_creation_cannot` measures the maximum number of creations simultaneously in flight and requires it above 1 for a four-way batch, which fails if the pool is removed and holds at any host load. The same counter also runs over the same four specs created one at a time and must read exactly 1 — an in-test vacuity floor, so a counter that always reported above 1 can no longer satisfy the batched arm on its own. The file's header now states the rule for the next reader: nothing here may gate on a duration or on a ratio between durations, because the pool's win is bounded by git's brief repo lock and the two samples are never taken under the same load.
- **A workflow run that called no model at all is no longer told a free local model ran.** `IntrospectPanel`'s answer to "what is costing money" routes through `runCostText`, which knew only two zeros — a measured one (`priced: true`) and an unrecorded one (`priced: false`) — and spelled the measured one *"every step was measured and cost nothing (a free local model)"*. But a deterministic template calls no model: measured on a fresh container with **no model provider configured**, `knowledge-health` completed two steps whose `step_completed` rows each carried `"model": "", "provider": "", "cost_usd": 0.0`, so `priced` was `true`, the measured-zero branch fired, and the panel announced a free local model directly beneath its own `Models · none recorded` cell — which reads that very field. The sentence now takes the third fact it was missing and says *"no step on this run recorded a model, so there was no model spend to price"*; a run that did name its model (an Ollama `gemma4:12b` at `cost_usd: 0.0`) keeps the free-local sentence, because there the claim is true.
- **Launching a loop with no model bound now tells you where to fix it, instead of asking you whether you fixed it.** [#3470](https://github.com/PersonalClaw/PersonalClaw/issues/3470). `POST /api/loops/classify` refuses with 409 `model_unresolved` and a complete sentence — *"No model provider resolves for use case 'background'. Connect a model in Settings → Models to analyze tasks."* — worded deliberately at the handler so the front end's no-model matcher recognises it. The composer called it with `.catch(() => null)`, threw the whole `ApiError` away, and rendered a fixed **"Could not analyze the task — is a model configured?"** from the `null`. Not a swallow: the failure was terminal, visible, and fabricated nothing. Two narrower things were wrong. It **asked the user a question the server had just answered**, which puts the diagnosis on the person least able to do it; and it **dropped `Settings → Models`**, which is the entire remediation. A shorter sentence is not a kindness when the longer one is the one that says what to do. The rejection is now kept and relayed, so the number of causes you can tell apart on this surface is the number the backend can tell apart. The written sentence survives for exactly one case and is not a dual path: `readableErrText` returns `''` for the closed opaque set (`Failed to fetch`, `Load failed`, `HTTP 500`), i.e. a request that never reached a backend that could author anything — relaying that would paint an empty alert box. The decision lives in one exported function beside the page rather than inline in a 75-line handler, because "whose sentence wins" is the thing worth testing, and the swallow census for the file ratcheted 3 → 2 in the same change.
- **Four labelled cells on a workflow run page rendered a label with nothing after it, on any run that finished in under a second.** `fmtElapsed` returns the empty string for a zero — correct for the inline chips whose callers guard on it (a run that has not started has no elapsed time to show), and wrong for a `<Stat>` or a rail `<dd>`, which render their label unconditionally. Measured on a fresh container: the deterministic bundled template `knowledge-health` completed with `stats.duration_secs: 0.0` and `template_card.duration_p50/p95: 0.0`, so **`Duration`, `Duration p50`, `Duration p95` and the findings rail's `Took`** all rendered blank — 4 of the page's 39 label/value pairs. Its sibling step's `duration_secs: 0.007` rounded to `0s` and rendered normally, so one panel showed the same `Took` label as `0s` and as blank space for two measurements **seven milliseconds apart**; and a blank cell is indistinguishable from `LedgerRailsPanel`'s deliberate em dash, which means *"the ledger carried no key"* — the opposite claim. Those five call sites now use a new `elapsedStat`, the labelled-cell formatter (same rounding, a measured zero renders `0s`, a non-figure reads `not recorded`); the four guarded inline callers keep `fmtElapsed` unchanged.
- **"Needs you" now names one set, so the dashboard can no longer say "All clear" while a project shows work waiting on you.** [#3471](https://github.com/PersonalClaw/PersonalClaw/issues/3471). Measured with a single `blocked` task: the project's Work board read **NEEDS INPUT · 1** while the dashboard, at the same moment, read **"All clear — nothing waiting on you."** Neither surface was lying — they compute genuinely different sets (the card is `approvals ∪ inbox ∪ skill proposals`; the board's `needs_input` group also holds blocked tasks and stalled runs) — and a third use of the phrase sat in the loop-status registry. **The phrase was the defect**: a confident negative about a set the reader has no way to know is narrower than the words. Widening the card's set was the other option and is unbounded — "everything waiting on you" would have to absorb blocked tasks, stalled runs, suspended and review rows, which is the Work board, a surface that already exists, and none of those resolve inline, which is this card's whole contract. So **the narrower set gives up the broader phrase**: `needs_input` keeps "Needs you", the dashboard section is now **To triage**, and its empty state says *"Nothing to triage — no approvals, messages or skill proposals are waiting"* — scoped to the lanes it actually read. The product tour, which quoted the old label, moved with it. A fourth drift went too: the loop-status registry's own header lists `"Needs you" vs "Needs input"` among the divergences it retired when it became the single vocabulary, while the project page's Work-board label map hand-typed `'Needs input'` for the same wire value — a fifth per-page copy its rail could not see, so the drift it declared fixed was live. That map now reads the registry, and a coherence rail compares the two objects directly so it cannot diverge again silently.
- **A screen reader no longer says "tab 2 of 6" for the task form's Status field — every single-choice field is now announced as the radio group it is.** [#3472](https://github.com/PersonalClaw/PersonalClaw/issues/3472). Status and Priority rendered as `role="tab"` inside a `role="tablist"` **with no tabpanels**, so the user was told they were navigating a tabbed interface that does not exist and told nothing about the values being mutually exclusive alternatives of one field; `tablist` additionally promises manual activation and a revealed panel, and neither ever held. Both fields are the shared `Segmented` control, and the census is what makes this a primitive fix rather than two call sites: **50 call sites, and zero `role="tabpanel"` anywhere in the app** — the only two are in the chat Activity panel, which does not use `Segmented`. So sixteen other genuine fields (schedule When/Runs, trigger type, approval mode, loop granularity and project kind, inbox reclassify, the approval remember-scope, the report cadence, …) were making the same untrue announcement, and fixing the two named ones would have left them. `Segmented` is now `role="radiogroup"` + `role="radio"` + `aria-checked`. **The keyboard handler did not change at all**, which is the evidence the roles were the only thing wrong: a radiogroup's arrow model is "move AND select", which is what it already did, whereas a tablist's is manual activation. The six genuine tab strips that do reveal a pane (chat Activity, Memory Studio, the loop cockpit, Files, Terminal and its drawer) are hand-rolled and untouched. The misuse also **taught the wrong test query** — `getByRole('button', { name: 'High' })` times out against a `role="tab"`, so authors reached for `getByRole('tab')` and encoded the wrong semantics as correct; 24 such queries across 14 files moved to `role="radio"` with the markup, and a rail now fails any new one outside those six strips. A prior owner ruling recorded this as an open question in the tab-strip rail; that rail now asserts the answer.
...
- **The home screen’s “Needs you” card now opens the inbox item it names, instead of dropping you on the inbox list.** A captured note appeared in the card under a control whose own label was `Reply: user — Decide whether the IoT VLAN…` and whose tooltip read “Open to reply” — and pressing it navigated to a bare `#/inbox` with no item panel open. `routeFor` spelled the inbox destination `'inbox'` and the Reply handler spelled it `navigate('inbox')` a second time, directly beneath a comment promising “reply in the detail where the draft editor lives”: the comment described the intent and the call did something else. `?open=<id>` is the inbox’s own deep link — `InboxPage` reads the `open` param and its list carries an `anchorKey` added precisely so a deep-linked row scrolls into view — so the destination existed the whole time and only the id was dropped. This is the same defect #258 named on the approval lane of this very widget (“the only row that could resolve the approval also sent the user nowhere if they opened it”), left behind when that lane was fixed. Both entrances now resolve through one derivation, and the id is URL-encoded so a channel-backed id’s colons cannot re-split the query.
- **Two security controls that resolved an unreadable config to their most permissive value now refuse instead.** [#3456](https://github.com/PersonalClaw/PersonalClaw/issues/3456), [#3457](https://github.com/PersonalClaw/PersonalClaw/issues/3457). Both are `except` arms around `AppConfig.load()`, which is a different mechanism from [#3424](https://github.com/PersonalClaw/PersonalClaw/issues/3424): that one was the *normal return* handing back a permissive dataclass default, closed centrally in the loader, and no loader change can reach the *exception* path. **`guard_unattended_spawn`** caught the read failure and `return`ed, which is no gate at all, in a function whose own docstring says *"Fail closed: a runtime with no catalog row cannot be verified, so it is refused too — the flag's whole promise is that nothing unproven runs while nobody is watching."* A read that failed cannot establish that the operator turned `agent.unattended_requires_verified_adapter` off, so it now raises `UnverifiedAdapterError` naming the read failure; the refusal stays narrow because the existing `if not unattended or not runtime_id: return` runs first, so interactive chat and the native in-process loop never reach the config read. **`guardrails/denylist.py`'s `_load_config_rules`** returned `([], [])`, waiving every `security.autonomy_denylist` glob and every `security.denied_commands` pattern the operator set, on the reasoning that "the built-ins still apply" — but the built-ins are a different and narrower control (a fixed sensitive-path list plus fixed command patterns), so they are not a reason to drop the operator's half. A denylist that could not be read has not said nothing is denied; it has said it does not know. It now returns `None` for a failed read, and `check_action` refuses the action with `matched="config:unreadable"` **before** any other check, so the message a user sees says why everything is being refused rather than whichever built-in happened to match. **No value is substituted in either case, deliberately**: a denylist's restrictive value is "everything", which a list cannot express, so this is the category `CONFIG_ON_DISCARDED_READ`'s membership rule excludes — refusing is a restrictive posture that invents nothing. Both arms are only reachable on an *unexpected* raise now that a corrupt `config.json` resolves fail-closed by value, so reaching either one means the host cannot read its own configuration, which is precisely when nothing unattended should start. The third site found in the same sweep, `budget_from_config`'s fall-back to an unlimited spend ceiling, is **not** changed here and is filed as [#3458](https://github.com/PersonalClaw/PersonalClaw/issues/3458) with the measurement: five of its six production callers already fail open on purpose with the reason stated in code, so a raise there would be swallowed at five seams and is already redundant at the sixth (`proactive/autoexec.py`, which refuses on the grounds that "an unverified ceiling authorises nothing"). Shipping it would have added another protection that reads as present and cannot fire.
- **A first chat no longer shows the assistant's finalized reply twice until reload.** Creating a session remounts the chat view, and two independent races in that one window could each paint the same completed answer a second time. **History and the stream both delivered it:** the remounted view's history refresh can observe the persisted assistant message before the terminal WebSocket text reaches the tab, so the refresh painted the canonical answer and then the stream coalescer appended the same text beside it, even though storage held one message. The client now fences only that current turn's replay — the refreshed history must be a strict extension of the same cached latest-user tail — and suppresses text chunks alone until the terminal status arrives, so a stale history backfill, an edit or variant switch, or an older assistant arriving while a newer user turn streams cannot arm it, and tool and activity frames still refine a multi-segment answer. **The terminal flush appended instead of replacing:** the replace-vs-push decision was read inside a deferred React state updater, so an updater React applied *after* the turn's closing boundary had released the live text run saw "no run in progress" and pushed the full answer beside its own streamed prefix. Ownership is now claimed synchronously at the moment each flush is emitted, so a boundary can no longer move the answer out from under an update already in flight.
- **A `config.json` that cannot be parsed no longer re-widens a deliberately narrowed security posture — it now fails CLOSED.** [#3424](https://github.com/PersonalClaw/PersonalClaw/issues/3424). `AppConfig.load()` discarded an unreadable config and substituted the dataclass defaults with no signal to the caller, and three of those defaults are *less safe than the narrowest value the field can hold*. Measured on an operator config storing `interactive` and an empty `subagent_cwd_allowed_roots`: a **truncated** file (a disk-full write, an interrupted save, a partially synced home) and a **non-object** payload both came back as `approval_mode: auto` — which auto-approves every tool call for a subagent's lifetime — and re-populated `subagent_cwd_allowed_roots` with `["~/workspace","~/workplace"]`, where an empty list is not an absence of configuration but *how the feature is disabled*. **Non-UTF-8 bytes were a third behaviour again**: `read_text()` carried no `encoding` and the `except` did not name `UnicodeDecodeError`, so those escaped the loader and raised out of `load()`, while `entity_routes` had already widened its own `except` to include them — the two loader families disagreed about the same input. The shape worth naming is that a guard *had* been written for exactly this: `subagent.py`'s cwd check has an `except` arm whose comment forbids the re-widening, and it could never run, because this loader does not raise — it returns. A protection that read as present and was unreachable. `load_with_migration_state` now has **three** outcomes rather than two, the same contract `providers/entity_routes._load_entity_settings` acquired in [#3411](https://github.com/PersonalClaw/PersonalClaw/issues/3411): *absent* (no file, or zero bytes — first run, where the defaults genuinely are the user's intent), *parsed*, and *discarded*. A discard resolves the fields in the new `CONFIG_ON_DISCARDED_READ` table to their most restrictive value (`agent.approval_mode` → `interactive`, `agent.subagent_cwd_allowed_roots` → `[]`, `agent.unattended_requires_verified_adapter` → `True`), and the table carries a **stated membership rule** so it cannot grow by vibe: the field must be a security control *and* its restrictive value must be expressible. That second half is why the token and RSS ceilings are deliberately **not** in it — `guardrails.budgets.max_tokens_per_*` and `sandbox.max_pids`/`max_rss_mb` default to `0`, meaning *unlimited*, so a discard does drop an operator's ceiling, but there is no restrictive number to substitute and inventing one would put a bound the user never chose in front of them as if it were stored. **Failing closed does not brick a first run**, which is asserted rather than assumed: zero bytes (a bare `touch`, a create that never got its bytes) reads as *absent*, matching the ruling `read_config_for_merge` already made for this same file on the write side. And the substitution is no longer silent — `personalclaw doctor` reports an unreadable config as an issue, rendering the substituted fields from the table rather than retyping them, next to the `approval:` line that used to print `auto` while the file said `interactive`. The operator's bytes are never overwritten by the substitute: `migrated` is `False` on a discard so the writing boot path returns early, and `save()` would refuse anyway because `read_config_for_merge` raises rather than writing over a document whose unmodeled blocks it cannot preserve. Also moved off the hot path: the discard is logged once per distinct outcome instead of on every one of ~300 reads per session, the same discipline `validate_config_data_cached` already applies one line below it.
- **The dead-link gate was scanning 76 of the repository's 267 markdown files, and the 191 it skipped included every document a stranger opens first.** `docs-lint-baseline.json` reported `dead_link: 0` and the ratchet that defends it was green, but the census scope was `docs/**` — so `README.md`, `SHOWCASE.md`, `CONTRIBUTING.md`, `AGENTS.md` and every README under `src/`, `web/`, `mobile/` and the bundled app directories were never opened. A gate whose only claim is "docs links are checked" was not checking the front page. The scan set is now **every tracked `*.md`** (267 of 267, asserted by equality against `git ls-files` rather than by a `> 0` floor — the old floor is exactly what let the scope sit at 76 while reading green), and markdown **images** are in scope too, since a broken image is a visibly broken document. Widening it surfaced two real dead links that had been invisible, both fixed here so the census still lands at its measured population instead of baselining a known defect: **(1)** `mobile/store/play-data-safety.md` carried a find-replace accident where the link *text* stayed `MOBILE-COMPANION` while the *target* became the disclaimer sentence `../../the MOBILE-COMPANION plan (internal, not in this repo)` — it rendered as a live link to a percent-encoded sentence plus a stray paren, and the sibling `mobile/README.md` already handled the same situation correctly as plain prose; **(2)** the bundled `ollama-models` README pointed at `../LICENSE`, one directory too high, when its `LICENSE` is a sibling — and called it "the apps repo LICENSE" although the bundle is vendored into this repo. Three false claims in the generator's own docstring went with it: it cited `EXECUTION-PROTOCOL §3` and `tools/gen_roadmap_dashboard.py` as authorities, neither of which is in this repository, and it asserted the `**Status:**` regex "agrees" with that absent generator. It now also states plainly that a dead **anchor** passes silently (only a missing *file* is a finding) and that a raw-HTML `<img src=…>` is checked by no rule here — a checker that cannot fire, left undocumented, reads as coverage it does not provide. Both widenings were proven non-vacuous separately: injecting a dead link and a dead image into `README.md` produced two findings under the new scope, zero under the old one (which never opened the file), and one under the old regex (which excluded images).
- **The composer no longer reports itself idle while the previous run is still live, so a message sent in that window is not silently absorbed.** [#3444](https://github.com/PersonalClaw/PersonalClaw/issues/3444). Sending on a brand-new chat *creates* the session, and backfilling the key into the URL re-keys `ChatSession` — which remounts it. The replacement's `streaming` flag was `useState(false)`, so for one async round trip, until the mount load's `running` came back, the composer's action button read **"Send message"** over a run that was still going. `resolveSendButton` was already correct; its precondition was wrong. `send()` branches on the same flag, so a click in that window started a **fresh turn**: it painted a user bubble and posted with no `queue_mode`, and the server — which knows the run is live — did not dispatch a turn but queued the message and answered `{queued:true}`, a response that path does not read. Measured from a Playwright trace driving six scripted turns: **5 user turns rendered for 6 sends**, prompt (2) absent with no error and no retry affordance. The live run is now handed **across** the remount: `ChatPage` owns the session key a run was dispatched for — above the boundary, exactly as it already owns the pending routing suggestion for the sibling defect from this same remount — and the replacement seeds both `streaming` and the `streamingRef` that `send()` actually branches on from it. So the button reads *"Steer — send into the running turn"* and the click takes the steer path, which paints no phantom bubble at all: the message is injected into the running turn and echoed as a `steered` chip, or queued with its own `queue_push` chip when the runtime has no drain seam. Either way it is on screen. The handoff is released the moment a turn settles, so a later visit cannot inherit a finished run and offer Steer over an idle backend.
...
- **A workflow loop whose body is an agent step now runs more than one round.** Every round of such a loop finished its work and then nothing happened: the run reported "deadlocked" after exactly one iteration, which is why a loop template's "stop when a round adds nothing new" never got the chance to be true. The two places a step can finish were not treated alike — an in-process step advanced the loop and a dispatched agent step did not — and only the second is what every shipped loop template uses. **Two related limits are now measured and named rather than latent**, and a loop that hits either is handed to you instead of quietly looking busy: an agent step cannot yet be re-run by the same loop inside its 15-minute duplicate-execution window, and an agent step's answer does not yet reach the next node in the shape its template declared (so a loop's "did this round find anything?" field cannot be read). Both are on the list to fix next.
- **A workflow loop's round cap written as `{{inputs.…}}` was not a cap at all.** `deep-research` declared its round limit that way, and the engine only honours a literal number — so the one template using the form had no upper bound on its rounds, and the template linter accepted it on the incorrect grounds that the value is resolved before the loop starts. The cap is now a literal, the linter reports the binding form as unbounded, and the loop also declares a stall timeout and a real "nothing new this round" exit.
...
- **A release can no longer publish a gateway image whose dashboard does not work.** [#3558](https://github.com/PersonalClaw/PersonalClaw/issues/3558). `release.yml`'s gateway smoke runs `personalclaw --version` with `--entrypoint ''`, so the gateway never started. It proved the binary was in the image, not that the product worked, and nothing else at release time touched the dashboard. The wheel had the opposite story: `scripts/verify_wheel.py` is a release-blocking step that asserts a packaged `static/dist/index.html` plus a live `GET /` 200. So the wheel could not ship broken and the image could. Measured 2026-09-23 in containers against the published 0.1.3, on both on-ramps `README.md` offers: `uv tool install personalclaw` served `GET /` 200 with two asset references and a 6,964,545-byte first bundle, while `docker run … ghcr.io/personalclaw/personalclaw-gateway:latest` served **503** with the "Build the dashboard" placeholder, **zero** asset references, and no `static/dist` in site-packages. `/api/healthz` answered 200 `{"status": "ok"}` on both, which is exactly why a status-only gate calls the broken one green. A new release-blocking `gateway-dashboard` job now builds `Dockerfile.backend --target runtime` from the release commit and runs `tools/docker_single_container_smoke.py --image` against it: healthz 200, `GET /` 200 HTML carrying the SPA mount point, and the shell's **first script fetched and required to come back with a JavaScript content-type**, because the SPA's `index.html` fallback answers any unknown `/assets/*.js` with 200 and a status-only check cannot tell that from a real bundle. `images` now `needs` that job, so **nothing is pushed** unless the assertion passes: `images` pushes `:X.Y.Z`, `:X.Y` and `:latest` and only then smokes what it pushed, so an assertion added after the push would leave a broken `:latest` in GHCR with no GitHub Release to explain it. It reuses the tool `.github/workflows/docker-single-container.yml` already drives on every relevant PR, so the release gate and the PR gate cannot drift into two dialects, and it builds rather than pulls, because a published tag is a past commit and `ghcr-anonymous-pull.yml` already owns the published-artifact question. Proven in both directions before landing: an image built with the Dockerfile's `web` stage deleted fails with `/ returned 503 (want 200 HTML)` while its healthz still reads 200 `ok`, the real published 0.1.3 image fails the same way, and the correctly built image passes with its 6,159,438-byte bundle served as `text/javascript` and the negative control returning 404. The `--entrypoint ''` per-arch smoke is deliberately untouched, since it is arm64's only execution evidence and the dashboard gate runs the runner's native amd64 only.
...
- **First-run setup could require a model provider it offered no way to configure: Ollama's on-ramp is discovery-only, and Ollama is never in the app catalogue because it is already installed.** [#3529](https://github.com/PersonalClaw/PersonalClaw/issues/3529). The Essential-apps step marks "Model provider" **required**, and its zero-key on-ramp for Ollama offered only a loopback probe plus an opt-in LAN sweep — if both missed, the block said *"No local model found on your network"* and offered nothing else. The catalogue below could not help either: `ollama-models` ships `native: true` (pre-installed), and `resolve_catalog_entries`'s "Library exclusion" correctly drops an already-installed app from "available to install" — but that catalogue card was the *only* route into `ConfigureProvider`, the schema-driven form that renders a provider type's `settingsSchema` (`endpoint`, required and settable per `GET /api/model-provider-types`). So the one provider already installed was the one provider onboarding could never let you configure. Confirmed as the common case rather than an edge one: a **containerised** gateway's loopback is the container's own, never the host's, and its LAN sweep scans the container's own subnet — so for every containerised install, discovery finding nothing is close to guaranteed (measured live: the host's own Ollama answered at `host.docker.internal` and an RFC-1918 gateway address, neither of which the sweep enumerates). `EssentialsStep` now reads `/api/model-provider-types` itself and, for any registered type whose app has no catalogue card — computed against the live catalogue, never hard-coded to Ollama — renders an "Already installed" fallback that goes straight into the same `ConfigureProvider` the catalogue path uses, driven entirely by that type's own `settingsSchema`, so a provider that ships pre-installed tomorrow with a different required field needs no onboarding change. The on-ramp's own empty states are no longer silent either: a missed loopback probe used to say nothing at all until "Scan my local network" was clicked, and an empty scan named no next step — both now state the fact and point at the manual entry, and only when that route truthfully exists (the same registry read shared with the fallback above, never a second guess).
