...
- **A scheduled script can now read a tool refusal instead of dying on it.** [#3407](https://github.com/PersonalClaw/PersonalClaw/issues/3407). `ScriptContext.call_tool` documents *"Returns the result dict"*, but the in-sandbox launcher's `_post` helper called `urllib.request.urlopen`, which **raises `HTTPError` on every 4xx and discards the response body**. So every refusal `POST /api/tools/invoke` can issue was unreadable to a cron-script author — `risk_confirmation_required` (403), `tool_disabled` (403), an unknown tool or provider (404) — and the reason the route put in the body was thrown away. The docstring's own teaching example was the case that broke: it tells authors a call without `confirm_risk="destructive"` is refused with 403, and the guard that implies (`r = ctx.call_tool(...); if not r["ok"]: ...`) never ran, because the call raised. `_post` now catches `HTTPError`, reads the body the route sent, and returns it as `{"ok": False, "error": <sentence>, "status": <code>}` plus `"code"` when the gateway sent one — so a script branches on the reason rather than crashing on it, and both of the route's two failure shapes (the `{"error": {"code", "message"}}` envelope and the flat `{"ok": false, "error": str}` form) arrive readable. The line held deliberately: a request that got **no answer at all** (no gateway listening) still raises, because "the gateway refused" and "there was no gateway" are different facts. `ctx.notify()` shares the helper and gains the same behaviour, which its docstring now says. Pinned by a test that runs the **real** launcher as a subprocess against a stub answering the way the route does — it reported `HTTPError: HTTP Error 403: Forbidden` before the fix, with the 200 arm passing in the same run.
