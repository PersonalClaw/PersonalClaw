...
- **A content search that hits its deadline *inside* the worker thread now returns the same 504, audit row and "Narrow the directory" advice as one that hits it outside.** [#3399](https://github.com/PersonalClaw/PersonalClaw/issues/3399). The Python fallback is bounded twice — an outer `asyncio.wait_for` and an in-thread deadline/stop check that raises `_ContentSearchTimedOut` — and the two rails race. The handler's `except` named only `TimeoutError`, so whichever of the two won decided whether the user got a 504 or an unhandled exception. The sentinel is deliberately **not** a `TimeoutError`: that lineage is an `OSError`, which the fallback's own `except OSError: continue` read handler would swallow into a partial result reporting `truncated=False`, so re-basing it was the wrong close. Naming it in the handler's `except` is the right one, and the escape is now pinned by a test that reds with the bare exception on the old clause.
