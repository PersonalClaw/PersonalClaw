...
### Removed
- **⚠️ Two runtime-editable config fields that governed nothing are gone: `knowledge.idempotent_persist` and `workflows.max_concurrent_nodes`.** [#465](https://github.com/PersonalClaw/PersonalClaw/issues/465). Both validated, persisted and read back cleanly through `PATCH /api/config/personalclaw` while no code anywhere read the value — so the round trip convinced you the setting had taken effect and nothing changed. Neither is replaced, because in both cases the honest fix was deletion rather than a reader. `idempotent_persist` is an **invariant, not a setting**: its own help text named the reason ("stops a retried, resumed or rewound synthesis node from writing a second near-identical article that later reads as independent corroboration"), and off would manufacture exactly that corroboration — knowledge writes stay resolved by content-derived identity, unconditionally. `max_concurrent_nodes` claimed to be a total "partitioned across typed lanes", and the two per-lane fields beside it — `workflows.max_concurrent_llm_nodes` and `workflows.max_concurrent_io_nodes` — *are* the live partition; use those, which now have controls on Settings → Workflows. **If you had either key in `config.json`, it is now ignored** (loading does not fail; the key is simply not read). Class B/S, clean break under the pre-1.0 banner: no gate and no migration, and `personalclaw snapshot` before upgrading is advised as usual. Four of the twelve inert paths that issue measured remain open and are named there.
