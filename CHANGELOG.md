...
- **The context gauge is monotone, in range, and honest about a window nobody declared.** [#3405](https://github.com/PersonalClaw/PersonalClaw/issues/3405), [#3406](https://github.com/PersonalClaw/PersonalClaw/issues/3406). Two independent defects composed into one unusable instrument on a locally-bound model, and each hid the other. **The denominator was fabricated:** every adapter carried its own absent-model fallback (`_DEFAULT_CONTEXT_WINDOW`, 128000 in `llm/openai.py` and 200000 in `llm/anthropic.py`) and divided by it for any model `model_tokens.json` has never heard of. Measured live against Ollama 0.34.2 / `gemma4:12b` serving a 32768-token window, that denominator was **3.91x too large**, the highest value the gauge could *ever* report was **25.19%**, and the 70% compaction trigger was therefore not late but arithmetically **unreachable** — while the reading rendered with exactly the same confidence as a real one. **And the numerator fell as the context grew:** `usage.prompt_tokens` is what the provider *accepted*, so on a runtime that truncates silently it tracked the real prompt to **98.39%** of the window and then collapsed to a flat **50.01%** (16,387 tokens, half the served window) for every larger prompt, returning HTTP 200 with no exception to catch. A context that was 100% full and shedding input read as half empty, and a session that stepped over the ~120k-145k-character band between two turns never saw a reading above 70% again. **One owner now answers both.** `personalclaw.context_gauge` is the single place a token report becomes a reading, and it reports one of three things: a measured percentage, **full** (100%), or **nothing**. `model_windows.resolved_context_window` is the one reader that can say "no window was declared" — the per-binding `context_window` override, else the table entry, else `None` — and there is deliberately no default behind it, because a percentage of an undeclared window is a fabricated measurement rather than a conservative one. The adapter-local fallbacks are deleted, not deprecated. A reading is **full** when the provider reported at least a window's worth, or when it reported **fewer** input tokens for a prompt at least as large as one it has already measured — a provider handed a larger prompt cannot honestly report fewer tokens. That rule is deliberately **ordinal**, and the first attempt was not: it compared the report against a chars-per-token floor, and a live drive falsified that outright. Measured on one host against one model, a prompt of repeated characters tokenizes at **7.99 chars/token** while the curve recorded in #3405 ran at **4.50**, and no single floor straddles those — being safe against sparse text requires dividing by more than the densest content's ratio while catching a half-window truncation requires dividing by less than half the sparsest content's, which across 2-8 chars/token is arithmetically unsatisfiable. The observed false positive was a **998-character prompt reading 100% full**. The shipped rule assumes nothing about tokenization, so it is indifferent to content; the gauge is therefore stateful per binding (two integers: the largest prompt it has seen and what was reported for it), and compaction is handled by the same comparison rather than by an exception, since compaction *shrinks* the prompt and the reading is free to fall with it. Two limitations are asserted rather than hidden: a binding whose very first prompt is already truncated has nothing to compare against, so it reads flat rather than falling until a larger prompt arrives; and a hypothetical provider that caps every report below its window carries no signal at all, which a test pins as unreachable so the exclusion cannot quietly become a claim. **The bundled `ollama-models` app got the same treatment on both halves**, because a UI-configured Ollama binding is served by that app and not by core: its `/api/ps` served-window probe is unchanged, but a probe that answers nothing now yields an unmeasured gauge instead of falling back to `LOCAL_SERVED_CONTEXT_WINDOW` — dividing the measured 16,387-token report by that 4096 floor displayed **400%**, and `model_windows` says in its own trap note that the floor is for the char *estimate*, where erring toward early compaction is cheap. The floor keeps that one honest job, and `runtime._estimated_context_pct` is now reachable on the undeclared local path precisely *because* the measured gauge declines. **In the UI, unknown says so:** the composer's plain dot (already the "no measurement" state) now carries a title naming the cause and the remedy — set "Served context window" on the provider in Settings -> Models — because a ring and a bare dot are one pixel apart, so "unknown" was indistinguishable from "barely any context used". **Four invariants are now property tests** (`tests/test_context_gauge_properties.py`, hypothesis), and each is one the shipped code violated: a larger served prompt never lowers the reading; `0 <= pct <= 100` with past-window states representable rather than wrapping; **every configured trigger threshold is reachable for every window the product can resolve**, which is the one that catches "the peak is 25.19%, so the gate can never fire"; and an undeclared, unlisted window yields *unknown*, never a confident number. The provider model they sweep is the load-bearing part: an honest provider, a capping one, and the **measured** Ollama step function that reports the true count while the prompt fits and half the window once it does not — a suite that only swept monotone reports would pass on the broken gauge. Text density is swept from 2.0 to 12.0 chars/token for the same reason, and it is the property run that caught the capping provider's unreachability. **Verified against the live binding after the fix**, same host and model: with the window declared the sweep reads 0.43 -> 28.65 -> 45.82 -> 55.37 -> 57.27 -> **100 -> 100** percent across 998 to 600,000 characters — monotone, in range, crossing every trigger, with the 998-character false positive gone; undeclared, every arm reports nothing. **The gap that reopened #2364 is closed with a paired test, not a docstring:** the override did reach the measured gauge, but every test proving it used `llama3.1:8b`, a model the *table* lists, so the undeclared arm had a window either way and the assertion could not tell "the override was read" from "a window was found". On `gemma4:12b` — absent from the table, and the model the issue was measured on — the undeclared arm now reports nothing and declaring `"32768"` alone moves the live gauge to 81.4% and over the compaction threshold, same provider, same turn, one variable.
