...
- `PATCH /api/config/personalclaw {path, value}` — single-field writes, allowlisted (`_EDITABLE_CONFIG`); non-editable paths return 400. Turning `agent.yolo` on also needs `"confirm": true` in the body (the JSON literal), or it answers `400 confirmation_required`.
