...
**PersonalClawApps** (PR #76), where the suite lives per this plan's §7 and `PEP-11`'s scope — one
...
real scaffold (`personalclaw app new research-lab --type tool`) and filled in: five tools over a
...
Verified: all eight PersonalClawApps CI jobs run green locally before the push — `tests` (30, bare
...
PersonalClawApps is already a default git Store source (`catalog._DEFAULT_GIT_SOURCES`), so a merged
