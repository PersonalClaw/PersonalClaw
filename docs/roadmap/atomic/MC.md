...
**Status:** done — clause 1 landed 2026-08-26; clause 2 (SW sound/badge mapping) landed 2026-09-04 via #2359.
...
**Clause 2 — the SW sound/badge mapping — LANDED 2026-09-04 (#2359).** With `MC-5` on `main`
(the `push` handler in `web/src/sw.ts` and `web/src/app/pushPolicy.ts`'s `notificationFor()`
that compose every word of the notification from the payload's `kind`), #2359 added the
per-(source,kind) `sound` key to plan 42's rules field — `notification_rules.Rule` was
`mode`/`targets`/`conditions`/`verify` with no `sound` key before, and `MC-5` did not add one —
together with its coercion + PUT guard and a control in `NotificationRulesMatrix.tsx`, drawing
the voice from the closed set in `web/src/design/soundCues.ts` rather than minting a vocabulary.
A service worker cannot play audio, so a matched `kind` vibrates/badges the notification and
messages an open client to play the voice. See the plan's Execution log for the shipped design.

### `MC-7` — S4 Capacitor shell wrapping the served companion route

**Status:** done — 2026-09-04. The shell, its config, the navigation rail, the safe-area
mechanism and the repo-location decision all landed and `no forked UI` is rail-enforced; `mobile/`
is the first production consumer of `web/src/lib/endpoints.ts`. OWNER-RESIDUAL: the
`builds for iOS+Android and renders the live companion` clause needs an Xcode + CocoaPods +
Android SDK pass on a Mac — a toolchain gate, not code (`mobile/ios` and `mobile/android` are
deliberately gitignored, so nothing in-repo builds them and no code change closes it).
`docs/roadmap/plans/MOBILE-COMPANION.md`'s Execution log names exactly what a human must install
and run.
