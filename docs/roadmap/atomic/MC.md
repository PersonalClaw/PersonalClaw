...
**Status:** 🟡 **PARTIAL, 2026-08-27.** The shell, its config, the navigation rail, the safe-area
mechanism and the repo-location decision all landed; `no forked UI` is rail-enforced. The
`builds for iOS+Android and renders the live companion` clause is **environment-gated, not
skipped** — the development host has no Xcode (`xcodebuild` is a Command Line Tools stub), no
CocoaPods and no Android SDK, so neither native build is runnable anywhere in this repo's CI or
on this machine. `docs/roadmap/plans/MOBILE-COMPANION.md`'s Execution log names exactly what a
human must install and run.
