...
**Status:** 🟡 2026-08-27 — the schedule half landed; every `done_when` clause is now gated.
The three that were unmet (the clock job firing, `cadence: monthly|weekly|off`, and "`off`
disables cleanly") ship as one system clock trigger
(`action_providers/identity_report_provider.py`), ONE config field
(`learning.identity_report_cadence`) and a cadence control on the Learning page. The
`PHF-14` dependency this atom recorded is GONE: `config/loader.py` measured 5581 lines on
`origin/main` against the 6000 ceiling — 419 of headroom, not the 100 the block was filed
against — so no decomposition was needed or attempted. `dag.json` is the owner's to flip;
...
structural and independent of any model: `evals/runner.py` rewrites `PERSONALCLAW_HOME` to a
