...
| `EI-3` | ✅ (#1227) | Spawn-audit CI test + wrap remaining agent-influenced spawn seams | `EI-1` | tests/test_spawn_ceiling_audit.py is green with every agent-influenced seam ceiling-wrapped or explicitly operator-exempt; adding an unmapped create_subprocess_exec/Popen/StdioServerParameters on a branch fails CI naming the file:line |
...
**Status:** done — merged via core PR #2394 (commit 5b156d829) + PersonalClawApps #64.
...
**Status:** done (2026-09-07, PR #2558 — SC5 measured live against a real tmux 3.7c daemon; the recorded ceiling was an unprobed environment, not an ungrantable one, and owner checklist #2490 is obsolete). The 2026-09-03 PARTIAL note is kept below for history: — **PARTIAL, 2026-09-03.** The runner-lifecycle half (idle-release + WORK-R8-style lease
record + transparent reconnect, §3.1(5)) and the durable-session RECOVERY half landed in merged commit
485ef7b6d: the recovery sweep reattaches to a still-alive tmux worker and the lease holder renders in
Settings → Agents. The SPAWN half is unbuilt and stays **EI-2-blocked** — nothing in core spawns a durable
`pclaw-<project>-<run>-<session>` worker (its sole producer has only the reader as a caller), and the
sandbox-interior shell that would spawn it is EI-2's container provider. The atom stays `todo` until the
spawn half lands; do NOT schedule standalone. See the `dag.json` `blocked_reason` and the plan's
2026-08-24 `## Execution log` entry.
