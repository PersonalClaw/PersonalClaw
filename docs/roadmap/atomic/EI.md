...
| `EI-8` | ⬜ (§6 done, §6.2 open) | Turn-bound two-phase file checkpointing + /rewind-to-turn + localhost web preview | `EI-1` | SC8: after a turn that mangled three files, /rewind-to-turn N previews exactly those files with diffs and restores them byte-identical on confirm; .env files were never captured; the checkpoint store respects its cap and prunes with the session; a run's dev server port surfaces an "Open Preview" affordance to localhost:<port> |
