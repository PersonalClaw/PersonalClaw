...
DISCOVERABILITY-LAUNCH decomposed into 9 atoms (DL-1..DL-9): 4 done (S1 migration/scaffold/sync; S2 docs site + llms.txt + landing + release flip, PR #20; S3 screenshots + README rework; T3.4 launch-post draft, published on a source-controlled blog tier in personalclaw.dev@cab5065), 5 todo (T3.1 demo-home seed [core], launch media GIF+social images, comparison pages, listing/checklist program, research republication). Verified against code: only the empty fixture ships (T3.1 open), screenshots+capture infra present, 14-topic learnings corpus at docs/research/learnings/, no live keyurgolani/ product URLs.
...
| `DL-7` | ⬜ | S4 T4.1: comparison data file + peer-product /compare pages | `DL-1`, `EXT:LEARNING-VISIBILITY:benchmark results for matrix rows` | src/data/comparisons.json holds every competitor claim with source_url + retrieved ISO date; /compare/{peer-product} and /compare/hermes-agent render from it with a 'choose them if' section each |
...
### `DL-7` — S4 T4.1: comparison data file + peer-product /compare pages

**Status:** todo

Session 4 — Comparison + listing program (T4.1)

**Done when:** src/data/comparisons.json holds every competitor claim with source_url + retrieved ISO date; /compare/{peer-product} and /compare/hermes-agent render from it with a 'choose them if' section each
