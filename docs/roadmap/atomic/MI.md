...
6 atoms along the plan's own session seams. MI-1 (entity+resolver), MI-3 (duplex), MI-4 (screen-context) are independent and buildable now. MI-2 (cloning engine) carries the sole cross-plan edge on LOCAL-MODEL-MANAGER-V2, and MI-6 carves out its OmniVoice-vs-CosyVoice engine-selection spike as a separate numeric atom (the "MI-2c" remainder — a letter suffix is unrepresentable under the status-sync id regex, so it takes the next free number). MI-5 is the UI + full-matrix validation capstone depending on MI-1..MI-4. MULTIMODAL-IO is complete: all 6 atoms are done — MI-5 (voice bindings + profile-manager UI, one-click migration, full-matrix as-a-user validation; merged 2026-09-04 via core PRs #2386 + #2446) closed the plan.
...
**Status:** done — merged via core PR #2386 (commit 283a5b433) + #2446 (commit 9862ed905).
...
**Status:** done — merged via core PR #2425 (commit 2ec168d00) + #2436 + PersonalClawApps #70.
