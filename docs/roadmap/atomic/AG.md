...
Plan is IN PROGRESS: the S1-S4 substrate (chokepoint, budgets/scan, floor, profiles/egress/health/FE) is fully shipped, and the S5.1/S5.2/S6.1 earned-autonomy rung ladder is now COMPLETE end to end (decision layer, routing at all three dispatch seams, promotion proposals, the HTTP ladder + undo executor, and the FE chip/panel/undo affordance). Six todo atoms remain: the rev-18 research atom `AG-14` (loop ceilings), the inert SafetyProfile/egress-tier wiring gap (audit 2026-08-04, independently completable) and four logged deferrals (apps-repo native structured_output + channel send() live-writes; run-scope budgets awaiting AUTOMATION-SUBSTRATE; profile/trust enforcement behaviors awaiting WORKFLOWS-V2 engine per-template profiles).
...
| `AG-14` | ✅ | Loop ceilings: LoopStopReason enum + wall-clock deadline + max_cost (T05) | — | LoopStopReason is a closed enum persisted on the run record; loops accept optional wall-clock deadline + max_cost and stop with the corresponding reason when exceeded; ceilings wired through config round-trip points; tests cover each stop reason incl. deadline and cost trips; existing free-text consumers migrated. |
