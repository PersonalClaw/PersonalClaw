...
BROWSE-AUTOMATION has shipped its first three atoms (BA-1 extraction, BA-2 CDP+safety-script+BROWSE egress, BA-3 browse loop + ActionProvider contract); with BA-6 (scheduled-actuator) and BA-8 (extension connector) now merged, only BA-5 and BA-9 remain todo. The earlier "nothing has shipped, so all 9 atoms are todo" reading of this header was true when written and is no longer. The plan builds one app-contributed `browse` action provider for token-frugal autonomous web interaction. It decomposes cleanly along its own session/task IDs: original S1-S4 (extraction → CDP+safety+egress → browse loop+ActionProvider → credential handoff), the 2026-07-26 amendment (A1 folds into extraction, A2 live-mirror+auth_needed, A3 scheduled-actuator), and the 2026-07-29 amendment (B1-B4+VB user-browser target via a loopback extension). Hard cross-plan gates: the whole thing is Wave-2, gated on AUTONOMY-GUARDRAILS (egress chokepoint, safety profiles, budget/model-call chokepoint, needs-input gate, earned-autonomy ladder, fail-closed ApprovalGate) and WORKFLOWS-V2-AUTOMATION-SUBSTRATE (action-node dispatch); the actuator consumes WATCHED-SOURCES' escalation seam; the extension connector consumes COMPANION-APPS' device-session/pairing (§C1/C2 — must not fork it).
...
**Status:** 🟡 2026-08-28 — the credentials-never-transit-the-agent invariant is STRUCTURAL and proven
across six surfaces (prompts, step ledger, notes, run payload, park detail, logs), each absence
assertion paired with a control leg; per-site profiles persist under
`$PERSONALCLAW_HOME/browse/profiles/<site_slug>/` and are kept out of exports AND snapshots via
`inventory.IGNORED` (not `secret=True` — a secret entry is captured by snapshots on purpose); both
handoff triggers (§5.3 pre-run, §5.2 mid-run refusal) route through the SHIPPED needs-input gate; a
second run reuses the profile without re-auth. **The human-authenticates-in-a-headful-window leg is
NOT observed** — core deliberately does not launch Chrome (`browse/transport.py`), so the handoff
hands over the binding argv instead; the human step is simulated by `record_login()` and a real
third-party 2FA login is unobservable here. Flips to `done` when this code is on `main`
(`dag.json` is the driver's to write). See the plan's dated log entry.
...
**Status:** done — merged via core PR #2353 (commit 964bab2ca).
...
**Status:** done — merged via core PR #2339 (commit 242a8b253).
...
**Status:** 🟡 every `done_when` clause met and proven; flips to `done` when this code is on `main`
(`dag.json` is the driver's to write).
...
**Status:** done — merged via core PR #2350 (commit cf5b3f661) + apps PR PersonalClawApps#58.
