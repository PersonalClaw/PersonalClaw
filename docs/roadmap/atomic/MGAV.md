...
9 atoms: all 9 shipped (S1 graph model+linker, S2a recall arm, S2b push reflex, §1.3 knowledge pre-pass PR #118, S3 formation+Louvain, S4 vault two-way, memory citations, S5 slots, S5 FE+config). All deps intra-plan; no cross-plan gate.
...
H1, and a non-empty value between the H1 and the `personalclaw:generated` marker.
