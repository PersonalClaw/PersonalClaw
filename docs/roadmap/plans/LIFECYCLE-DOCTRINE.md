...
**Status:** DESIGNED, **DEFERRED BY OWNER DECISION** — deepened 2026-07-18 with code recon (initial PROPOSED 2026-07-18 from the pre-launch investigation & owner alignment review). **Owner decision reaffirmed 2026-07-28: this plan lands near the END of roadmap execution, once the architecture has stopped moving.** Until then the project runs an explicit two-standard posture, now written into the contributor-facing docs (`CONTRIBUTING.md#breaking-changes`, `AGENTS.md`, the README pre-1.0 banner, the PR template): the **maintainer** makes breaking, backward-incompatible clean-break architectural changes with **no migrations** (release notes advise `personalclaw snapshot`), while **contributors are not expected to** — contributor guidance keeps lifecycle-doctrine-shaped rules (stay additive, don't hand-roll gate/migration machinery, surface a needed break instead of shipping it). Consequently: **no `lifecycle/` package exists** (verified 2026-07-28), and any plan task that references `lifecycle/gates.py` or `lifecycle/migrations/` **re-scopes to clean-break form** when the maintainer executes it — the gate is dropped, the migration becomes an idempotent backfill keyed on inspecting the data, and the executor records a DEVIATION and keeps building.

> **Correction, 2026-07-30 (owner ruling).** This paragraph previously ended "…is **not startable as written** until this plan lands." That sentence was wrong and it propagated: it became a hard `Depends on:` in INBOX-NOTIFICATIONS-UNIFICATION, a "🔴 NOT CLEAN / not startable" verdict in the workspace `ROADMAP.md`, and a hard rule in the workspace `CLAUDE.md` — parking a fully buildable, high-leverage plan on **wording rather than on code**. This plan is a *methodology*: it governs whether a breaking change may land in one commit or must go gate → dual-path → migrate → cleanup. It binds **outside contributors**. The maintainer is exempt during 0.x under the README banner, so its absence makes migration-bearing plans **cheaper, never blocked** — the gate, the dual path, and the cleanup session simply do not exist. Nothing in this repository is blocked on this plan. What it is genuinely still needed for: giving future outside contributors real gate/migration machinery, and retiring the pre-1.0 banner on the way to 1.0. See [EXECUTION-PROTOCOL §2](EXECUTION-PROTOCOL.md) and the workspace `AGENTS.md` section "You are working as the OWNER".
**Created:** 2026-07-18
...
**Depends on:** nothing. **Nothing depends on it either, for maintainer execution** (see the 2026-07-30 correction above) — the plans once listed here as dependents (INBOX-NOTIFICATIONS-UNIFICATION, LOOPS-EVOLUTION Phase 4, AUTOMATION-SUBSTRATE trigger unification, DISTRIBUTION S4) all execute as clean breaks under the banner; DISTRIBUTION S4 already shipped that way. The one real ordering constraint is **SECURITY-HARDENING S1**'s keychain slice, which is deliberately sequenced after this plan because silently losing a user's stored credentials is not an acceptable clean break.
