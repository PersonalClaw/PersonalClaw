...
## Execution log — ES-8 (trust-graduation ladder) — decomposed into ES-13..16

- **[2026-09-04] ES-8 done-by-decomposition per owner ruling 2026-08-27 (question 2 of 11).** The ruling narrowed the trust-graduation gate to inputs that exist — ONE rung vocabulary, `guardrails/autonomy.py` (`rung_rank`/`rung_state`/`grant_rung`/`resolve_rung`), rather than minting a second maturity/L3 dialect — and directed decomposing this ~4-session atom. ES-8 keeps no code of its own; its work is now four first-class sub-atoms re-scoped onto those rungs: **ES-13** (§4.2 trust record) on ES-1/ES-5/ES-7, **ES-14** (§4.3 ladder rungs) on ES-13, **ES-15** (§4.4 graduation/revocation) on ES-14, and **ES-16** (§4.4 human-attention accounting) on ES-13. The four are newly filed and unstarted — no code yet. ES-9 (Loop-3 field metrics) is repointed off the ES-8 rollup onto the ladder leaves ES-15+ES-16 so the ready frontier stays honest. This catalog has no rollup/decomposed key convention (zero lettered ids across the tree), so the sub-atoms are first-class numeric atoms, following the WF2UNI-12 → WF2UNI-14 split precedent.

## Execution log — ES-13 (Trust record: per-scope trust ledger feeding autonomy.py rung decisions)

- [2026-09-04][ES-13] LANDED on main; atom flipped ✅. Merged via core PR #2416 (merge commit 00eda6a29). `guardrails/trust_record.py` persists one durable JSON record per action-type scope under `~/.personalclaw/evals/trust/`: `grant_rung` writes the grant receipt (rung, grant time, evidence window) and `demote` writes a revocation receipt (floor, cause, demotion count) — a record write failure never blocks the user's grant, the ledger is the receipt, not a second gate. `resolve_rung` reads the record on every decision and clamps a revoked scope to its floor even when the flat rung store diverges (the record outlives the store — fail-safe direction); only a fresh human grant clears the flag. The single-dialect rail validates every stored rung against `autonomy.RUNGS` on write AND read, so no second maturity/L3 vocabulary is minted. WHY: all four done_when clauses map to shipped, test-railed code; the ES-8 decomposition chain now opens — ES-14 (ladder rungs) and ES-16 (attention accounting) both depend only on ES-13.

## Execution log — ES-16 (Human-attention accounting: per-scope pending-attention debt with decay + demotion signal)

- [2026-09-04][ES-16] LANDED on main; atom flipped ✅. Merged via core PR #2422 (merge commit 01fae0425). §4.4 attention accounting ships as a PURE QUERY over events the run journal already carries — the ledger addition is one optional field, `resolved_after_secs`, stamped when a human answers a gate (dwell measured from the continuation's `created_at`); auto-approved gates deliberately carry none, because a gate nobody looked at cost no attention. `workflows/introspection.py` computes the projections on demand: `attention_events` (human gate answers + mid-flight edits + judge divergences), `attention_debt` under a 7-day half-life exponential decay, `attention_trend` (first-half vs second-half, sample-gated), and `post_grant_rise` — the mechanical demotion signal, which fires only with a real sample on *both* sides of a grant so a single noisy run cannot revoke earned trust. A bounded `GET /api/workflows/attention` serves per-scope summaries and the Learning page's Attention panel renders them on the Table family with StatusPill: rising is the one warn-toned verdict, and an unmeasured trend renders as "too few runs" rather than dressing up as "flat" — the two-weight discipline applied to a metric. Promotion proposals cite the trend through a `note_for` callback the gateway sweep injects, so `guardrails` never imports `workflows` and a citation failure degrades the proposal's prose instead of blocking the proposal. WHY: both done_when clauses map to shipped, test-railed code — decay and trend render on the Learning page from the `resolved_after_secs` addition, graduation proposals cite the trend, and a post-grant rise files a demotion signal into the rung machinery. ES-9 still waits on ES-15.

## Execution log — ES-14 (Ladder rungs: trust thresholds onto autonomy.py rungs + rung chips)

- [2026-09-04][ES-14] LANDED on main; atom flipped ✅. Merged via core PR #2445 (merge commit f2f8ed7c9). Trust thresholds map onto autonomy rungs through the EXISTING `grant_rung`/`resolve_rung` (`rung_rank` orders them): crossing a threshold grants the mapped rung, and no parallel rung ladder is minted — the single-dialect ruling from the ES-8 decomposition holds through this leaf too. Rung chips render from `rung_state` on template rows and in the approval dialog, so the human answering a gate sees the scope's earned rung at the exact decision point rather than on a separate page. WHY: all done_when clauses map to shipped code on the one rung vocabulary; ES-15 (graduation/revocation) unblocks, and with it the ES-9 field-metrics leaf behind it.

## Execution log — ES-15 (Graduation/revocation: promote on sustained success, demote on failure/kill events)

- [2026-09-05][ES-15] LANDED on main; atom flipped ✅. Merged via core PR #2453 (merge commit a96e7776f). The revocation half of the §4.4 ladder — the promotion half already shipped (`propose_promotions` files the graduation proposal on sustained success; a human click grants). This lands the fail-safe direction §4.2 rules: the four trigger events the plan names now mechanically void standing autonomy grants with no human in the loop, while re-granting always takes a click. The shared seam is `ladder.revoke_granted_scopes(cause, evidence_id, source)`: every STANDING grant — a store entry with a `granted_at`; a demotion floor carries none, which is what makes a second pass naturally idempotent — is demoted through `autonomy.demote` (floor + cooldown + trust-record revoked flag + SEL audit), and ONE inbox notification names the scopes, the cause and the triggering evidence id. The four wirings: `model_watchdog.check`'s rebind branch (a trust-invalidating binding change voids the evidence window behind every grant; `evidence_id` carries the fingerprint); `learning/attribution`'s `owes_revert` branch (a HARMFUL LEARN-R16 verdict — the revert proposal retires the CHANGE, this retires the AUTONOMY); `evals/studies.file_demotion_proposal`'s LOSS branch (a failed pre-registered §2 study, the strongest causal evidence the substrate produces); and the gateway autonomy sweep's nodding leg (`workflows/handlers.nodding_revocation_cause` — gateway-side so `guardrails` never imports `workflows`, the same inversion discipline as ES-16's `note_for` callback). The notification is a REGISTERED kind (`guardrails/autonomy_revocation`, WARNING, attention item) with a frontend label row and a source route, because the registry rail red the first CI round on an unregistered emitter — the row is the one durable receipt saying why a user's runs just lost capabilities. One unwritable record does not stop the rest (railed), and revoking with nothing granted files nothing. WHY: every done_when clause maps to shipped, test-railed code — sustained success promotes via the shipped proposal path, all four named events demote/revoke through `grant_rung`/`resolve_rung` so the next run falls back to per-stage, and every transition is SEL-audited (`tests/test_guardrails_revocation.py` + the notification-kinds rails). ES-9 (Loop-3 field metrics) is now the plan's ONE open atom, READY on the ladder side — both leaves it depends on (ES-15+ES-16) are done; its remaining deps are EXT.

## Execution log — ES-9 (Loop-3 live field metrics beside lab results + lab_field_divergence, amendment E3)

- **[2026-09-05] IMPLEMENTATION LANDED on a feature branch; atom stays ⬜ for the owner's dag.json flip.** `evals/field_metrics.py` is E3's ledger query module: one `SubjectRow` per subject — lab score (Loop 1: the newest `results.tsv` `template_ab` row, pin and all) | gate status (Loop 2: the newest gate report on a proposal targeting the subject, projected through the SAME `gate.summary` the inbox row renders) | field trend (Loop 3) — computed per request, stored nowhere new (§4.4 discipline; railed by a file-census test). The two subject families E3 names are decided by the REGISTRY, not the evidence: templates (from recent runs ∪ study rows) and registered action types, because the family decides which demotion a divergence files. Field sources, all shipped records: 👍/👎 from `feedback.jsonl` current verdicts attributed by `producer_id == subject` (the exact `_feedback_rejections` dialect, via a new `producer_verdict_series` reader); edit-before-approve from run-journal `user_edited_mid_flight` against human-answered gates (auto-approved gates yield NOTHING) — **template-scoped only**, because plan 58 defers explicit edit records and no store captures an edit on an action type's output, so that cell is honestly `None`, never `0.0`; approvals/rejections/undos from the SEL tail via `autonomy.approval_outcome_series` (built on the SAME `_outcome_events` walk `promotion_eligibility` counts — one attribution dialect, refactored not duplicated) plus reversed reversal records. The trend is `field_trend` — halves-over-good-rate, sample-gated at 6 signals (`attention_trend`'s reasoning), and `""` renders as "too few signals", never as flat. **The divergence flag is three conjuncts** — lab `score_new > score_old` on the newest row, field trend `falling`, and the newest field signal postdating the lab row (E3's "post-ship") — each with a vacuous partner in the tests, because the flag files a demotion and a demotion must rest on a measured contradiction. **The §4.2 demotion signal is mechanical**: `sweep_lab_field_divergence` rides the gateway autonomy sweep (beside the nodding leg; the evals layer may import both guardrails and workflows), is gated on `evals.enabled` (fail-closed — the row still RENDERS the flag when evals is off; only the demotion is suspended) and on any standing grant existing at all; a divergent action type loses its OWN grant through the new `ladder.revoke_scope` (demote → floor + cooldown + trust-record `revoked` + SEL + the registered `guardrails/autonomy_revocation` notice — scope-attributed evidence must not punish the neighbours), and a divergent template revokes standing grants wholesale through `revoke_granted_scopes`, the same consequence the failed-study and nodding triggers carry, because all three are template-scoped proof that the "system behaves well" evidence behind every grant is void. Both paths gate on `granted_at`, so a standing divergence files once and then nothing. **Surfaces:** read-only `GET /api/evals/field-metrics` (`evals_disabled` 404 / new registered `field_metrics_unreadable` 500; a GET must never demote) and the "Lab vs field" panel on the Learning page (Table family + StatusPill; `lab_field_divergence` and a falling field trend are the warn-toned verdicts; every `None` rate is an em dash). Rails updated: `HTTP_ERROR_CODES` +1, offline reference regenerated; no new config keys, no new notification kinds (the revocation notice reuses ES-15's registered pair), no new persisted files. Tests: `tests/test_evals_field_metrics.py` (module + sweep, and — since 2026-09-06 — the gateway call site) in the production-writer discipline, `web/src/pages/learning/FieldMetricsPanel.test.tsx`. **Honest gap, stated:** the atom's `EXT:FEEDBACK-SIGNAL … edit-before-approve records` dep names records plan 58 never shipped (its §"Open" defers edited-then-sent capture to LEARN-R5) — the template-side rate derives from the run ledger's own gold edit signal instead, and the action-type cell stays unmeasured until an owner rules on growing plan-58 capture.

- **[2026-09-06] Verification sweep: the entry above OVERSTATED its own test coverage; corrected, and the gap it named is now closed.** The 2026-09-05 entry claimed `tests/test_evals_field_metrics.py` covered "module + sweep + wire". It did not: every sweep test drove `sweep_lab_field_divergence` directly, and **nothing asserted the sweep had a production caller at all**. The demotion was genuinely wired (`gateway.py`'s autonomy scan, beside the nodding leg) so the clause held, but it held on one unrailed line — a refactor could have lifted the call out and left a divergence that renders on the Learning tab and demotes nothing, with the whole suite still green. `test_the_gateway_sweep_is_the_divergence_paths_production_caller` now asserts the CALL SITE (not the behaviour, which the other tests already prove) on the `test_the_gateway_scan_is_the_proposal_paths_production_caller` precedent: the sweep appears in `_scan_autonomy_promotions`, and that scan appears in the poll loop that runs it. Second gap in the same shape on the frontend: `FieldMetricsPanel.test.tsx` pinned only the ABSENCE of a gate run, so the Loop-2 half of "renders beside the lab score" was one-directional — two cases now pin the positive direction, one asserting the `gated (+0.042)` cell shares a `<tr>` with the lab score (the "beside" is the claim, so the same row is the assertion) and one pinning a negative delta's sign. WHY the log is amended rather than only appended to: a coverage claim that was false when written is the kind of thing a later reader takes as proof, which is exactly how three sweeps of this roadmap drew false conclusions from a plan's own prose. **Also recorded, not fixed** — two of the four rate cells the done_when names have no live producer in production, both tracing to the `EXT:FEEDBACK-SIGNAL:plan-58` input the atom's dep list already names: (a) the 👍/👎 rate is unreachable for BOTH subject families, because no surface stamps a workflow-template name or an action-type key as a `feedback_producer` — the only production stampers are the three inbox prompt refs, `loop_judge`/`loop.kind`, `routing_pair`, app-namespaced ids and skill keys, and `workflow_surfacing` sits in `PRODUCER_KINDS` with no writer outside tests; and (b) the action-type edit-before-approve cell, per the gap the entry above already states. This is not an ES-9 regression: `field_metrics` deliberately reuses `autonomy._feedback_rejections`'s attribution, so it inherited an already-inert dialect rather than inventing a wrong one, and growing new thumbs surfaces belongs to plan 58 / LEARN-R5. The projection, the UI, the detector and the mechanical demotion the atom OWNS are all shipped and railed; the template-family divergence is production-reachable end to end (a study's pin carries `subject["template_id"]` into the ledger's `scenario_id`, and that is the same workflow name the run-journal signals group by).
...
- **[2026-09-06] MINTED from measured evidence, not from reasoning.** A run of the LV-7 learning
  ...
  builds the child environment as `os.environ.copy()` and then rewrites `PERSONALCLAW_HOME` to a
  ...
  `$PERSONALCLAW_HOME/config.json` with no environment override. There is therefore no path by which
...
- **Least privilege is the child env, and it is the load-bearing half.** `_spawn_cell` no longer calls
  ...
  key variable; its value is forwarded under the cell-scoped `PERSONALCLAW_EVAL_PROVIDER_KEY` and
