...
**Status:** DECOMPOSED — the executable work now lives in [`../atomic/WV.md`](../atomic/WV.md) as 15 atomic plan(s).
...
- **`artifact_update` action provider**: dashboard-style templates generate an HTML skeleton once and refresh it via pure transforms re-binding `{{nodes.x.output}}` slots. This is a NEW action provider → registered in the action_providers registry AND added to `ALLOWED_HOOK_PROVIDERS` (src/personalclaw/validation.py) — see §7.5.
...
| v2 `run-workflow` action provider (hooks/triggers start runs) | `action_providers` registry (`register_action_provider`) + name added to `ALLOWED_HOOK_PROVIDERS` (src/personalclaw/validation.py) — omission means hook_create rejects it even though the UI offers it |
...
- 2026-08-01 — **DONE (Slice 0: scaffold + data model + store + bindings + validator).**
  ...
  **Validated as a user** on an isolated dev home (:10801, never :10000), two boots: all
  six config keys served by `GET /api/config/personalclaw` with correct defaults; a PATCH
...
- [2026-08-11][WV-12] DONE — the two-layer compaction ladder for LLM-backed nodes. A long-horizon node previously died on a context-length rejection the engine could have avoided: it sent the prompt as-is and let the provider decide.
  ...
  **DISCOVERY — a test escaping its home, found via a 120s timeout.** The first e2e draft patched only `workflows.store.config_dir`. That is not isolation: `tasks/native.py:46 _tasks_dir()` binds `config_dir` itself, so one run read the developer's REAL task store (43,277 files) through `_task_map()`, which stamps a derived comment count per task — roughly 259,000 file opens, 35s, tripping the timeout. Setting `PERSONALCLAW_HOME` took it to 1.09s. The 35s was confirmed identical with and without this atom's change before concluding. The fixture docstring records it. Also worth flagging as a latent O(n²) in task creation that any test touching the real home will hit.
  ...
  **Gate (independent, judged by EXIT CODE):** black/isort/flake8 all 0; `mypy src/personalclaw harness` clean on 789 files (mypy caught a real error during implementation — `**pin` splatted into typed kwargs, now named); the new 38-test suite plus `inert_surface_baseline`, `agent_reference` and `docs_lint_baseline` — 67 passed, no baseline regeneration needed. PYTHONPATH proved via `compaction.__file__` resolving under the worktree.
- [2026-08-11][WV-13] DONE — `on_item_error: collect` finally has an executor, and `ItemErrorPolicy` has a ratchet that refuses to let the next member ship without one.
  ...
  **DEVIATION — none on scope.** One process note: a `git checkout -- src/personalclaw/workflows/models.py` used to undo the temporary fourth member reverted the whole file including this atom's own docstring work, which had to be re-applied. Reaching for `git checkout --` to undo a two-line probe is the destructive-git shape; a targeted edit back would have cost nothing.
- [2026-08-12][WV-14] DONE — `on_overlap: queue` finally queues instead of starting a concurrent run, and `OverlapPolicy` has a ratchet that refuses to let the next member ship without a branch.
  **The finding: the policy did the exact OPPOSITE of its name, and it violated the enum's own docstring.** `run_workflow_provider.py:124-175` compared against `SKIP` (early return, `outcome="skip"`, nothing starts) and `CANCEL_PREVIOUS` (`store.request_cancel` per active prior, then start), and let `queue` **match neither branch and fall straight through to `store.create` + `_launch`** — launching immediately beside the still-running prior. `OverlapPolicy`'s own comment reads `SKIP = "skip"  # default — a per-minute trigger must not stack runs`, so the one policy whose name promises ORDERING was the one that stacked them, unboundedly, and silently. Reachable and round-tripped rather than theoretical: `models.py:730` parses `on_overlap` off a def (unknown → SKIP), `models.py:717` serializes it, `native_defs.py:108` accepts it from a def payload, and `inert-surface-baseline.json` had independently listed `enum:OverlapPolicy.QUEUE` as inert for the length of the program.
  **Semantics, one line each.** SKIP (default) — a prior is in flight ⇒ nothing is created and nothing starts. QUEUE — a prior is in flight ⇒ the start is PERSISTED as an unlaunched run and started when that prior ends; ordering, not concurrency. CANCEL_PREVIOUS — a prior is in flight ⇒ cancel it, then start now; the newest fire wins. With a free def all three start immediately: `queue` is not `always queue`, or a per-hour trigger against a one-minute workflow would never start anything directly.
  **The queued-vs-hand-made-draft marker: `run.extra["overlap_queued"]`, written in the SAME insert as the row.** `RunStatus.DRAFT` is where an unlaunched run already lives — but it is ALSO where a user's deliberately-unstarted editor draft sits, so a drain keyed on "DRAFT for this def" would start work the user never asked to start, which is the worst available outcome of this atom. `extra` is a persisted TEXT/JSON column (`store._JSON_COLUMNS`) round-tripped by `_row_to_run`/`_run_to_params`, so the marker survives a restart with the row; passing it through `WorkflowRun(extra=...)` rather than marking after `create` leaves no window in which the row is a plain DRAFT. `test_a_hand_made_draft_is_never_launched` pins it directly: a marker-less DRAFT for the same def survives BOTH drain call sites still DRAFT with no controller. **Rejected, deliberately:** a new `RunStatus.QUEUED` member — a state-machine change, since `active_runs`, `TERMINAL_RUN_STATUSES`, `_ROOT_TO_RUN`, `materialize`'s exhaustive state→status table and the FE status union + `workflowMeta` badge `Record` all switch on that enum and none of them need to (the FE already renders a `Draft` badge with a dashed-circle icon, so a queued run reads sensibly with **no `web/` change and no web gate**); `RunOrigin` — it says WHO started a run, not what it is waiting on; a journal fact — durable but unqueryable, since "which drafts are queued" would mean opening every draft's ledger.
  **The drain's idempotency: `concurrency.single_flight`, reused, not reinvented.** That is the same flock `leases.acquire_claim` is built on. Three independent guards, cheapest first: (1) the flock — cross-process for two gateways, and also cross-coroutine, because flock conflicts across separate open file descriptions even inside one process; contended means SKIP, never wait in line; (2) the supervisor's controller registry, since `watchdog.launch` ALREADY returns the existing controller for a run id it holds — checking it first also closes the window before a new controller's tick loop has written RUNNING, which is exactly when `active_runs()` still reads empty; (3) the "is anything active for this def" re-check INSIDE the lock. Two LIVE call sites: `controller._finish` after the terminal status is written and published — awaited inline, because a floating task makes the handoff untestable and can outlive the loop that made it, and fully guarded because `_finish` is the single terminal writer (WF2-R10) and must not fail a run's recorded outcome over the NEXT run's start — and `watchdog._poll_once` after adoption, via `drain_all`.
  **The cap: one — coalesce-to-one, and never silent.** Depth 1 keeps the promise the name makes (the fire is not dropped; it runs next) while bounding the backlog. Unbounded depth does not: run N+2 does the same work as N+1 with staler inputs, so a workflow that once ran long would spend hours replaying fires whose reason has expired — one late run silently becoming a multi-hour backlog. A dropped start is loud in BOTH places the atom's brief demands: the returned `stdout` carries `dropped`, `reason: "queue_full"`, `queue_depth`, `max_queue_depth` and `queued_run_id`, and the provider logs a WARNING (asserted, not assumed — `test_a_dropped_start_is_logged`). It stays `outcome="skip"`, because nothing durable was created, rather than minting a fourth vocabulary member for a case the existing one describes correctly. `queued` also gates the immediate start, so a fire arriving in the window between "the prior finished" and "the drain launched the pending run" cannot jump the queue — a subtler version of the same out-of-order bug.
  **Restart: the queue IS re-drained, bounded by the 5s poll — stated rather than left to assumption.** A queued start is a durable DRAFT row plus its `spec.json`, and `watchdog._poll_once` calls `drain_all`, so the first poll after the gateway comes back drains whatever was pending; no separate boot hook. Driven by `test_the_watchdog_poll_drains_after_a_restart` with a FRESH `WorkflowWatchdog` that has never seen the run. The one thing deliberately NOT preserved: if the boot sweep suspended the prior to PAUSED, PAUSED counts as active, so the queued run waits for an explicit Resume instead of overtaking a run that is not finished (`test_the_queue_waits_while_the_prior_is_only_paused`) — correct, not a gap. And a queued run whose spec directory vanished is FAILED rather than left queued, safe for exactly the reason the watchdog's orphan reaper is safe (a DRAFT run has no controller): a queue head the drain can never launch would be re-examined on every poll forever and would block every start behind it.
  **`outcome: "queued"` is a new vocabulary member, and that ripple was MANDATORY, not cosmetic.** `triggers.executor._record_fire_outcome` maps an unrecognized runner status to `Outcome.FAILED` — so shipping the string without its readers would have recorded a healthy queued fire as a FAILURE and spent `autopause`'s 5-failure budget on it, the same shape S155 already paid for with `skip`/`done`/`report`. Updated in the same change: `STATUS_TO_OUTCOME`, `SCHEDULE_STATUS_TO_OUTCOME` and `HOOK_STATUS_TO_OUTCOME` → `Outcome.DEFERRED` (its "parked / yielded / resource-busy" half) each with their own reason string, so the two DEFERRED cases stay distinguishable; `hooks.py` and the manual-fire handler pass it through instead of folding it into `ok`/`success`; `engine.dispatch_action` maps it to DEGRADED, because a DONE node would tell the frontier this action's work completed when it has not begun; and `action_providers/base.py` documents the member together with the rule that adding one means updating those maps. `"skip"` would under-report a real run record; `"launched"` would claim work that has not started.
  **The ratchet, and proof it can fail.** The decision moved into one named function, `overlap.decide(policy, active=, queued=)`, exhaustive over the enum with a RAISING tail; the provider's call site additionally refuses an `OverlapAction` it has no branch for BEFORE anything is created, because the dangerous default *there* is "fall through and launch" — which is literally the bug. Four tests hold it: every member driven, the raise asserted, an AST read of `decide` asserting it names every member by attribute (a behavioural test alone would pass a fallthrough shared by two members — exactly how `QUEUE` hid), and a source check that every `OverlapAction` member has a call site. Proven to fail: temporarily adding a fourth member `WV14_RATCHET_PROBE` reds `test_every_member_has_a_branch[wv14_ratchet_probe]` and `test_the_source_branches_on_every_member_by_name` (2 failed, 5 passed). Proven to be a real fix, separately: forcing `decide`'s QUEUE branch back to `START` reds 12 of 27 tests, the regression test among them. Both probes reverted by a TARGETED edit — never `git checkout --`, which is the destructive shape WV-13's log recorded.
  **Driven on the real path.** `test_a_finishing_run_launches_the_queued_one` is create → prior finishes → queued run launches, through a real `RunController` reaching its real terminal write with a real `WorkflowWatchdog` as `services.supervisor`, on a real store/journal/flock under an isolated home. The prior's precondition is a RUNNING row with no controller, which is exactly the shape adoption sees. 27 tests in `tests/test_workflows_overlap_queue.py`.
  **DISCOVERY — `HOOK_STATUS_TO_OUTCOME` has no `launched` entry.** `hooks.py` writes `last_status = "launched"`, and `project_hook_fire` falls back to `RAN if last_run else FAILED`, so a fire-and-forget lifecycle hook is projected as "it ran and did something durable" — the T7 distinction the schedule path keeps. NOT fixed here: adding the entry changes an existing observable for a case this atom does not own, and folding it in would have hidden a real behaviour change inside a queue fix. One line, its own atom.
  **Baseline.** `inert-surface-baseline.json` regenerated with `scripts/generate_inert_surface_baseline.py`, never hand-edited: 151 → 150 total, `enum` 24 → 23, the single removed line being `enum:OverlapPolicy.QUEUE`. `docs/architecture/workflows.md`'s module table gained an `overlap.py` row, which the doc-coverage test in `test_workflows_hardening.py` requires for any new `workflows/` module. No other baseline moved.
  **DEVIATION — none on scope.** The atom brief cited `controller.py:3101` as the `run_finished` journal site; it is at `controller.py:3333` on this branch. The mechanism claim (the `_finish` neighbourhood, the single terminal writer) was correct, so this is a stale line number rather than a premise mismatch, and the hook went exactly where the brief pointed.
- [2026-08-12][WV-15] DONE — every status a fire actually writes is now a key of the table that translates it, and an AST rail makes the next unmapped one impossible to ship.
  **The finding, and it closes WV-14's own DISCOVERY.** That log recorded "`HOOK_STATUS_TO_OUTCOME` has no `launched` entry … one line, its own atom". Auditing all three tables against their real writers found it was not one line and not one table: **nine values across four writers** were live and unmapped. `hooks.py:590` writes `skipped_incident` — the incident kill switch returns BEFORE the provider is called — and `hooks.py:653` writes `launched`; both fell to `hook_to_record`'s `RAN if last_run`, so a hook that ran NOTHING was projected as "it ran and did something durable". `gateway.py:1429` (`_record_blocked_fire`) writes `status: "blocked_injection"` and `service.py:665` (`_record_suppression_row`) writes all six `INERT_OUTCOMES` members into `ScheduleRun.status`; both fell to `schedule_run_to_record`'s `FAILED`, so a **defended** injection attempt and a **quiet-hours skip** both read as genuine failures — and the suppression rows also stayed in `feed_response`'s `did` half instead of folding into `suppressed_ids`, burying the fires that mattered under the ones that never happened. Criterion 8 persists those rows precisely so a user can ask "why did my automation not run"; the projection was answering "it failed".
  **The silent fallback is the defect, not the missing key.** Both fallbacks were individually defensible — `RAN if last_run` reads "we know it ran, we merely lack the verdict", and `FAILED` follows `FireRecord.from_dict`'s "a row this build cannot classify must not count as a success". Neither says anything, which is exactly how four statuses survived two sessions (WV-14 and S155) that edited these very tables. So: `schedule_run_to_record` now WARNS on any non-empty unmapped status, and `hook_to_record` splits into three explicit cases — mapped; absent (`""`, a store row written before the field existed, the one case that still reads `RAN`, because "it ran" is the fact we have and the verdict is the part missing, and it cannot hide a new status since an executing hook always writes one of the literals); unmapped-and-loud (`FAILED` + a warning naming it). No branch swallows a value silently.
  **The mappings, each chosen against its neighbour, and what a user now sees instead of "ran".** `launched` → `DEFERRED` in the hook table, identical to `SCHEDULE_STATUS_TO_OUTCOME` and `executor.STATUS_TO_OUTCOME`: T7's whole point is that started ≠ succeeded and the background turn records its own outcome in its own run — the user reads an info-toned "deferred / outcome not yet known" instead of a green tick. `skipped_incident` → `SKIPPED_GATE`: nothing ran, nothing was spent, nothing changed, which is that member's family (quiet-hours / cooldown / condition-false) and, through `INERT_OUTCOMES`, a ledger row that folds out of the default runs inbox — **not** `REFUSED`, which `blocked` already carries (a denylist refusal is a verdict on THIS action, while an incident suspends every automated action and lifts on its own), and **not** `DEFERRED`, because deferred work still starts and this fire is dropped and never retried. `blocked_injection` → `BLOCKED_INJECTION`, the member that exists for exactly this row: the red-shield "blocked" badge `statusMeta` already renders, and it leaves `TRUE_FAILURE_OUTCOMES`, so a successful defence stops looking like a fault. The six inert members → themselves, identity **by construction** (`**{v: v for v in sorted(INERT_OUTCOMES)}`) rather than six hand-copied lines, because the writer's own guard is `outcome not in INERT_OUTCOMES: return` — that set IS the vocabulary, and a seventh member must not need a second edit to stay honest.
  **No new vocabulary member, deliberately — but `weight` moved.** All nine landed on existing `Outcome` members; WV-14 paid the full cost of adding one (three tables + `engine.dispatch_action` + the FE union) and nothing here needed it. What did change is honesty about weight: `LEDGER_WEIGHT_OUTCOMES` (deferred ∪ blocked_injection ∪ inert) replaces the hook projection's hardcoded `FULL` and the schedule projection's `DEFERRED`-only check. `FULL` means "earned a run directory and a journal", and a fire that never reached a runner has no exit code to show — the hook projection was claiming one for a hook the incident switch had stopped.
  **DISCOVERY — autopause was never affected, and that is worth recording.** `autopause.consecutive_failures_from` reads the RAW store rows (`trigger` → `outcome` → `status`, in that order) and already skips inert exit types, so the mis-projection never spent a single unit of the failure budget. This was purely a history/UI lie, and the fix moves no autopause decision. Worth stating because the obvious assumption — "`failed` is the only outcome that counts toward autopause, so a wrong `failed` autopauses healthy triggers" — is wrong here, and acting on it would have produced a much larger and unnecessary change.
  **The rail is AST-based because a regex finds only the easy half.** A `grep` over `hooks.py` finds `skipped_incident` and MISSES `launched`, whose write is `hook.last_status = result.outcome if result.outcome in ("launched", "queued") else "ok"` — a NAME whose values live in the condition's `in` tuple. Two of the four writers have that shape, `service.py` writes a variable pinned by an early-return `not in` guard, and the manual-fire handler writes a local assigned across four branches then passed as `status=status`. So `tests/test_triggers_status_vocabulary.py` infers each write's POSSIBLE VALUES: constants, both arms of a conditional expression, `str()`/`or` unwrapping, local-name resolution, and `in`/`not in` guards — **guards first**, because `status=outcome` resolves to `{""}` through its assignment and to the correct six through its guard. The boolean handling is asymmetric on purpose: `A and B` holding implies B, and `not (A or B)` implies `not B`; nothing else proves anything about one operand, so nothing else is guessed, and an unreadable comparator pins NOTHING rather than pinning a guess. Anything unresolvable FAILS rather than counting as clean, because "I could not tell" is how these four got in.
  **Vacuity, and the second direction of drift.** Per-writer `min_sites`/`min_values` floors measured against the real source (hooks 8/7, gateway-`ScheduleRun` 2/3, service 1/6, manual-fire 1/4, `_runner` 2/2), so a shape change that stops matching reds instead of reading as clean. And `test_the_writer_file_census_is_pinned` enumerates every module that assigns `.last_status` or constructs a `ScheduleRun`, because a NEW writer file is drift no per-file scan can see; `web_poll.py` is named there as writing an int HTTP status to a different entity, so its absence from the tables is a decision on the record rather than an oversight. The executor table's coverage is honestly PARTIAL and says so in its own comment: that runner is INJECTED, so an app provider or `mcp_automation._http_runner`'s HTTP body is out of static reach (`classify`'s unrecognized→FAILED rule covers the open half); the rail covers `gateway._clock_loop._runner`, the one in-repo runner, scoped by function name and asserted to EXIST so a rename reds the rail instead of silently scanning nothing.
  **Proven to fail, two ways.** (1) Adding a third member to `hooks.py:653`'s `in` tuple (`"wv15_probe"`) reds `test_every_status_a_writer_produces_is_in_its_table[hooks.py sets ScriptHook.last_status]` with "can write ['wv15_probe'], which HOOK_STATUS_TO_OUTCOME does not map" — 1 failed, 14 passed — which also demonstrates it reads the exact shape the regex missed; reverted by a TARGETED edit, never `git checkout --`, and `hooks.py` confirmed byte-identical to HEAD afterwards. (2) Re-running the scan against the PARENT commit's tables reds all four real gaps at once: hooks → `['launched', 'skipped_incident']`, gateway → `['blocked_injection']`, service → the six `skipped_*`. The manual-fire handler and `_runner` stay green in that run, which is also the audit verdict for those two.
  **The other two tables' audit, with counts.** `SCHEDULE_STATUS_TO_OUTCOME`: three writers (gateway 2 sites / 3 values; service 1 site / 6 values; manual-fire 1 site / 4 values), and it was missing 7 of its 12 keys — fixed here. `executor.STATUS_TO_OUTCOME`: its in-repo writer `gateway._clock_loop._runner` produces exactly `{error, launched}`, both already mapped — COMPLETE, no change. The manual-fire handler's `{success, failure, launched, queued}` were all already mapped too.
  **No `web/` change, verified rather than assumed.** `scheduleMeta.statusMeta` already renders `deferred`, `blocked_injection` and the whole `skipped_` family (by prefix, so `skipped_incident` also renders sensibly on the hooks list, which still exposes the raw `last_status`), and no new `Outcome` member means the FE lifecycle union is untouched. So no `web/` files changed and no web gate was owed.
  **DEVIATION — none on scope.** Two premise corrections, both cosmetic: WV-14's DISCOVERY names the function `project_hook_fire`, which is `hook_to_record` on this branch, and the brief described `hooks.py:650` as `"launched" if result.outcome == "launched" else "ok"` where WV-14 had already widened it to the two-member `in` tuple. Both were mechanism-correct, so the finding held exactly as described. `inert-surface-baseline.json` did NOT move (no enum member gained or lost a reader) — verified by running the baseline test rather than regenerating.
