...
**Status:** DECOMPOSED — the executable work now lives in [`../atomic/CRE.md`](../atomic/CRE.md) as 8 atomic plan(s).
...
- [2026-08-04][S1 amendment — follow-up, NOT a reversal] A platform-hardening comparison re-examined the `--dist worksteal` decision. **The S1 root-cause work stands** — fixing the four isolation bugs in-code rather than shipping loadscope/reruns as mitigations was correct, and those fixes are real. The narrow gap the root-cause pass did not close: `grep xdist_group tests/` returns **nothing**, and `worksteal` does not honor the mark anyway, so there is **no way to serialize a test that genuinely requires it**. Two flakes have since been recorded as PRE-EXISTING on main — `test_subagent.py`'s two SEL-audit tests (~1-in-3, full run only) and `test_knowledge_merge`/`test_refiner_fencing` (unclosed-sqlite resource exhaustion) — so the pyproject comment's "deterministic under worksteal with no reruns" is no longer accurate as written. Owned by [PLATFORM-HARDENING-FLOORS](PLATFORM-HARDENING-FLOORS.md) §6.1, which prioritizes the two **root causes** over any scheduler change: the subagent one is a **security-audit gap, not a test problem** — the SEL write in `_force_reap`/`_spawn_with_approval` sits inside a swallowing `except Exception`, so if contention can make an audit write vanish silently under test it can in production. The scheduler decision (`SH6.1`) is deliberately sequenced **last** and gated on a measured wall-time comparison; keeping `worksteal` is an acceptable outcome. Also: the pyproject comment must be corrected either way.
- [2026-08-11][CRE-8 — the suite was writing to the developer's REAL home; fixed at the seams + railed] DONE. **Measured first, on `feature-wf2lea9-polish-tier` and byte-identically on its parent (so pre-existing, owned by nobody):** `pytest tests/ -k "project or memory or knowledge or recall"` appended **+44,402 bytes** of `artifact_save`/`tool_invocation` rows to the user's real `~/.personalclaw/security_events.jsonl` (caller identities `mcp_core`, `dashboard:chat-1`) plus 46 new + 16 rewritten real-home entries; `pytest tests/test_learning_routes.py` alone **+8,961 bytes** (its accept/reject tests reach `_audit` → `sel().log_api_access`); a **full suite** touched **26 real-home families**, *net-shrank* that audit log by **151,963 bytes** — the 365-day retention prune ran against the user's live audit trail — and **deleted 123 files**. Destroying real audit history is a worse outcome than appending synthetic rows to it: appended rows look synthetic, a prune leaves nothing to notice. **Root cause, two seams not thirteen:** writers were attributed by instrumenting `open`/`os.open`/`os.mkdir`/`os.replace`/`sqlite3.connect`/`Path.write_*` and dumping a stack whenever the target was under the real home — every one of the 26 families resolved through `config.loader.config_dir()` (153 call sites) or SEL's own `sel._default_dir()`. `sel.py`'s `self._dir = base_dir or _default_dir()` is the headline: `CRE-6`'s `_reset_sel_singleton` gives every test a FRESH `SecurityEventLog`, and a fresh SEL with no home patch binds the REAL home — the reset fixed the bleed it was written for and left the write. **Fix (test-side, `conftest._isolate_real_home_writers`):** redirect those two seams to a per-test tmp dir, and ONLY when the caller expressed no preference — an explicit `$PERSONALCLAW_HOME` (*including one pointed at the real home*) and a repointed `$HOME`/`Path.home` both pass through untouched, because that is precisely what `test_cli_gateway_flags`'s `--approval yolo` rails and `test_seed`'s main-home rails do to assert a refusal; redirecting a deliberate choice would have made those rails pass for the wrong reason. Declared ABOVE `_reset_sel_singleton` so it tears down last (the singleton is cleared around every test, so the next `sel()` *constructs* one and must still see the redirect). Every module-level `from ... import config_dir` binding (58 modules) is re-pointed by identity, since patching the loader cannot reach a name already bound; the 95 function-local imports, `as _cd` aliases included, resolve from the loader at call time. **Both alternatives re-rejected, with the reasons written into the docstring:** a global `PERSONALCLAW_HOME` for pytest jobs is what the S1 amendment above deliberately REMOVED (it takes precedence over `Path.home()` inside `config_dir`, so it defeats both the env-precedence tests and the default-resolution tests), and a blanket `Path.home` patch is rejected in `_isolate_session_map`'s own docstring for breaking the real-home safety rails — and would additionally redefine the unrelated `Path.home()/".aws"`/`".ssh"` paths the artifact and task sensitivity tests assert on, making *those* vacuous.
- [2026-08-11][CRE-8 — DISCOVERY (real product bugs): a home frozen at import time is unreachable by any fixture] The rail (below) caught **147 entries still landing in `subagents/`** after the fixture was in place. `subagent_persistence` computed `_SUBAGENTS_DIR = config_dir() / "subagents"` at **module scope**, i.e. before any fixture exists — so no test-side redirect could ever reach it, and in production a `$PERSONALCLAW_HOME` established after first import was silently ignored for the life of the process. Fixed at source by resolving per call (`_subagents_dir()`), and the two tests that monkeypatched the constant were realigned to the new seam (not weakened). Swept the class: `session_map._SESSIONS_DIR` was the same shape and *worse* — its two readers decide whether a session still exists, so `prune()` was deleting session-map entries based on the contents of the developer's real `~/.personalclaw/sessions`; converted to `_sessions_dir()` (its sibling `transcript_path` already carried this exact workaround and its reason in a docstring). `schedule._DEFAULT_DIR` was dead code whose import-time `config_dir()` **mkdir'd the real home merely by importing the module** — deleted, with the now-unused import. Three instances, one class; the rail found the one a code read had missed.
- [2026-08-11][CRE-8 — the rail, and why it has teeth] Detection lives in `tests/real_home_guard.py` (importable, so `tests/test_real_home_guard.py` can drive it against a throwaway root under `tmp_path`) and is armed by `pytest_sessionstart`/`pytest_sessionfinish` in `conftest.py`. `sessionstart` records `time.time_ns()` (controller only — xdist workers share the one real home, so per-worker arming would multiply one leak into N reports); `sessionfinish` walks the real home ONCE and names every entry with a newer `mtime_ns`, catching creation, in-place modification, growth, and deletion via the surviving parent's mtime. Snapshot-then-diff was rejected: two walks of a >100k-file tree taxes every single-file `pytest` run, whereas this makes startup O(1) and only the finish path pay (measured 2.8s). **Hard failure, not a report** — `session.exitstatus = TESTS_FAILED` — because the post-fix population is **zero**, measured over a full suite, so there is nothing to grandfather; `ALLOWED_RESIDUE` is empty and exact-match only, and a prefix/glob allowance would turn the rail back into a baseline that ratifies whatever leaks next. An absent real home is reported as "nothing to compare" rather than converted into a failure (the CI case), and the detector stats without ever opening, writing or following a symlink out of the tree. The one false-positive shape — a gateway running against the real home while the suite runs — is itself a defect the failure text names, since the dev gateway is documented to run against an isolated `PERSONALCLAW_HOME`. Non-vacuity proof: append, create, byte-identical rewrite (the shape a size-only diff misses — the suite really did rewrite `skills/*/SKILL.md` identically), deletion, symlink non-follow, and a no-write-during-scan assertion.
- [2026-08-11][CRE-8 — validation] `-k "project or memory or knowledge or recall"` 44,402 → **0** bytes and 0 changed entries (2223 passed, and 50.9s → 26.1s once it stopped hammering a 27MB real log); `test_learning_routes` 8,961 → **0** (29 passed); `test_token_auth` + `test_workflows_api` 0 (163 passed); safety rails green (`test_seed` 104 tests incl. main-home refusal with and without `--seed-replace` and the symlinked-target refusal, `test_cli_gateway_flags` yolo refusals); `-k "sel or security_event or conftest or isolation"` 457 passed; `test_inert_surface_baseline` + `test_agent_reference` + `test_docs_lint_baseline` 29 passed; `make lint` green (black/isort/flake8/mypy 802 files). Full suite: rail reports the real home **unchanged**, with 0 residue confirmed independently by an out-of-band size+mtime snapshot diff, so the rail is not the only witness to its own success. **Residue: none — `ALLOWED_RESIDUE` stays empty.** Pre-existing reds unchanged and not ours: `test_harness_validate.py` 3/11 (fails in ANY worktree, passes in the main checkout) and the async workflow `Timeout (>120s)` / `Runner is closed` flakes under CPU contention. NOTE for the reader: the "loop-validation" real-home rail named in `_isolate_session_map`'s docstring has no matching test in the tree; the concrete real-home refusal rails are `test_seed` and `test_cli_gateway_flags`.
