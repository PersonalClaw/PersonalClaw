...
- **No new action providers** → `ALLOWED_HOOK_PROVIDERS` (src/personalclaw/validation.py) is **untouched** — this plan adds no hook-fireable actions. (If a later phase adds a `vault-sync` action for the automation substrate, it follows the app-delivered action rule + allowlist addition; explicitly out of v1.)
...
**Drove the whole sweep on a live gateway** (port 10241, isolated `PERSONALCLAW_HOME` +
`PERSONALCLAW_WORKSPACE` under `/private/tmp`, SPA rebuilt in-tree and confirmed as the served
