...
- [2026-08-10][PR-6 / B1.1+B1.2] DONE: Windows rung 1 (Docker Desktop) guide + per-release Windows checklist. B1.1: new `## Windows via Docker Desktop` section in `docs/guides/platforms.md` covering the four things the task names — WSL2 backend (Hyper-V explicitly untested), volume semantics, localhost ports, and `.env` on Windows paths. Every fact was read out of `deploy/compose/compose.yaml` rather than assumed: gateway `127.0.0.1:10000`, web `3000`(308→HTTPS)/`3443`, self-signed cert out of the box, named volume `personalclaw_home` at `/data`, `restart: unless-stopped` on all three services. Two real Windows gotchas documented because the compose file makes them non-obvious: (1) `env_file: ../../.env` is repo-root-relative, so copying `compose.yaml` elsewhere on its own silently drops your keys; (2) do NOT bind-mount an NTFS path over `/data` — SQLite's small random I/O + file locking across the Windows↔Linux boundary is both slow and lock-flaky, whereas the named volume lives on the WSL2 VM's ext4 disk. Also noted `personalclaw service install` does not apply on this path (container restart policy replaces it). B1.2: `### Windows smoke (rung 1 — Docker Desktop)` added to `docs/maintainers/release-runbook.md`'s "After it finishes" verification — pull the TAGGED image (not `latest`), both services healthy, then the task's four checks: dashboard over HTTPS, the 3000→3443 redirect, one real streamed chat turn (proves gateway + nginx SSE/WS + credentials), `personalclaw snapshot` inside the container (verified a real command at `cli.py:460`), and state surviving a restart. The checklist states plainly that it is the ONLY evidence behind the matrix's Windows-Docker row, and that a failure is release-blocking for that row: fix the guide or demote the row. Matrix row moved `unverified` → `supported` with `checklist:Windows via Docker Desktop`, annotated "written, not yet executed verbatim" — owner task 2 (V-B1) is what converts that into an executed run. README row updated to match. Verified both new anchors resolve and the guide's cross-links point at real headings.
