...
- **First-party product-app exemplars ([PRODUCT-EXPERIENCE-PARITY](PRODUCT-EXPERIENCE-PARITY.md) §7's suite, recorded here as each ships):** `research-lab` (tool — unattended multi-cycle research campaigns: a question becomes a sub-question tree, an hourly headless cron advances one cycle per tick and fans its worklist to subagents, findings synthesise into one markdown report; declares only `storage` + `cron`, because the app is the ledger and the engine does the researching). Staged in-tree at `scratch/research-lab/` pending the owner push, pinned by `tests/test_research_lab_app.py`, row added to `app-registry.json`. These prove the platform on real product surface rather than on teaching fixtures.
