...
- **2026-09-06 — `design-critique` (`PEP-14`, suite 3/9) — a `tool` exemplar for the ANALYSIS
  shape.** PersonalClawApps PR #77. What it exemplifies that `ET-6`'s four do not: an app whose whole
...
- **2026-09-06 — `code-review` (`PEP-12`, suite 1/9) — a `tool` exemplar for the EXTERNAL-PROCESS
  shape.** PersonalClawApps PR #75, merged `edd7336`. Deep-reviews a GitHub pull request one file at a
...
- **2026-09-06 — `research-lab` (`PEP-13`, suite 2/9) — a `tool` exemplar for the UNATTENDED-CAMPAIGN
  shape.** PersonalClawApps PR #76. A question becomes a sub-question tree that grows from findings; a
...
- **2026-09-06 — `notes` (`PEP-16`, suite 5/9) — a `tool` exemplar for the VERSIONED-USER-DATA
  shape.** PersonalClawApps PR #78. Markdown notes over a real git repo: write/read/list/search,
...
- **2026-09-06 — `issue-radar` (`PEP-17`, suite 6/9) — a `tool` exemplar for REACHING A THIRD-PARTY
  SYSTEM WITH `network: false`.** PersonalClawApps PR #80. It reads GitHub and GitLab only through the
...
- **2026-09-06 — `spec-builder` (`PEP-18`, suite 7/9) — the first exemplar of an app that is a FRONT
  OVER A CORE ENGINE rather than an implementation of a provider contract.** PersonalClawApps PR #81.
  ...
  `personalclaw.sdk.*` contract, so the app ships as `tool` and documents why rather than importing
...
- **2026-09-06 — `ops` (`PEP-19`, suite 8/9) — a `tool` exemplar for a GATED MUTATION PATH.**
  PersonalClawApps PR #83. An on-call first responder over one incident ledger, where alarms arrive as
...
- **2026-09-06 — `companion` (`PEP-20`, suite 9/9) — the first bundle where A STORED RECORD AND THE
  ...
  PersonalClawApps PR #86. `ET-6`'s four teaching bundles all persist what they serve, and
  ...
  `personalclaw.sdk.*` so the boundary job stays clean) and the honest satisfaction of an opt-in
...
- **2026-09-06 — `docs-slides` (`PEP-15`, suite 4/9) — the first bundle that made the platform GROW A
  SEAM instead of a workaround.** PersonalClawApps PR #79, with core PR #2510. `ET-6`'s four teaching
  ...
  was the promotion the import-boundary lint's own error message prescribes — `personalclaw.sdk.documents`
...
- **2026-09-03 — MINTED (`ET-9`): create the github.com/PersonalClaw/registry public repo.** Carves
...
- **2026-09-03 — MINTED (`ET-10`): provision GHCR publish credentials.** The container/registry
  publication path `ET-8`'s personalclaw.dev registry surface depends on. Owner action, marked
