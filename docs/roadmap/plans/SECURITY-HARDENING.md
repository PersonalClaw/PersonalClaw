...
**2026-09-03 — SH-11 minted: owner supplies the Apple signing + notarization secrets and the minisign release key.** `SH-3` shipped the signing scheme + verifier with the trust store deliberately EMPTY — an agent must not generate the private key it would hand over — so the owner half was implicit in `SH-3`/`SH-4`/`DC-1` prose with no atom of its own. It now has one: run `scripts/sign_app.py gen-key --signer PersonalClaw`, drop the `.pub` into the trusted-keys store, and load the four Apple Developer signing secrets into the CI `release` environment. Marked OWNER-ONLY, so the new `gated_frontier` derived key carries it. It directly unblocks the SIGNING side of `DC-1` (signed+notarized mac build) and `SH-4` (release-pipeline signing); the task also grouped `DCU-3`, whose actual blocker is a separate macOS Accessibility/TCC grant tracked in `DCU-3`'s own annotation, not these secrets.
...
**2026-09-06 — `SH-12` built the signer field OPTIONAL, because a required one would have manufactured claims.** `app-registry.schema.json` gains `signer` and `validate_registry.py` records and echoes it per listing — a `Signer` column in the PR comment, `signer_state` + `signer` in the JSON report. Required was the tempting shape and the wrong one: most community apps are not signed, so every existing listing and every third-party contributor would have had to write *something*, and the something would have been whatever `CONTRIBUTING.md`'s example suggested — converting "this author never told us" into a confident-looking answer nobody made. Optional costs one thing, and it is the thing this repo has now been bitten by three times (`network: false` rendering as "not declared", an unindexed source rendering an empty permission list, a cell reporting no token usage as zero): absence has to stay distinguishable from denial. So the verdict records FOUR states, not two — `named`, `unsigned` (a reserved value an author writes to state on the record that they do not sign), `undeclared` (the field was left out), `invalid` — and the PR comment renders three unmistakably different phrases plus a refusal. A JSON `null` is refused rather than read as omission, and `signer` joins the author-owned change identity so a PR that edits only the signer re-validates instead of being skipped as untouched. Nothing is verified and nothing is inferred: the registry cannot verify a signature (one verifies against the verifier's own trust store, and a listing carries no key), and a shallow clone of a *source repo* is not the release bundle a signature covers, so a missing signature file in the checkout is not evidence of an unsigned release — recording that guess would be the same lie one step along. The field is a claim by the listing's author, exactly like `maintainer`, and every surface says so. The four live exemplar rows and the committed valid sample were left untouched: none of them declares a signer, because none of them signs anything until `SH-11` provides the key — which is the honest reading the optional shape exists to allow.
