...
**Status:** DECOMPOSED — the executable work now lives in [`../atomic/OO.md`](../atomic/OO.md) as 7 atomic plan(s).
...
- **AGENTS.md (both repos) + CLAUDE.md pointer:** the agent-facing contributor brief — build/test/lint commands, doctrine one-pager (clean-break-within-class, provider boundary, sdk-only app imports, validation bar), the roadmap session discipline (scope/definition-of-done/deviation-ledger/escalation triggers) for roadmap work, repo layout map, "what will get your PR rejected" list. `CLAUDE.md` = one line pointing at AGENTS.md (single source).
...
- **PR template required fields (the contract every PR fills):** *What changed* · *Change class (R/B/S per the lifecycle mental model in CONTRIBUTING.md)* · *What you validated as a user* · *Docs touched*. This mirrors the AGENTS.md definition-of-done — it's how a reviewer (or the owner auditing delegated work) checks a PR at a glance.
- **AGENTS.md content contract:** build/test/lint commands, the doctrine one-pager (clean-break-within-class, provider boundary, sdk-only app imports, validation bar), the roadmap session discipline for roadmap work, the repo-layout map, and the explicit "what gets your PR rejected" list. `CLAUDE.md` = single line → AGENTS.md (one source).
...
|---|---|---|---|
| T1.1 | CONTRIBUTING update (core): add "The model" section (stated model per Design), roadmap intake path, DCO explainer; keep the doctrine + lifecycle-mental-model section current | `CONTRIBUTING.md` | model + intake + DCO sections present; no contradictions with the CONTRIBUTING breaking-changes section |
...
- **[2026-07-31][T2.3 + T3.3] DONE (code/docs side); one OWNER action remains, and it is a real API
  ...
  **Discussions is ENABLED on both repos** (owner-approved; `has_discussions: true` verified). README
  ...
  directly contributable (implement a plan task under the AGENTS.md roadmap session discipline, take a good-first-issue, or
