...
- **Action providers:** this plan adds NONE. If a future session exposes remediation as a `run-doctor` action provider (so triggers can fire it), it MUST be added to `ALLOWED_HOOK_PROVIDERS` (`src/personalclaw/validation.py`) or hook create/update rejects it — restated because §4's engine is where that provider would be born.
...
- [2026-08-24][PR2-10] **DONE — the ACP mid-turn delivery path (S6.2's remainder).** The
  ...
  **Live validation (isolated home + workspace under /private/tmp, never `~/.personalclaw`).**
...
- **Gates.** `make lint` clean (mypy 944 files). Targeted 722 passed across
  ...
  `python -m personalclaw.manifest_reference` produced zero drift.
...
- **Runtime import sweep after the deletion: 999/999 modules imported, 0 failures.** mypy cannot
  catch a stranded first-party import under `ignore_missing_imports`, so every `personalclaw.*`
