# Plan: Sibling Hardening (KiroCrew) — Enforcement Floors, Trust Seams & Gate Ergonomics

**Status:** DESIGNED — created 2026-08-04 from a code-verified comparison against
`github.com/kirodotdev/KiroCrew` (Amazon's open-sourced sibling of this product: same
Gateway/sessions/memory/skills/taskrunner/apps/SEL architecture, down to shared module names).
Every item below was verified against **both** codebases; nothing is adopted on the strength of
their README.
...
task rows were found inert, plus four self-contained developer-gate items. **Soul guardrail:**
adopt *mechanisms and their measured hazards*, never their posture. KiroCrew ships a
default-on telemetry beacon, an EC2 launcher, a gateway-federation hub, and ten i18n catalogs —
**none of those enter this plan** (§7 records why, so a future session does not mistake them for
unbuilt ideas). Where we are already ahead — provider-agnostic core, the Workflows-V2 engine,
zero tracking, the design-system ratchets — this plan changes nothing.

---

## 0. Ownership map — read this before touching any task row

**This plan is mostly a set of amendments to other plans.** Nine of the twelve items studied
...
| App-backend env inheritance | EXECUTION-ISOLATION `D1` (2026-07-29 amendment) | **re-home, unchanged** — already correctly specified (allowlist, not scrub). §2.2 only adds the two sibling spawn sites it does not cover |
...
KiroCrew hit the consequence and documented it as observed, not theoretical
(`docs/architecture/resource-protection.md`): `preexec_fn` forces CPython off
`posix_spawn`/`vfork` onto a plain `fork()` of the whole gateway and runs Python bytecode in the
child before `exec`. A lock another thread held at fork time cannot be released there, so the
child can wedge before reaching `exec` — and a wedged child takes more than itself down:
...
`EI-A1` as written. This is exactly the failure class of
`[[wired-but-wrong-controls]]`: a control that runs every time and is still category-wrong.

### 1.2 The corrected mechanism: deliver limits after `exec`

Adopt their delivery model. Limits are applied by a tiny **exec shim** — a stdlib-only module
...
**Profile split** (theirs, adopted — a single global ceiling is wrong, and this is why):
...
| `session_host` | ACP session hosts (`acp/transport.py`) | **raises** NOFILE to the inherited hard limit, nothing else. A session host multiplexes many MCP pipe pairs; the 1024 cap caused EMFILE crashes on their side. No OOM bias — a trusted host must not be the preferred kill target |
...
KiroCrew closes exactly this: every proxied request carries
`X-KiroCrew-Proxy: <ts>:<hmac>` over `<ts>:<METHOD>:<path>[?q]:<sha256(body)>` with a ±60s
window, verified **fail-closed** by the backend's middleware, secret at
...
blocker ([[cross-repo-sdk-boundary-is-not-a-blocker]]). Core lands the helper first; apps follow
in the same session.
...
renamed key, a silently dropped `_meta`, a field that stopped being written. KiroCrew generates
`config-baseline.json` from the dataclass schema registry and commits it, so any schema change
...
The higher-value half. Five memories describe the same defect class in different costumes:
[[live-reader-of-an-unwritten-key]] (a reader for a key nothing writes),
[[enum-member-nobody-writes]] (an enum member with no writer),
[[declared-kind-without-a-runtime]] (four trigger kinds shipped declared+inert),
[[defaulted-field-is-an-unsupplied-input]], [[wf2-inert-control-detection]]. In every case the
symptom is the same: **something is declared and nothing on the other side of the seam consumes
or produces it**, and the tests pass because they hand-build the state the writer was supposed to
create.

A generated, committed, **per-file counter that may only shrink** is the natural home for it. Their
`error-code-baseline.json` proves the ergonomics at scale — it carries
`missing_code: 1465` per file with the comment *"the CI ratchet and the worklist… Never raise a
number to make CI pass."* A count makes a large cleanup schedulable and makes silent backsliding
impossible.
...
gate teeth at zero is an outage ([[enforcing-a-dead-control-is-an-outage]]) — measure, commit the
real number, then ratchet.
...
KiroCrew's `python setup.py test_e2e` boots a real gateway against a packaged fake model backend,
then shells Playwright at it — no model, no credentials, no network, no cost. Their wiring
details are worth copying because each is a trap they already hit:
...
Second idea worth taking: their `KIROCREW_STRICT_ON_LOOP_PERSIST=1` promotes a
"we always call it through the helper" convention into an **enforced invariant for the duration of
the gate** — a raw call that skipped the helper raises instead of silently losing data. We have
several such conventions (on-loop persistence, atomic writes, `security.is_fenced` per
[[attributed-tag-substring-check]]).
...
| SH4.5 | Port the hourly UI-validation loop's click-path into the harness as specs, so it is reproducible in CI rather than manual ([[personalclaw-hourly-ui-validation-loop]]) | `web/e2e/` | the loop's core surfaces are asserted by the offline gate |
...
layering scheme; adopt theirs**, because it is better than what a from-scratch pass would produce:
...
`/etc/**` by failing to match. This is precisely [[wired-but-wrong-controls]] (paths compared as
strings) — a control that runs every time and is still category-wrong.
...
mark anyway — so there is **no way to serialize a test that genuinely requires it**. Two memories
record live consequences: [[personalclaw-subagent-sel-flake]] (two SEL-audit tests, ~1-in-3, only
in the full run; the memory explicitly notes this "contradicts the pyproject claim that the suite
is deterministic under worksteal") and [[personalclaw-knowledge-merge-xdist-flake]] (unclosed
sqlite handles — a resource-exhaustion flake). Both are classed PRE-EXISTING on main.

KiroCrew's `--dist loadgroup` argument is the useful framing: without group honoring, tests
needing serialization scatter across workers and produce **flaky races rather than reproducible
ordering bugs**. The unclosed sqlite handle is a real bug either way; `worksteal` is why it
presents as flake instead of a failure you can debug.
...
mechanical enforcement** — no docs job in `ci.yml`, no docs linter in `scripts/`. Given
[[plan-headers-lie-execution-logs-dont]] (25 of 66 plan `**Status:**` lines were wrong), and that
KiroCrew shows the same drift (their `governance.md` documents two matchers their own code
removed), a cheap checker is well aimed.
...
Small, verified, each belonging to an existing plan. Recorded here so the comparison is not lost;
...
| Subagent event coalescing | WORKFLOWS-V2-WORK-CONTAINERS | Above ~8 active subagents, buffer per-agent delta events (latest wins) and flush one batched WS frame per ~1s tick. `_MAX_CONCURRENT = 3` today; **needed before the cap rises** — their parent-session injection wall already bites at 8, which that plan's amendment records |
...
| `highlights` + `screenshots` manifest fields | APP-PLATFORM-EVOLUTION | We have `heroImage`/`tags`. These two most change how an app reads in the Store. Their store fetches `app.json` from the app's own repo (`git archive`, 24h cache) so a version bump needs no registry edit — we already do repo-sourced manifests; only the fields are missing |
| `effort.py`-style capability table | MODEL-USE-CASES-V2 | One source of truth for reasoning-effort levels **and which models reject the knob**. We thread `reasoning_effort_override` through `session.py:517` with no central table |
| Config-referencing-paths drift validator | ECOSYSTEM-TOOLING | Their `CODE_APPROVERS.yaml` validator fails the build when a path pattern matches no file. The generalizable half; applies to `_EDITABLE_CONFIG` and plan task tables |
...
approvals). KiroCrew ships **seven** channels to our one, and "works from my phone" is the
retention and demo story. Their WeCom/WeChat/Webex work independently validates that our
transport-provider seam is the right abstraction. **No plan edit** — this is an argument for
picking `S2-3` up sooner.
...
already have OIDC build-provenance attestation (`release.yml:218`), which is stronger than their
...
- **Telemetry beacon.** Their implementation is unusually principled (five fields, CDN configured
  not to log client IPs, per-app receipts HMAC'd so they cannot be correlated into an
  installed-app profile, auto-off in CI and non-default homes, enterprise pin-off). It is still a
  daily default-on outbound signal. **Our zero-tracking posture is the better promise — keep it
  and market it.** Their disclosure is worth reading only as a model of honest documentation.
- **Gateway-to-gateway federation / multi-instance hub** (their `instances.md`) — already a
  **permanent architectural veto** here; Companion-Apps' multi-gateway switching is the sanctioned
  alternative. Listed only so their polished implementation is not mistaken for a new idea.
- **Cloud launcher** (their `cloud.md`, an EC2/CloudFormation provisioner) — out of soul
  (local-first). Note their own honesty: the shell deny-list and in-layer chokepoints are
  "best-effort friction, not containment," because the default sandbox does not hide `~/.aws`.
  The transferable artifact is the **honest containment write-up**, which §1/§2's doc rows already
  adopt — not the feature.
...
Their `i18n-check.mjs` spawns seven scripts, keeps every byte of output, and reports twelve checks
in **one table**, with the verdict as a pure function over plain data. The reasoning generalizes:
an `&&` chain of N independent checks tells a PR about only its **first** failure — so a tree with
three unrelated problems costs three full push/wait rounds to discover, over independent
...
- **Decide the ceiling's trust root** for §5: which path holds `Ceiling`, and how it is protected
  ...
  — their answer assumes an enterprise operator we do not have).
...
- **§1 is the risky one.** The exec shim changes how every agent-influenced child starts. The
  `session_host` profile exists precisely because a uniform NOFILE cap broke ACP sessions on their
  side — validate with a real multi-MCP-server config, not a unit test.
- **§5's trust root is weaker here than there.** On a single-user machine an agent that can run
  code can often reach the ceiling file. Document what the layer does and does not buy (§1/§2's
  doc rows set the precedent) rather than overstating it — the standing rule from
  EXECUTION-ISOLATION's `D0` is that an inaccurate security claim is a live defect.
...
- [2026-08-04][plan] DISCOVERY: EXECUTION-ISOLATION `EI-A1` specifies `preexec_fn` and justifies
  ...
  loop thread. Executing that row as written would introduce the gateway-wedge hazard KiroCrew
  documents as observed. §1 rewrites the mechanism (post-`exec` shim + four profiles + a
  `preexec_fn` AST tripwire). No code was changed by this plan.
...
- [2026-08-04][plan] DISCOVERY: nine of the twelve studied items already have plan owners; this
  ...
  plans — the comparison independently re-derived both, which is corroboration, not new scope.
