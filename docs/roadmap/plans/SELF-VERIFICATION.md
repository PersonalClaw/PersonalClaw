...
- **One boundary check already exists as a test** — `tests/test_action_schema_executor_parity.py` (the `ALLOWED_HOOK_PROVIDERS` ↔ action-provider-registry parity check, `src/personalclaw/validation.py`). It is the proof-of-pattern for §1's scanner: an architectural invariant expressed as an executable check. There are ~800 tests but no *spec layer* naming which invariants exist, which tests verify them, and which checks a given diff requires.
...
| `hook-provider-parity` | any new `register_action_provider` name appears in `ALLOWED_HOOK_PROVIDERS` (`src/personalclaw/validation.py`) — promotes the existing parity *test* into a diff-time check |
...
- **No new provider TYPE and no new action provider.** The commit watcher is a cron *script* (the `run-script`/schedule-script seam, path-fenced); the QA run fires through the existing `run-workflow` action provider — already in `ALLOWED_HOOK_PROVIDERS` (`src/personalclaw/validation.py`), so **no change to that frozenset is needed**. If a later revision did add an action provider, it MUST be added there or hook create/update rejects it — restated because this plan is exactly where someone might be tempted to invent a "qa-run" provider; the answer is no, `run-workflow` + template inputs cover it.
...
## Execution log — SV-10 (Self-QA evidence bundle capture + optional fix-branch + end-to-end validation)

- [2026-09-04][SV-10] LANDED on main; atom flipped ✅. Merged via core PR #2410 (merge commit 2905bb3e8). `selfqa/evidence.py` computes each bundle file's size and SHA256 from the bytes on disk into a schema-versioned `manifest.json` and `register_bundle` composes the run's proof into ONE content-addressed Artifact through the provider registry; the contact-sheet and GIF come from ffmpeg as a local subprocess with typed degradation recorded in `degraded[]` instead of estimated digests; the `required_artifacts` gate in `workflows/engine.py` blocks completion when any declared proof is missing (Criterion #7); with `fix_branch_enabled`, `selfqa/fix_branch.py` yields a never-merged `pclaw/selfqa-<sha8>` branch linked in the Task (Criterion #8). WHY: both done_when criteria map to shipped, deterministically-enforced code — the digest/manifest half moved out of LLM instructions into `evidence.py`, and the completion gate makes missing proof a block rather than an instruction. `SV-11` is the last open SV atom.

## Execution log — SV-11 (Retire the interim commit-watcher cron script onto the AUTO-R12 vcs trigger)

- [2026-09-04][SV-11] LANDED on main; atom flipped ✅ and the plan is COMPLETE — all 11 atoms done. Merged via core PR #2440 (merge commit aa9a2cb81). The bundled `self-qa` template rebinds its trigger to AUTOMATION-SUBSTRATE's vcs preset (file kind watching `.git/refs/heads/*`), the interim `selfqa/scripts/selfqa_commit_watch.py` cron script is deleted, and a rule spec asserts the script stays absent now that the vcs trigger kind exists — the retirement §6 always promised, executed rather than left as a standing IOU. Two design rails pass honestly rather than by baseline bump: the `selfqa` state home is declared as a first-class StateEntry in the state inventory, and the action-provider chokepoints suite carries a documented-exemption test proving the provider earns its exemption properties. WHY: all three done_when clauses map to shipped code — the rebind, the deletion, and the absence rail.
