...
- **No new action providers ship in this plan.** Downstream actions are the substrate's business. IF a connector pack ever ships one (e.g. a `post-back` action), it follows the `apps/webhook-action` precedent — `type: "action"` app **AND its name added to `ALLOWED_HOOK_PROVIDERS` (src/personalclaw/validation.py)** or hook/trigger create rejects it — stated here so no pack author trips on it.
