...
- [2026-08-25][PEP-5] DONE: V3 validation is the API suite, not a manual gateway drive:
  ...
  fixture `~/.claude` bound by `CLAUDE_CONFIG_DIR` and a fixture home bound by `PERSONALCLAW_HOME`, with
...
- [2026-08-25][PEP-9] DISCOVERY: **`--bundle` alone does not make the OUTPUT network-free.**
  ...
  DISCOVERED (`$PERSONALCLAW_ARTIFACT_BUILD_ROOT`, else the source checkout, else
...
- **[2026-09-06] 🟡 IMPLEMENTATION LANDED for `PEP-15` (Docs/Slides, suite 4/9) across two branches; the atom
  stays ⬜ for the owner's `dag.json` flip.** The bundle is **PersonalClawApps `feature-pep15-docs-slides`**
  (`docs-slides/`: `app.json`, `provider.py`, `app_cli.py`, `test_provider.py` (39 tests), README, LICENSE —
  scaffold-generated by `personalclaw app new docs-slides --type tool`, then filled in), and the seam it rides
  is core `feature-pep15-docs-slides` (`src/personalclaw/sdk/documents.py` + `tests/test_sdk_documents_seam.py`).
  **Core merges first** — without the facade `provider.py` cannot import. Three tools: `deck_from_brief`
  (markdown → a real `.pptx`), `document_from_brief` (markdown → a compiled `.docx`/`.pdf`), and
  `docs_slides_formats` (what this build can actually render). §7.3's "reuse, don't rebuild" is the whole
  design: the bundle vendors no file-format library and contains no OOXML vocabulary, because Knowledge reads
  `.pptx`/`.docx` back through core's own parsers and a second renderer in an app would drift from the one that
  reads it. **The seam had to be promoted, not reached around** — `personalclaw.documents` was core-internal, so
  the only alternative was vendoring `python-pptx`; the promotion is what the app import-boundary lint's own
  error message prescribes. `register_writer` is deliberately left OFF the facade: an app-registered format
  would be visible to every other caller of the shared registry while that app is enabled and gone when it is
  disabled, so `available_formats()` would stop being a property of the build. **Deck and document are separate
  tools, not one with a `format` switch** — in a deck a `## ` heading is a slide boundary, in a document a
  section heading in one flow, so a single switch would silently change what the author's headings mean;
  `document_from_brief` refuses `pptx` outright. Output containment is an ABSENCE: no tool takes a path/dir
  argument, so the widest write is the `storage` grant the manifest already holds (`network: false`), and a
  caller-supplied `filename` is reduced to a bare stem (`Path(...).name`, then leading dots, then a character
  filter, then 80 chars) — tested from both ends, that the written path's parent IS the app's out dir and that
  the traversal target does not exist. **"Openable" is measured, not inferred**: the `.pptx` is read back with
  core's `pptx_parser` and the `.docx` with its `docx_parser`, because a byte-count assertion passes on 30 KB of
  unopenable zip. Gates: apps-repo `tests` (39 green under the CI posture), `manifest-validate` (57 manifests),
  `boundary`, `quality-declarations` (57 apps), `settings-schema-posture` (49 schemas), `prompt-cache-posture`,
  `live-writes-posture` — the last two type-scoped to model/channel apps and therefore vacuously clean for a
  `tool` app; core `tests/test_sdk_documents_seam.py` (7), the SDK surface + import-cycle rails, and the
  inert-surface ratchet green **without a baseline edit** (the promoted names got an in-repo reader rather than a
  blessed higher count). No `HTTP_ERROR_CODES` entry, no config key, no durable write, no capability-table id, no
  hook provider, no tool-schema or manifest change — so no offline-reference regeneration. **The local-Store leg
  IS validated headless**, on the exact Store path `ET-6` used: `apps.source.resolve` →
  `apps.app_manager.install(origin="local")` in an isolated `PERSONALCLAW_HOME`, then
  `providers.loader.load_all_extensions()` registers `DocsSlidesProvider` and all three tools appear in
  `tool_providers.registry.list_all_tools()`; invoked through the registered provider, `docs_slides_formats`
  reported `decks: pptx / documents: docx, pdf` and the deck + document written into the app's own data dir read
  back through core's parsers with the right slide titles and headings. **Clauses NOT met:** nobody has driven it
  in a BROWSER (no chat-surface call, no look at how the manifest renders in Settings → Tools), and it is not
  listed in the Store — a registry listing needs a published repo the validator can reach, which is an owner
  push, the same one `ET-9` is blocked on. A rendered `.pdf` has a valid `%PDF-` header and `%%EOF` trailer but
  has not been opened in a reader; only the `.docx` half is parser-round-tripped. The ECOSYSTEM-TOOLING
