...
Plug-in fidelity (non-negotiable): registered via `register_action_provider` in core (it is store-adjacent plumbing, like `create-task`) **and added to `ALLOWED_HOOK_PROVIDERS` (`src/personalclaw/validation.py`)** — without that, trigger create/update rejects it even though the UI offers it. `supports_dry_run = True` (a dry run renders the transform and reports the would-be diff without writing — consistent with the T9 dispatcher rule). It does NOT support blocking (it is a sink, not a gate).
...
- **`artifact-update` action provider** (§2.2): implements `ActionProvider`, registered via `action_providers/registry.py:register_action_provider` among the core-native set, **AND added to `ALLOWED_HOOK_PROVIDERS` (`src/personalclaw/validation.py`)** — the create/update validation allowlist (skipping this is the known rejection bug-class). `supports_dry_run=True`; settings schema follows the action-provider conventions (enums over free text, absolute paths).
...
- **`src/personalclaw/dashboard/tile_refresh.py`** (new) — `render_skeleton()` is a thin binding of
...
- [2026-08-26][AS-7] **PARTIAL — stays `todo`.** The `menu-bar-companion` bundle shipped in
  `PersonalClawApps#53` (rebase-merged, `754030cc`): 20 files, 49 tests green, a
...
- [2026-08-26][AS-7] ✅ **ATOM FLIPPED `done` (PersonalClawApps#54, rebase-merged `f3963408`).**
...
- [2026-08-26][AS-6] 🔴 **DEVIATION / process incident — a test booted the REAL gateway against the
  ...
  real gateway: it regenerated `~/.personalclaw/agents/personalclaw.json` and appended one
  `tool_policy.no_session_key` row to `~/.personalclaw/security_events.jsonl` before the 120 s pytest
...
- [2026-08-26][AS-6] **UNMET — the L2 producer does not exist.** The layer model, the ceiling, the
  ...
  `~/.personalclaw/surfaces/` loader. Building one is an agent-writable-code surface that wants its own
...
- [2026-08-26][AS-6] ✅ **REMAINDER CLOSED — all six `done_when` clauses now hold.** The two `UNMET`
  ...
  evidence the tick should act on. What landed: `personalclaw/surface_overlay.py` (the L2 producer — the
...
- [2026-08-26][AS-6] **DESIGN — no write endpoint, and refusals ride a 200.** An overlay is authored
  with the ordinary file tools under `$PERSONALCLAW_HOME/surfaces/`; an HTTP writer would be a second
