...
~~**`web_watch`'s headless tier is NEWLY STARTABLE**~~ **CORRECTED 2026-08-25: it is BUILT and `WF2AUT-7` is `done`** (see the note at the end of this file) — `src/personalclaw/web/render.py` ships an
...
- [2026-08-10][WF2AUT-11] PARTIAL — half 1 (idle runtime) SHIPPED; half 2 (autonudge deletion) BLOCKED. Atom stays in_progress.
  ...
  **DISCOVERY (my brief was wrong):** I told the implementer to run `mypy src tests`; that cannot pass in this repo (duplicate module names — `tests/` has no `__init__.py`). The repo's real target is `make lint` → `mypy src/personalclaw harness`.
...
- [2026-08-18][ad-hoc fix, no atom] DISCOVERY + DONE — crit 8's suppression writer wrote into the WRONG PersonalClaw home, and so did the rate meter that reads it back.
  ...
  **The rail is positive AND negative, because either alone is vacuous.** A new `homes` fixture points the REAL `config_dir()` at a **decoy** home via `PERSONALCLAW_HOME` (env var, not a monkeypatched function, so the resolution path under test is the one an ad-hoc script takes) plus `PERSONALCLAW_WORKSPACE`, and asserts `config_dir() == decoy` so the split cannot silently collapse. `test_a_SUPPRESSED_row_lands_in_the_TICKS_home_not_the_AMBIENT_one` asserts the 3 rows ARE under `base_dir` (fails if a fix writes nowhere) and that `decoy/cron-history` **does not exist** (fails if they go to the ambient home). `test_the_RATE_METER_reads_the_TICKS_ledger_not_the_AMBIENT_one` seeds the decoy with 5 rows and the tick's home with 2, asserts the decoy really reads 5 as a positive control, then asserts the meter returns 2 — without that control, reading `2` could equally mean the seed failed.
...
- **[2026-08-27] OWNER RULINGS — `WF2AUT-11` and `WF2AUT-12`.**
  ...
  *against* completion, not for it). The deletion stays here and becomes trivially true once the port lands.
  `WF2AUT-12` (E4): **the webhook fire path rides the landed `src/personalclaw/inbound/` substrate.** It already ships
...
- **[2026-09-03][`WF2AUT-12`] FLIP → `done` (merged PR #2345).** The webhook fire endpoint landed:
  ...
  riding the landed `src/personalclaw/inbound/` substrate per the 2026-08-27 E4 ruling above. `dag.json`
...
- **[2026-09-06][`WF2AUT-11`] FLIP → `done` (merged PR #2504).** Both halves, and the second one only
  ...
  `src/personalclaw/autonudge.py` is deleted with **no shim**, all 10 importers repointed, and
