...
**Status:** DECOMPOSED — the executable work now lives in [`../atomic/WF2LOO.md`](../atomic/WF2LOO.md) as 18 atomic plan(s).
...
| Any new action provider (none currently planned; R15 uses tools, not hooks) | Would additionally require `ALLOWED_HOOK_PROVIDERS` (src/personalclaw/validation.py) |
...
tool-schema surface changed, so `python -m personalclaw.manifest_reference` regenerated to no diff.
