...
| S111 | **The last read-only callers re-point** — four surfaces still described the legacy file: a user with live automations read as NOT engaged with automation, a cron's `session='origin'` reply went NOWHERE, suggestions lost their scheduled context, and the whole investigate snapshot came back blank. Driving my own re-point found 3 more (legacy-only attributes, the `health_status` rename, a nested provider key) | AUTOMATION-SUBSTRATE §6 | ✅ DONE (#332) |
| S112 | **Delete the `ScheduleService` class** — core side is READY (779 lines, only 2 lifecycle calls left, and the `state.py` legacy fold-in is provably dead since S110). BLOCKED on a cross-repo dependency: `sdk/channel.py` re-exports the class and `PersonalClawApps/slack-channel` uses it in 12 places, incl. a user-facing `/cron` CRUD surface. Owner must decide whether that app re-points, retires the surface, or keeps a shim | AUTOMATION-SUBSTRATE §6 | ⛔ BLOCKED (E5) |
