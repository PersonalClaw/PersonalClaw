#!/usr/bin/env python3
"""PR validation for the PersonalClaw community app registry (``registry.json``).

CI runs this on every pull request that touches ``registry.json``. It answers one
...
# ── The schema (the authority; registry.schema.json is its published mirror) ──
...
class RegistryResult:
    """The verdict on a whole ``registry.json`` (or on the rows a PR changed)."""
...
def check_row_schema(row: object, *, allow_file_repos: bool) -> list[Reason]:
    """Structural validation. This is the authority ``registry.schema.json``
...
def build_schema() -> dict[str, Any]:
    """The published ``registry.schema.json``, generated from the constants above.
    ...
    Regenerate with ``python validate_registry.py --emit-schema > registry.schema.json``
    (``test_registry_validation.py`` reds if the committed file has drifted).
    """
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "https://raw.githubusercontent.com/personalclaw/registry/main/registry.schema.json",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "registry", type=Path, nargs="?", default=None, help="path to registry.json"
    )
    parser.add_argument(
        "--emit-schema",
        action="store_true",
        help="print registry.schema.json (generated from this module) and exit",
    )
    parser.add_argument(
        "--base",
        type=Path,
        default=None,
        help="the base branch's registry.json; only added/changed rows are validated",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.registry is None:
        parser.error("a registry.json path is required (or pass --emit-schema)")
