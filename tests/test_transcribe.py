...
class TestIsAvailable:
    ...
    async def test_disabled(self):
        with patch("personalclaw.providers.use_cases.load_use_case_settings", return_value={"enabled": False}):
            assert await is_available() is False

    @pytest.mark.asyncio
    async def test_enabled_no_active_model(self):
        with patch("personalclaw.providers.use_cases.load_use_case_settings", return_value={"enabled": True}), \
...
class TestTranscribeAudio:
    ...
    async def test_successful_transcription(self):
        ...
        with patch("personalclaw.providers.use_cases.load_use_case_settings",
    ...
    async def test_large_file_segments_and_stitches(self, tmp_path, monkeypatch):
        ...
        with patch("personalclaw.providers.use_cases.load_use_case_settings", return_value={"enabled": True}), \
             ...
             patch("personalclaw.stt.registry.active_stt", return_value=(prov, "turbo")), \
             patch("personalclaw.transcribe._ffmpeg_present", return_value=True), \
             patch("personalclaw.transcribe._transcribe_segmented", side_effect=_fake_transcribe_segmented):
            result = await transcribe_audio(str(f))
