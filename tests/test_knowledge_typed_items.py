...
class TestHandlers:
    def test_create_item_endpoint(self, store):
        ...
        resp = _run(H.create_item(_req(store, "POST", {"type": "note", "title": "N", "content": "hi", "tags": ["t"]})))
    ...
    def test_create_bookmark_rejects_non_http_scheme(self, store):
        ...
        for bad in ("javascript:alert(1)", "data:text/html,<script>x</script>", "file:///etc/passwd", "not a url"):
        ...
        ok = _run(H.create_item(_req(store, "POST", {"type": "bookmark", "url": "https://example.com"})))
    ...
    def test_update_rejects_non_http_url(self, store):
        ...
        store.db.commit()
        resp = _run(H.update_item(_req(store, "PATCH", {"url": "javascript:alert(1)"}, match_info={"id": iid})))
        assert resp.status == 400
        ...
        ok = _run(H.update_item(_req(store, "PATCH", {"url": "https://example.org"}, match_info={"id": iid})))
    ...
    def test_bookmark_dedup_returns_existing(self, store):
        ...
        r1 = _run(H.create_item(_req(store, "POST", {"type": "bookmark", "url": "https://example.com/x"})))
        assert r1.status == 201
        first_id = json.loads(r1.body)["id"]
        r2 = _run(H.create_item(_req(store, "POST", {"type": "bookmark", "url": "https://example.com/x"})))
        assert r2.status == 200
        assert json.loads(r2.body)["id"] == first_id  # same item, no dup
        # A different URL still creates a new item.
        r3 = _run(H.create_item(_req(store, "POST", {"type": "bookmark", "url": "https://example.com/y"})))
        assert r3.status == 201 and json.loads(r3.body)["id"] != first_id

    def test_create_rejects_unknown_type(self, store):
        from personalclaw.dashboard.handlers import knowledge as H

        resp = _run(H.create_item(_req(store, "POST", {"type": "nonsense", "title": "X", "content": "y"})))
        assert resp.status == 400
    ...
    def test_create_rejects_media_type_via_json(self, store):
        ...
        ok = _run(H.create_item(_req(store, "POST", {"type": "note", "title": "n", "content": "c"})))
        assert ok.status == 201
    ...
    def test_journal_blank_title_defaults_to_date(self, store):
        ...
        resp = _run(H.create_item(_req(store, "POST", {
            "type": "journal",
            "content": "A long first journal entry about the day's work and reflections.",
        })))
        assert resp.status == 201
        body = json.loads(resp.body)
        assert body["title"] == datetime.now().strftime("%B %-d, %Y")
        # A non-journal with blank title still falls back to the content slug.
        resp2 = _run(H.create_item(_req(store, "POST", {
            "type": "note", "content": "Note body that becomes the seeded title prefix here.",
        })))
        assert json.loads(resp2.body)["title"] == "Note body that becomes the seeded title prefix here."[:60].strip()

    def test_journal_editable_same_day(self, store):
        from personalclaw.dashboard.handlers import knowledge as H

        jid = store.create_typed_item(item_type="journal", title="J", content="today's entry")
        resp = _run(H.update_item(_req(store, "PATCH", {"content": "edited same day"}, {"id": jid})))
    ...
    def test_journal_immutable_after_creation_day(self, store):
        ...
        store.db.commit()
        resp = _run(H.update_item(_req(store, "PATCH", {"content": "rewrite history"}, {"id": jid})))
    ...
    def test_cannot_change_text_item_to_media_type(self, store):
        ...
        bad = _run(H.update_item(_req(store, "PATCH", {"type": "image", "item_type": "image"}, {"id": nid})))
    ...
    def test_content_edit_reenriches(self, store):
        ...
        enq: list[str] = []
        resp = _run(H.update_item(_req(store, "PATCH", {"content": "new body"}, {"id": nid}, enqueued=enq)))
        ...
        enq2: list[str] = []
        resp2 = _run(H.update_item(_req(store, "PATCH", {"tags": ["x"]}, {"id": nid}, enqueued=enq2)))
    ...
    def test_content_edit_reingest_false_skips_enrich(self, store):
        ...
        nid = store.create_typed_item(item_type="note", title="N", content="old body")
        enq: list[str] = []
        resp = _run(H.update_item(_req(store, "PATCH", {"content": "fixed typo", "reingest": False}, {"id": nid}, enqueued=enq)))
    ...
    def test_regenerate_missing_only(self, store):
        ...
        store.update_item(b, insights={"summary": "done"}); store.db.commit()
    ...
    def _list(self, store, query=""):
        ...
        app = _app(store)
        req = make_mocked_request("GET", "/api/knowledge/items" + (f"?{query}" if query else ""), app=app)
    ...
    def test_list_filters_by_provider(self, store):
        ...
        store.create_typed_item(item_type="note", title="native one", content="a", provider="native")
        store.create_typed_item(item_type="note", title="from notion", content="b", provider="notion")
        store.db.commit()
    ...
    def test_list_entities_parses_aliases_and_redacts_description(self, store):
        ...
        store.add_entity(
            "DeployBot", "service", aliases=["db", "deployer"],
    ...
    def test_get_item_embedded_entities_parse_and_redact(self, store):
        ...
        eid = store.add_entity("Vault", "service", aliases=["kv"],
...
class TestKnowledgeTools:
    def test_create_search_get_round_trip(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            r = _run(prov.invoke("knowledge_create", {"type": "note", "title": "Recipe", "content": "boil water", "tags": ["food"]}))
    ...
    def test_create_gist_carries_language(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            r = _run(prov.invoke("knowledge_create", {"type": "gist", "title": "snippet",
                                                       "content": "x=1", "gist_language": "python"}))
            assert r.success
    ...
    def test_create_untitled_journal_gets_date_title(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            r = _run(prov.invoke("knowledge_create", {"type": "journal", "content": "Daily reflection."}))
            assert r.success
            iid = r.output.rsplit(" ", 1)[-1]
    ...
    def test_update_enforces_journal_immutability(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            store = K.get_knowledge_store()
            jid = store.create_typed_item(item_type="journal", title="June 1, 2026", content="old entry")
            store.db.execute("UPDATE items SET created_at=? WHERE id=?", ("2026-06-01T09:00:00", jid))
            store.db.commit()
    ...
    def test_get_surfaces_summary_and_insights(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            store = K.get_knowledge_store()
            iid = store.create_typed_item(item_type="note", title="Doc", content="long body text here")
            store.update_item(iid, summary="A crisp summary.",
                              insights={"summary": "A crisp summary.", "key_points": ["kp one", "kp two"],
    ...
    def test_get_surfaces_file_shape(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            store = K.get_knowledge_store()
            iid = store.create_typed_item(item_type="image", title="photo", content="a sunset",
                extra={"file_path": "/x/y.png", "mime_type": "image/png", "file_size": 2048,
    ...
    def test_get_signals_pending_enrichment(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            store = K.get_knowledge_store()
            iid = store.create_typed_item(item_type="note", title="WIP", content="body",
                                          extra={"processing_status": "processing"})
            store.db.commit()
            g = _run(prov.invoke("knowledge_get", {"id": iid}))
            assert g.success and "still enriching" in g.output
            # A done item shows no such notice.
            store.update_item(iid, processing_status="done"); store.db.commit()
    ...
    def test_update_round_trip(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            r = _run(prov.invoke("knowledge_create", {"type": "note", "title": "Old", "content": "old body"}))
            item_id = r.output.rsplit(" ", 1)[-1]
            u = _run(prov.invoke("knowledge_update", {"id": item_id, "title": "New", "tags": ["x"]}))
    ...
    def test_update_requires_a_field(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            r = _run(prov.invoke("knowledge_create", {"type": "note", "title": "T", "content": "c"}))
            item_id = r.output.rsplit(" ", 1)[-1]
    ...
    def test_stats_overview(self, tmp_path):
        ...
        with patch.object(K, "_store", None), \
             patch("personalclaw.knowledge.knowledge_db_path", lambda: os.path.join(tmp_path, "k.db")):
            _run(prov.invoke("knowledge_create", {"type": "note", "title": "A", "content": "x", "tags": ["t1", "shared"]}))
            _run(prov.invoke("knowledge_create", {"type": "gist", "title": "B", "content": "y", "tags": ["shared"]}))
...
class TestGenerateIntelligence:
    def test_endpoint_reenqueues_full_pipeline(self, store):
        ...
        item_id = store.create_typed_item(item_type="note", title="N", content="boil water in a kettle")
        enq: list[str] = []
...
class TestMediaClassify:
    ...
    def test_classify_routes_source_code_to_gist(self):
        ...
        assert media.classify("readme.md") == "document" and media.code_language("readme.md") is None
...
class TestMediaItems:
    ...
    def test_store_file_item_webm_audio_classifies_and_mimes_as_audio(self, store, tmp_path, monkeypatch):
    ...
    def test_store_file_item_dedups_identical_content(self, store, tmp_path, monkeypatch):
        ...
        a = tmp_path / "a.txt"; a.write_bytes(data)
        item1, new1 = H._store_file_item(store, str(a), "a.txt")
        assert new1
        b = tmp_path / "b.txt"; b.write_bytes(data)  # same content, different name
    ...
    def test_store_file_item_source_code_becomes_gist_with_language(self, store, tmp_path, monkeypatch):
    ...
    def test_serve_path_returns_in_root_file(self, store, tmp_path, monkeypatch):
        ...
        iid = store.create_typed_item(item_type="image", title="x",
                                      extra={"file_path": str(good), "mime_type": "image/png"})
        path, mime = H._serve_item_path(store, iid, thumbnail=False)
    ...
    def test_delete_removes_media_files(self, store, tmp_path, monkeypatch):
        ...
        d = self._files_dir(tmp_path, monkeypatch)
        f = d / "del.png"; f.write_bytes(b"data")
        t = d / "del.thumb.webp"; t.write_bytes(b"thumb")
        iid = store.create_typed_item(item_type="image", title="x",
                                      extra={"file_path": str(f), "thumbnail_path": str(t)})
    ...
    def test_delete_removes_derived_media_artifacts(self, store, tmp_path, monkeypatch):
        ...
        d = self._files_dir(tmp_path, monkeypatch)
        src = d / "abc123-source.mp4"; src.write_bytes(b"movie")
        ...
        derived = [d / f"{iid}.audio.wav", d / f"{iid}.frame_001.jpg",
                   d / f"{iid}.frame_002.jpg", d / f"{iid}.dense_001.jpg"]
        for p in derived:
            p.write_bytes(b"x")
        # a sibling item's artifacts (different id) must NOT be swept
        other = d / "0000ffff-other.frame_001.jpg"; other.write_bytes(b"keep")
        req = make_mocked_request("DELETE", "/", app=_app(store), match_info={"id": iid})
        resp = _run(H.delete_item(req))
        assert resp.status == 200
        assert not src.exists()
    ...
    def test_delete_spares_file_outside_files_dir(self, store, tmp_path, monkeypatch):
        ...
        self._files_dir(tmp_path, monkeypatch)
        outside = tmp_path / "important.txt"; outside.write_bytes(b"do not delete")
        iid = store.create_typed_item(item_type="image", title="x", extra={"file_path": str(outside)})
        req = make_mocked_request("DELETE", "/", app=_app(store), match_info={"id": iid})
        resp = _run(H.delete_item(req))
        assert resp.status == 200
...
class TestSkillSynthesisParsing:
    ...
    def test_bold_headers_and_no_header_fallback(self):
        from personalclaw.dashboard.handlers.knowledge import _parse_skill_sections
        assert _parse_skill_sections("**DESCRIPTION:** D\n**PROCEDURE:**\nstep")["description"] == "D"
...
class TestArchivedHiddenFromRetrieval:
    ...
    def test_related_items_excludes_archived(self, store):
        ...
        store.add_mention(b, eid); store.db.commit()
        ...
        store.update_item(b, is_archived=1); store.db.commit()
    ...
    def test_entity_items_excludes_archived(self, store):
        ...
        store.update_item(iid, is_archived=1); store.db.commit()
        items = json.loads(_run(H.get_entity_items(_req(store, "GET", match_info={"name": "Zeta"}))).body)
    ...
    def test_entity_items_is_mentions_based_not_text_match(self, store):
        ...
        a = store.create_typed_item(item_type="note", title="Storage", content="we use ddb for the table")
        self._link(store, a, "DynamoDB")
        # Item B contains the word 'DynamoDB' in text but is NOT linked in the graph.
        store.create_typed_item(item_type="note", title="Aside", content="unrelated note that name-drops DynamoDB once")
        store.db.commit()
        items = json.loads(_run(H.get_entity_items(_req(store, "GET", match_info={"name": "DynamoDB"}))).body)
        ...
        assert json.loads(_run(H.get_entity_items(_req(store, "GET", match_info={"name": "Nonexistent"}))).body) == []
...
class TestStaleEmbeddingCount:
    ...
    def _embed(self, store, item_id, dim):
        import struct
        store.db.execute("UPDATE items SET embedding = ? WHERE id = ?",
