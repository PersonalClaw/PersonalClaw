...
def _approved(kind, artifact):
    return PlanStep(id=f"step-{kind}", kind=kind, title=kind,
                    objective="", status=StepStatus.APPROVED.value, artifact=artifact)
...
class TestGoalWalkthrough:
    ...
    def test_project_to_spec_into_unified_plan(self):
        kinds.ensure_loaded()
        loop = store.create(Loop(id="", name="", kind="goal", task="investigate the latency regression"))
        wt = kinds.get("goal").walkthrough()
        session = PlanSession(project_id=loop.id, steps=[
            _approved("intent", {"markdown": "Find the root cause.\nMore.", "success_criteria": "RCA documented"}),
            _approved("sub_goals", {"sub_goals": ["profile the hot path", "check the cache"]}),
            _approved("quorum", {"roster": [{"role": "investigator"}, {"role": "fixer"}]}),
            _approved("execution_plan", {"execution_plan": [{"role": "investigator", "target": "profile"}]}),
        ])
        ...
        assert spec["kind_config"]["execution_plan"] == [{"role": "investigator", "target": "profile"}]
    ...
    def test_on_finalize_decomposes_sub_goals_into_linked_tasks(self):
        import asyncio
        kinds.ensure_loaded()
        loop = store.create(Loop(id="", name="G", kind="goal",
...
class TestCodeWalkthrough:
    ...
    def test_project_decomposition_into_plan(self):
        kinds.ensure_loaded()
        wt = kinds.get("code").walkthrough()
        session = PlanSession(project_id="x", steps=[
            _approved("problem_framing", {"markdown": "Add OAuth.\nDetail."}),
            _approved("decomposition", {"phases": [
                {"stage": "implementation", "title": "build", "objective": "wire oauth",
                 "exit_criteria": ["tests pass"], "tasks": [{"title": "add provider"}]},
            ]}),
        ])
        spec = wt.project_to_spec(session)
...
class TestDesignWalkthrough:
    ...
    def test_project_build_plan_into_plan(self):
        kinds.ensure_loaded()
        loop = store.create(Loop(id="", name="", kind="design",
                                 task="build a warm design system for a recipe app"))
        wt = kinds.get("design").walkthrough()
        session = PlanSession(project_id=loop.id, steps=[
            _approved("brief", {"markdown": "Warm, accessible recipe-app system.\nMore."}),
            _approved("build_plan", {"phases": [
                {"step": "foundations", "title": "Foundations", "objective": "warm anchors"},
                {"step": "export", "title": "Document & export", "objective": "DESIGN.md"},
            ]}),
        ])
        spec = wt.project_to_spec(session)
    ...
    def test_finalize_merges_approved_token_overrides_into_kind_config(self):
        ...
        wt = kinds.get("design").walkthrough()
        session = PlanSession(project_id=loop.id, steps=[
            _approved("palette", {"token_overrides": {"color": {"primitive": {"brand": {"500": "#d65f2e"}}}}}),
            _approved("typography", {"token_overrides": {"typography": {"family": {"sans": "Inter, sans-serif"}}}}),
...
class TestStepPassRetry:
    ...
    def _seed(self, kind="design"):
        ...
        session = PlanSession(project_id=loop.id, steps=[
            PlanStep(id="step-0", kind="brief", title="Brief", objective="o",
                     status=StepStatus.PENDING.value, artifact=None),
        ])
    ...
    def test_retries_once_then_succeeds(self, monkeypatch):
        ...
        monkeypatch.setattr(type(kinds.get("design").walkthrough()), "parse_artifact_sentinel",
