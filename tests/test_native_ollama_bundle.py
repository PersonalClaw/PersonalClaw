...
def _evict_bundle_module() -> None:
    """Drop the bundle's cached module so the next enable re-runs its registration.

    ``ProviderRegistry.register`` imports the bundle's ``provider.py`` under a
    namespaced name and the import system caches it in ``sys.modules``. A gateway
    boot earlier in the same process (``tests/test_gateway_boot_provider_sync.py``)
    leaves that cache warm, so a later ``register`` reuses the cached module and
    never calls ``register_type`` again — the LLM registry, freshly reset, then
    reports no ``ollama`` type. Evicting the module is what makes the registration
    observable again; it is safe because the provider handles
    ``ProviderResolutionError`` on reload.
    """
...
def registered_bundle():
    ...
    The bundle module is evicted in SETUP as well as teardown. A gateway boot earlier in
    the same process (``tests/test_gateway_boot_provider_sync.py``) caches the module in
    ``sys.modules``; without the setup eviction the reset registry below stays empty and
    ``register`` reuses the cached module, so the type is never re-registered.
    """
...
def test_a_warm_module_cache_does_not_hide_the_registration():
    """Regression: a cached bundle module must not make the type look unregistered.

    Reproduces the cross-file failure deterministically in one file: cache the bundle
    module the way a gateway boot does, reset the LLM registry, then enable the bundle
    through the real dispatch path. Before the setup eviction this raised
    ...
    reset_default_registry()
    try:
        # Warm the cache exactly as a prior gateway boot leaves it.
        prov_reg.get_provider_registry().register(_manifest(), enabled=True)
        assert namespaced_module_name(APP_NAME, "provider") in sys.modules

        # A fresh registry with the module still cached: the setup eviction is what
        # lets the bundle's own file register the type again.
        reset_default_registry()
        _evict_bundle_module()
        prov_reg.get_provider_registry().register(_manifest(), enabled=True)
