...
#: Every tool either half shells out to, plus `node` — which the frontend half does not
#: call directly but does require: the script refuses to enter the render-smoke chain
#: unless BOTH `npm` and `node` resolve, so a sandbox that stubs only `npm` takes the
#: "toolchain not found" skip and every leg expecting the chain to fire measures nothing.
#: Stubbed so entering a half is cheap and observable. `git` is deliberately NOT here —
#: the script's own git calls are the thing being measured.
STUBBED_TOOLS = ("npm", "npx", "node", "black", "isort", "flake8")
