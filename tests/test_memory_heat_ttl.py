...
def test_heat_rises_with_visits():
    cold = MemoryRecord(id="a", kind=MemoryKind.SEMANTIC, recall_count=0, visit_count=0,
                        updated_at=datetime.now(tz=timezone.utc).isoformat())
    warm = MemoryRecord(id="b", kind=MemoryKind.SEMANTIC, recall_count=10, visit_count=5,
                        updated_at=datetime.now(tz=timezone.utc).isoformat())
...
def test_heat_decays_with_age():
    ...
    recent = MemoryRecord(id="a", kind=MemoryKind.SEMANTIC, recall_count=3,
                          last_accessed_at=now.isoformat())
    old = MemoryRecord(id="b", kind=MemoryKind.SEMANTIC, recall_count=3,
                       last_accessed_at=(now - timedelta(days=120)).isoformat())
    assert recent.heat(now=now) > old.heat(now=now)


def test_heat_bounded():
    huge = MemoryRecord(id="a", kind=MemoryKind.SEMANTIC, recall_count=100_000,
                        visit_count=100_000,
...
def test_expire_by_category_drops_old_debug(svc):
    ...
    svc.put([MemoryRecord(id="pref.debug_note", kind=MemoryKind.SEMANTIC, value="temp",
                          confidence=0.9, source="service", category="debug")])
...
def test_expire_keeps_durable_facts(svc):
    ...
    svc.put([MemoryRecord(id="pref.editor", kind=MemoryKind.SEMANTIC, value="vim",
...
def test_expire_keeps_recent_event(svc):
    ...
    svc.put([MemoryRecord(id="pref.recent_event", kind=MemoryKind.SEMANTIC, value="x",
                          confidence=0.9, source="service", category="event",
...
def test_expire_never_touches_user_explicit_global(svc):
    ...
    svc.put([MemoryRecord(id="pref.kept", kind=MemoryKind.SEMANTIC, value="keep me",
                          confidence=1.0, source="user_explicit", category="event",
