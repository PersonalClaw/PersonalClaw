...
def test_sdlc_status_surfaces_block_reason():
    ...
    pid = [p.id for p in store.list_all() if p.kind == "code"][0]
    store.update_status(
        pid, LoopStatus.BLOCKED, error_message="Build gate failed: pytest exited 1 on test_health"
    )
    r = _run(sdlc_tools.sdlc_status({"id": pid}))
    assert r.success
...
def test_sdlc_status_flags_ended_early_complete():
    ...
    pid = [p.id for p in store.list_all() if p.kind == "code"][0]
    store.update_status(
        pid, LoopStatus.COMPLETE, error_message="Cycle budget exhausted before all stages cleared"
    )
    r = _run(sdlc_tools.sdlc_status({"id": pid}))
    assert r.success
