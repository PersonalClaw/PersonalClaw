"""Routes for session templates + conversation export (SESSION-MANAGEMENT S3).

Two S3 capabilities that share nothing but their place in the session lifecycle, kept in
one route module because both are thin HTTP over a pure module beside them
(``session_templates`` and ``session_export``).
...
from personalclaw.dashboard import session_export, session_templates
...
async def api_session_export(request: web.Request) -> web.Response:
    ...
    try:
        ...
        logger.exception("export: reading transcript failed for %r", key)
        return web.json_response({"error": "could not read this conversation"}, status=500)
...
def register_routes(app: web.Application) -> None:
    """Wire the S3 template + export routes.
