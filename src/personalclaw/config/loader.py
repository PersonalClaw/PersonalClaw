...
* ``config/coercion.py`` — ``_meta`` and the value coercers. The LEAF: it imports nothing from
  PersonalClaw, which is what lets the sections below depend on it without a cycle back here.
...
them in this file, and ~290 tests monkeypatch ``"personalclaw.config.loader.config_dir"`` and its
...
from personalclaw.config.learning import (
    EvalsConfig,
    FeedbackConfig,
    LearningConfig,
    LoopsConfig,
...
class AgentConfig:
    ...
    durable_sessions: bool = field(
        ...
        metadata=_meta(
            ...
            "session on PersonalClaw's own tmux socket. On restart the recovery sweep "
...
class MobileConfig:
    ...
    (:mod:`personalclaw.push`). ``ntfy_topic_url`` is nonetheless a *destination*, so it
    ...
    push_backend: str = field(
        ...
        metadata=_meta(
            ...
            "subscription (needs `personalclaw push init`); 'ntfy' publishes to a "
    ...
    ntfy_topic_url: str = field(
        ...
        metadata=_meta(
            "ntfy topic URL",
            "Full https URL of your ntfy topic (e.g. https://ntfy.example/personalclaw). "
...
class AppsConfig:
    ...
    * ``registry_source_enabled`` is a *seed* switch. On first start with it on, the curated
      ...
      (see :func:`personalclaw.apps.catalog.seed_default_git_sources`). Turning it off later
...
class AppConfig:
    ...
    def load_credentials(self) -> dict[str, str]:
        ...
        from personalclaw.config.credentials import _dotenv_credentials, _keychain_credentials
