...
async def _transcribe_segmented_detailed(
    ...
    try:
        ...
        cmd = [
            ...
            "-f", "segment", "-segment_time", str(_STT_SEGMENT_SECONDS), pattern,
        ]
        ...
        for idx, chunk in enumerate(chunks):
            ...
            for seg in part.segments:
                merged_segments.append(TranscriptSegment(
                    ...
                    text=seg.text,
                    speaker=seg.speaker,
                    words=[TranscriptWord(w.start + offset, w.end + offset, w.word, w.prob)
                           for w in seg.words],
                ))
            if part.text:
        ...
        return TranscriptResult(
            text=flat, language=lang_out, duration=total_duration, segments=merged_segments,
        )
    finally:
...
async def _transcribe_segmented(provider, model_id: str, language: str, audio_path: str) -> str | None:
    ...
    try:
        ...
        rc = await proc.wait()
        segments = sorted(
            os.path.join(work, f) for f in os.listdir(work) if f.startswith("seg_")
        )
        if rc != 0 or not segments:
            logger.warning("STT segmentation failed (rc=%s); falling back to single call", rc)
            return await provider.transcribe(audio_path, model=model_id, language=language)

        logger.info("STT: transcribing %d segments of %s", len(segments), os.path.basename(audio_path))
        parts: list[str] = []
        for seg in segments:
            try:
