...
def load_surface_token(surface: str) -> str | None:
    ...
    Environment first (``PERSONALCLAW_INBOUND_<SURFACE>_TOKEN``) so a container can
    inject it without writing a file, then the on-disk token.
    """
    env_key = f"PERSONALCLAW_INBOUND_{surface.upper()}_TOKEN"
...
def token_problem(surface: str) -> str | None:
    ...
    if not token:
        return "no token configured (run: personalclaw inbound token create mcp)"
...
def inbound_cmd(args) -> int:
    ...
    print("    personalclaw config set inbound.mcp.enabled true")
