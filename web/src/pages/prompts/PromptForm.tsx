...
export function PromptForm({ draft, onChange, compact, nameLocked, registerInsert }: { draft: PromptDraft; onChange: (d: PromptDraft) => void; compact?: boolean; nameLocked?: boolean; registerInsert?: (fn: (text: string) => void) => void }) {
  ...
  // placeholders in the body not yet declared as variables — typed inline
  // declarations ({{ name::select::[a, b] }}) ride through with their type and
  // options so the add-chip creates the row the declaration asks for (issue 596)
      ...
      {includes.length > 0 && (
        <div className="rounded-md px-m py-2" style={{ background: 'color-mix(in srgb, var(--color-info) 10%, transparent)' }}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {includes.map((n) => (
              <span key={n} data-type="caption" className="inline-flex items-center gap-1 rounded-pill bg-surface-high px-2 h-7 text-on-surface-var"><Puzzle size={11} /> <span className="font-mono">{n}</span></span>
            ))}
          </div>
        </div>
      )}
