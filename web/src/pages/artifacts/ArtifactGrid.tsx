...
export const ArtifactGrid = memo(function ArtifactGrid({ artifacts, onOpen, onBrowseFiles, narrowed }: {
  ...
  if (!artifacts.length) {
    return narrowed
      ...
      : <EmptyState icon={Box} title="No artifacts" hint="Artifacts are named, versioned snapshots — widgets, docs, images, and files agents produce. Ask the agent to create a document, spreadsheet or deck; text files can also be saved as an artifact from the Files page."
          action={{ label: 'Browse text files', onClick: onBrowseFiles, icon: FolderOpen }} />
  }
  return (
