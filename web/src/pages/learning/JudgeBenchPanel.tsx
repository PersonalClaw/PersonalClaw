...
import type { JudgeBenchRecommendation, JudgeBenchRow, JudgeBenchView, NotRunView, SwitchedOffView } from '../../lib/api'
...
export function JudgeBenchPanel({ bench, error, onRetry }: {
  bench: JudgeBenchView | SwitchedOffView | NotRunView | undefined
  error: unknown
  onRetry: () => void
}) {
  // The ordinary states here are "the substrate is off" and "no benchmark has run", both decided
  // 200s, and both render as guidance, not as a failure. Any error is surfaced: the panel's
  // subject is "can I trust the judge?", and a swallowed fetch would answer it with silence.
  ...
  if (bench === undefined && error) return <LoadError what="judge benchmark" error={error} onRetry={onRetry} />
