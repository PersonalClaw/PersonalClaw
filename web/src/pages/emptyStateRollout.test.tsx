...
const PEP2_CENSUS: {
  ...
  { surface: 'Inbox', file: 'pages/inbox/InboxPage.tsx', verdict: 'produced',
    why: 'Inbox zero is the good news, and items arrive from connected providers rather than being authored — so "produced" still holds and there is still no create button. AMENDED 2026-09-07: this row previously read "the connect flow is Settings › Inbox, NOT a create button on an empty queue", and that reasoning was right about the create button but was applied to one state too many. It reasoned only about the CONNECTED empty queue. On a fresh install nothing has ever arrived, and there “Inbox zero” is not good news — it is a congratulation for a triage state the user has never been in. The title now branches on `neverReceived` alongside the hint, and that ONE branch carries an offer to the destination this row itself names. AMENDED AGAIN 2026-09-24: the 09-07 amendment named the flag `disabled` (`!status.enabled`) and captioned that branch “Inbox is not connected yet — Enable a source to begin”, which is FALSE: `enabled` is the poll half only, the native agent→inbox sink is always live, and the page’s own banner says so on the same screen. Measured on a fresh container, that headline sat under “Native source active” and the header’s Capture-a-note then delivered an item into the “unconnected” inbox. The branch is kept, the flag is now an items fact, and the copy no longer diagnoses a setup problem the user does not have. It is a setup on-ramp, not a create flow: it authors no inbox item, which is why the verdict stays "produced" rather than becoming "on-ramp". The success branch and the no-match branch still deliberately carry no action, because manufacturing a CTA out of good news is exactly what this taxonomy exists to prevent.' },
  ...
  { surface: 'Chat › Session map drawer', file: 'pages/chat/SessionMapDrawer.tsx', verdict: 'derived',
    why: 'The Session Map is a projection of the TRANSCRIPT, and the transcript\'s create flow is the composer — on screen, one tap away, behind this drawer. So a CTA here would be a second entrance to the composer, which is the defect this taxonomy names on Companion › sections. The hint teaches the mechanism instead ("send a message"), and the branch exists at all only because the rail\'s own answer to the same threshold — render nothing — is wrong for a surface the user DELIBERATELY OPENED: a blank panel after a tap reads as broken. Reachable only in a started session with fewer than two marks, i.e. the single opening turn.' },
