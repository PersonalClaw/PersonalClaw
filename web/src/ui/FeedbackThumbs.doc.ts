...
const doc: UiDoc = {
  ...
  description:
    "A quiet 👍/👎 pair mounted on AI JUDGMENT outputs (inbox classification/draft/digest cards, loop finding rows, the synthesized-skill inspector). 👍 is silent-positive ('Mark accurate' — recorded only for the accuracy denominator, it never implies 'I'll learn from this'); 👎 opens an optional one-line 'why' popover (Enter/click-away records without a reason, Esc cancels). State hydrates from GET /api/feedback/target on mount, reflects as a filled thumb, and is reversible (re-thumbing supersedes). Renders nothing when the feedback kill-switch is off (the hydrating read answers `{\"enabled\": false}` → hide).",
  props: [
    { name: 'targetKind', type: "FeedbackTargetKind", required: true, description: "The judgment kind — a member of the CLOSED vocabulary (inbox_classification | inbox_draft | inbox_digest | loop_finding | routing_suggestion | proposal_content | app_judgment | synthesized_skill). A new surface must extend the backend vocabulary first." },
    { name: 'targetId', type: 'string', required: true, description: 'The judged thing (inbox item id, finding path, suggestion id, skill key…).' },
