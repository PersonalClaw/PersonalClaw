...
const doc: UiDoc = {
  ...
  description:
    'A spotlight ("coach mark") tour over the real, mounted UI: the page dims except for a ring around one element, and a card beside it says what that element is. Each stop names an anchor, which it finds as `[data-tour="<anchor>"]` on the live DOM and re-measures on scroll/resize. It is a modal dialog because the page underneath is dimmed — focus is trapped in the card and re-taken on every stop, Escape exits from anywhere, and a click outside the card ends the tour rather than reaching a control the dim layer was covering. It never navigates and never persists anything: the host owns the step index, the routing and any record that the tour was taken.',
  props: [
  ...
  bestPractices: [
    ...
    { guidance: false, description: 'Do not rely on the spotlight to gate anything: every stop is skippable, Escape works from all of them, and a stop whose anchor is missing degrades to a centred card with no ring.' },
