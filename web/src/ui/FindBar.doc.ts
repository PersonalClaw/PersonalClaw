...
const doc: UiDoc = {
  ...
  props: [
    ...
    { name: 'initialQuery', description: 'Optional text to PRE-SEED the field with, read once at mount like a defaultValue. A seeded bar debounces, highlights, and brings its first match into view on its own — the scroll pressing ↓ once would give — which is how a chat-history search result deep-links into a conversation already scrolled to what it matched. Omit it (the default) and the bar opens empty and waits for the user to type. To seed a NEW term into an already-open bar, remount it (give the instance a key that changes with the term); a live prop change is deliberately ignored so it never overwrites what the user is typing.' },
