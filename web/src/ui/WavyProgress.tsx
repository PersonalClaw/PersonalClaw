...
import { instant, prefersReducedMotion } from '../design/motion'
 ...
 *
 *  🔴 REDUCED MOTION IS DECIDED HERE, THROUGH `design/motion`'s ONE ACCESSOR.
 *  This component shipped with NO reduced-motion branch at all, and the root
 *  `<MotionConfig reducedMotion="user">` does not cover it: that config neutralises
 *  framer TRANSFORMS, and neither `pathOffset` (the traveling crest) nor `pathLength`
 *  (the determinate fill) is a transform. So the crest traveled forever for a user who
 *  had asked the platform for less motion — an indefinite animation, which is the one
 *  kind the setting exists to stop.
 *
 *  🪤 AND `instant` ALONE CANNOT FIX THE INDETERMINATE HALF. Collapsing its transition
 *  to `{ duration: 0 }` while `repeat: Infinity` stands leaves an unbounded loop of
 *  zero-length animations — busier than the animation it replaced. An indefinite
 *  animation has to be NOT STARTED, not shortened, so that branch drops `animate`
 *  entirely and renders a plain `<path>` with no framer component in the tree. The
 *  determinate half is a one-shot tween to a known target, so there `instant` is
 *  exactly right: the fill lands at `value` with zero duration.
 *
 *  `data-wavy-progress` states which branch ran, because the family's rail
 *  (`ui/motion/family.reducedMotion.test.tsx`) asserts the decision from the DOM
  ...
  if (value == null) {
    return (
      <svg
        ...
        fill="none"
        aria-hidden
        data-wavy-progress={reduce ? 'instant' : 'animated'}
      >
        {reduce ? (
          // The whole wave, at rest. Not the frozen 60% crest the animated branch
          // starts from: a stopped partial segment reads as a STALLED bar, and this
          // mode has no progress to report — the caller's own line ("downloading ·
          // 120 / 400 MB") carries the state, which is also why the svg is aria-hidden.
  ...
  return (
    ...
    >
      ...
      <motion.path
        ...
        transition={reduce ? instant : { ease: 'easeOut', duration: 0.4 }}
      />
    </svg>
  )
}
