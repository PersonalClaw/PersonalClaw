...
const docs: UiDoc[] = [
  ...
  },
  {
    ...
    props: [
      ...
      { name: 'maxLength', description: 'Native maxlength. For a field the BACKEND bounds, pass the same limit here so the cap is reachable in the field instead of arriving only as a rejected save — a project name is capped at 200 server-side.' },
      ...
      { name: 'type', description: "'text' (default), 'password' to mask a secret (API keys, tokens), or 'number' for the full-width numeric field. 'number' is NOT a second NumberField: that stepper owns the dense right-aligned role and clamp-on-commit, and its numeric `value` cannot express \"unset\" — pick it for a stepper beside a label, pick this for a nullable schema-driven field that must stay clearable." },
      { name: 'minLength', description: 'Native minlength — maxLength’s pair. A schema-declared floor belongs on the control, not only in the handler that rejects the save.' },
      { name: 'pattern', description: 'Native pattern. Same reasoning as the length bounds: a declared shape belongs on the field. Deliberately withheld on a sensitive field — a regex fights a paste and the browser’s bubble would describe the secret’s shape.' },
      { name: 'min', description: 'Native min — the lower numeric bound, meaningful only with type="number".' },
      { name: 'max', description: 'Native max — the upper numeric bound, meaningful only with type="number".' },
      { name: 'trailingSlot', description: 'An interactive affordance pinned inside the RIGHT edge (a show/hide-secret eye, a spinner); adds the canonical pr-10 inset that clears it. Unlike leadingIcon it is NOT pointer-transparent, because it holds a real control — so pass the button itself. Mirrors ui/SearchField’s trailingSlot.' },
      { name: 'id', description: 'An explicit DOM id, for a call-site that publishes its OWN visible `<label htmlFor>` instead of wrapping the control in a Field. Without it the id is internal, so such a label resolves to nothing and the visible text names the control for sighted users only. Wins over `name`.' },
  ...
  },
  {
    ...
    props: [
      ...
      { name: 'surface', description: "Fill token, same vocabulary as TextInput — 'container' (default), 'high', 'base'. TextArea and Select were the family's two fixed-fill fields, which showed as drift inside one form: a stack whose text fields rode `high` had its one multi-line row rendering `container`." },
  ...
  },
  {
    ...
    props: [
      ...
      { name: 'surface', description: "Fill token, same vocabulary as TextInput — 'container' (default), 'high', 'base'. Select hardcoded container, which made it the family's only fixed-fill field, so a panel-on-panel form whose text fields rode `high` had to drop to bespoke chrome to keep its select matching." },
      { name: 'id', description: 'An explicit DOM id for a call-site publishing its own visible `<label htmlFor>`, exactly TextInput’s. Wins over `name`.' },
