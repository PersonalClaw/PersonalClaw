...
const doc: UiDoc = {
  ...
  props: [
    ...
    { name: 'contextPct', description: '0–100 context-window usage for the bound session → ring on the model pill. Omit it (undefined) when the backend reported no measurement OR could not resolve the model’s window — the pill shows a plain dot whose title says the usage is unknown, never a fabricated 0%. 100 means the context is full (including a provider that silently truncated the prompt); the value never exceeds 100 and never falls as the prompt grows.' },
