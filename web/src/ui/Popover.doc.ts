...
const docs: UiDoc[] = [
  {
    ...
    props: [
      ...
      { name: 'tabIndex', description: 'Roving-tabindex slot for a container that moves focus row to row (`lib/useMenuCursor`): 0 on the cursor row, -1 on the rest, so the menu is ONE tab stop rather than one per row. Omit inside a click-driven popover, where a button\'s default 0 is right.' },
