...
//
// Measured on `#/onboarding` (cycle 193): the step indicator carried no `aria-current`, and step
// transitions had no live region — so a screen-reader user was never told which of the three steps
// was live or that they had advanced. The rows are `<div>`s (not focusable) and the active step's
// title is not in any focused control's accessible name, so neither piece could be inferred another
// way. Two WAI-ARIA fixes, both asserted here:
//
